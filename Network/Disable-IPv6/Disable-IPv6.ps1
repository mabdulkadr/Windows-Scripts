...
try {
    ...
    # Tailored display: adapter IPv6 state
    try {
        $adapters = Get-NetAdapterBinding -ComponentID ms_tcpip6 -ErrorAction SilentlyContinue | Select-Object Name, Enabled -First 10
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- IPv6 Binding --" -ForegroundColor DarkGray
