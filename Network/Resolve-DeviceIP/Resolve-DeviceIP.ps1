<#
.TITLE
    Resolve-DeviceIP - Resolve device names to IPv4 addresses with ping status

.SYNOPSIS
    Resolve device names to IPv4 addresses with ping status

.DESCRIPTION
    Uses Resolve-DnsName and Test-Connection to confirm reachability of each name.
...
.EXAMPLE
    .\Resolve-DeviceIP.ps1
    Runs with default targets.

.EXAMPLE
    .\Resolve-DeviceIP.ps1 -TargetName "Server01","Server02"
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Resolve IP')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $ip = (Resolve-DnsName $Target -Type A -ErrorAction Stop).IPAddress | Select-Object -First 1; if (-not $ip) { throw "No A record for $Target" }
...
try {
    ...
    # Tailored display: DNS + ping
    try {
        $names = if ($TargetName) { $TargetName } else { @('localhost','google.com') }
        $rows = foreach ($n in $names) {
            $ip = try { (Resolve-DnsName $n -Type A -ErrorAction Stop).IPAddress | Select-Object -First 1 } catch { "no-record" }
            $ping = Test-Connection -ComputerName $n -Count 1 -Quiet -ErrorAction SilentlyContinue
            [PSCustomObject]@{ Name=$n; IP=$ip; Ping=$ping }
        }
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Resolve + Ping --" -ForegroundColor DarkGray
        $rows | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
    } catch {
        Write-Log -Message "Could not resolve devices: $($_.Exception.Message)" -Level 'WARNING'
    }
