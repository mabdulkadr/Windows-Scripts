...
**Resolve-DeviceIP** is a PowerShell CLI tool in the **Network** category of the **Windows-Scripts** enterprise toolkit. Uses Resolve-DnsName and Test-Connection to confirm reachability of each name.

**Category:** `Network`
**Script:** `Resolve-DeviceIP.ps1`
**Path:** `Network\Resolve-DeviceIP\Resolve-DeviceIP.ps1`
...
```powershell
Get-Help .\Resolve-DeviceIP.ps1 -Full
Get-Help .\Resolve-DeviceIP.ps1 -Examples
```

### Dry Run

```powershell
.\Resolve-DeviceIP.ps1 -WhatIf
```
...
|-----------|------|----------|---------|-------------|
| `TargetName` | `string[]` | No | `localhost` | One or more target device names. |
