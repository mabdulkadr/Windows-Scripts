<#
.TITLE
    Test-InternetConnectivity - Test connectivity to common Microsoft hosts

.SYNOPSIS
    Test connectivity to common Microsoft hosts

.DESCRIPTION
    Uses Invoke-WebRequest to verify reachability of Microsoft endpoints.
...
.EXAMPLE
    .\Test-InternetConnectivity.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Network,Connectivity
...
$SolutionName = 'Test-InternetConnectivity'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Test connectivity')) {
...
try {
    ...
    # Tailored display: internet endpoints
    try {
        $targets = @('8.8.8.8','1.1.1.1','microsoft.com')
        $rows = foreach ($t in $targets) {
            $ok = Test-Connection -ComputerName $t -Count 1 -Quiet -ErrorAction SilentlyContinue
            $lat = try { (Test-Connection -ComputerName $t -Count 1 -ErrorAction SilentlyContinue).Latency } catch { $null }
            [PSCustomObject]@{ Target=$t; Reachable=$ok; LatencyMs=$lat }
        }
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Internet Connectivity --" -ForegroundColor DarkGray
        $rows | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
    } catch {
        Write-Log -Message "Could not test internet: $($_.Exception.Message)" -Level 'WARNING'
    }
