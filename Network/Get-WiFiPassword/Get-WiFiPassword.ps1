<#
.TITLE
    Get-WiFiPassword - Retrieve saved Wi-Fi SSIDs and passwords (audit)

.SYNOPSIS
    Retrieve saved Wi-Fi SSIDs and passwords (audit)

.DESCRIPTION
    Enumerates saved Wi-Fi profiles via netsh wlan show profiles and extracts cleartext passwords (key=clear) for audit. Requires Administrator for password reveal; without elevation lists SSIDs only.
...
.PERMISSIONS
    Administrator for passwords; Standard user for SSID list
...
.EXAMPLE
    .\Get-WiFiPassword.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Network,WiFi,Audit
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Query WiFi')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $profiles = (netsh wlan show profiles) 2>&1 | Select-String "All User Profile" | ForEach-Object { ($_ -split ":")[1].Trim().Trim('"') }
        if (-not $profiles) { throw "No Wi-Fi profiles found" }
...
try {
    ...
    # Tailored display: Wi-Fi profiles
    try {
        $profiles = (netsh wlan show profiles) 2>&1 | Select-String "All User Profile" | ForEach-Object { ($_ -split ":")[1].Trim().Trim('"') }
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Saved Wi-Fi Profiles --" -ForegroundColor DarkGray
        if ($profiles) {
            foreach ($ssid in $profiles | Select-Object -First 10) {
                $detail = netsh wlan show profile name="$ssid" key=clear 2>&1 | Out-String
                $pass = if ($detail -match "Key Content\s*:\s*(.+)") { $Matches[1].Trim() } else { "(hidden - run as Admin)" }
                [PSCustomObject]@{ SSID=$ssid; Password=$pass } | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
            }
            Write-Log -Message "Audit only - do not share passwords" -Level 'WARNING'
        } else { Write-Log -Message "No Wi-Fi profiles stored" -Level 'INFO' }
    } catch {
        Write-Log -Message "Could not query Wi-Fi: $($_.Exception.Message)" -Level 'WARNING'
    }
