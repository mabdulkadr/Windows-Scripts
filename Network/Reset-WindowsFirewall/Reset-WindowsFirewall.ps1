...
.DESCRIPTION
    Runs netsh advfirewall reset to restore the default firewall configuration.
...
.EXAMPLE
    .\Reset-WindowsFirewall.ps1
    Runs with default targets.

.EXAMPLE
    .\Reset-WindowsFirewall.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Network,Firewall
...
$SolutionName = 'Reset-WindowsFirewall'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Reset firewall')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        & netsh advfirewall reset | Out-Null; if ($LASTEXITCODE -ne 0) { throw "netsh advfirewall reset failed" }
...
try {
    ...
    # Tailored display: comprehensive firewall reset verification
    try {
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Firewall Profiles (after reset) --" -ForegroundColor DarkGray
        ...
        if ($fw) {
            $fw | ForEach-Object {
                [PSCustomObject]@{
                    ...
                    AllowInbound     = $_.AllowInboundRules
                    LogFile          = $_.LogFileName
                    LogSizeKB        = $_.LogMaxSizeKilobytes
                }
            } | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }

            $enabledCount = ($fw | Where-Object { $_.Enabled }).Count
            Write-Host ("  Enabled profiles: {0}/{1}" -f $enabledCount, $fw.Count) -ForegroundColor $(if ($enabledCount -eq $fw.Count) {'Green'} else {'Yellow'})
            if ($enabledCount -eq $fw.Count) { Write-Host "  Firewall is ON (default after reset - secure)" -ForegroundColor Green }
        }

        Write-Host "  -- Firewall Rules Summary --" -ForegroundColor DarkGray
        $rules = Get-NetFirewallRule -ErrorAction SilentlyContinue
        if ($rules) {
            $byDir = $rules | Group-Object Direction | ForEach-Object { [PSCustomObject]@{ Direction=$_.Name; Count=$_.Count } }
            $byAct = $rules | Group-Object Action | ForEach-Object { [PSCustomObject]@{ Action=$_.Name; Count=$_.Count } }
            Write-Host "  By Direction:" -ForegroundColor Gray
            $byDir | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Gray }
            Write-Host "  By Action:" -ForegroundColor Gray
            $byAct | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Gray }
            Write-Host "  Total rules: $($rules.Count)" -ForegroundColor Cyan
        }

        Write-Host "  -- Reset Verification --" -ForegroundColor DarkGray
        $resetOk = ($fw | Where-Object { $_.Enabled }).Count -eq $fw.Count
        Write-Host ("  Reset status: {0}" -f $(if ($resetOk) {"SUCCESS - all profiles enabled (default)"} else {"PARTIAL - some profiles still disabled"})) -ForegroundColor $(if ($resetOk) {'Green'} else {'Yellow'})
        Write-Host "  Command: netsh advfirewall reset (restores defaults, removes custom rules)" -ForegroundColor Gray
        Write-Host "  Note: Custom app rules removed - re-add via GPO or Set-NetFirewallRule" -ForegroundColor Yellow

    } catch {
        Write-Log -Message "Could not query firewall after reset: $($_.Exception.Message)" -Level 'WARNING'
    }
