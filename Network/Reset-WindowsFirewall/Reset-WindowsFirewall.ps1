...
try {
    ...
    # Tailored display: firewall after reset
    try {
        $fw = Get-NetFirewallProfile -ErrorAction SilentlyContinue | Select-Object Name, Enabled, DefaultInboundAction
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Firewall After Reset --" -ForegroundColor DarkGray
        if ($fw) { $fw | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
    } catch {
