<#
.TITLE
    Reset-Winsock - Reset the Winsock catalog

.SYNOPSIS
    Reset the Winsock catalog

.DESCRIPTION
    Runs netsh winsock reset to recover the network stack.
...
.EXAMPLE
    .\Reset-Winsock.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Network,Winsock
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Reset Winsock')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        & netsh winsock reset | Out-Null; if ($LASTEXITCODE -ne 0) { throw "netsh winsock reset failed with $LASTEXITCODE" }
...
try {
    ...
    # Tailored display: comprehensive Winsock diagnostics
    try {
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Winsock Catalog (before reset) --" -ForegroundColor DarkGray
        try {
            $catalog = netsh winsock show catalog 2>&1 | Out-String
            $entries = $catalog -split "`r`n" | Where-Object { $_ -match "^\s*\d+\s+" } | Select-Object -First 8
            if ($entries) {
                Write-Host "  Catalog entries (first 8):" -ForegroundColor Gray
                $entries | ForEach-Object { Write-Host "    $_" -ForegroundColor Gray }
            } else {
                Write-Host "  Catalog: $($catalog.Substring(0,[Math]::Min(300,$catalog.Length)))" -ForegroundColor Gray
            }
        } catch { Write-Host "  Could not show catalog: $($_.Exception.Message)" -ForegroundColor Yellow }

        Write-Host "  -- Network Adapters (post-reset check) --" -ForegroundColor DarkGray
        $adapters = Get-NetAdapter -ErrorAction SilentlyContinue | Select-Object Name, Status, LinkSpeed -First 5
        if ($adapters) { $adapters | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }

        Write-Host "  -- Winsock LSPs (Layered Service Providers) --" -ForegroundColor DarkGray
        $lspPath = 'HKLM:\SYSTEM\CurrentControlSet\Services\WinSock2\Parameters\Protocol_Catalog9\Catalog_Entries'
        if (Test-Path $lspPath) {
            $lspCount = (Get-ChildItem -Path $lspPath -ErrorAction SilentlyContinue | Measure-Object).Count
            Write-Host "  LSP entries: $lspCount" -ForegroundColor Cyan
            Get-ChildItem -Path $lspPath -ErrorAction SilentlyContinue | Select-Object -First 3 | ForEach-Object {
                $packed = Get-ItemProperty -Path $_.PSPath -ErrorAction SilentlyContinue
                Write-Host ("    {0}: {1}" -f $_.PSChildName, $packed.PackedCatalogItem.Substring(0,[Math]::Min(50,$packed.PackedCatalogItem.Length))) -ForegroundColor Gray
            }
        }

        Write-Host "  -- Reset Result --" -ForegroundColor DarkGray
        Write-Host "  Command: netsh winsock reset (requires reboot)" -ForegroundColor Cyan
        Write-Host "  Status: Executed via Invoke-TargetAction - reboot pending" -ForegroundColor Yellow
        Write-Host "  Verify after reboot: netsh winsock show catalog | Measure-Object" -ForegroundColor Gray
        Write-Host "  Also consider: netsh int ip reset  +  ipconfig /flushdns" -ForegroundColor Gray

    } catch {
        Write-Log -Message "Could not check winsock: $($_.Exception.Message)" -Level 'WARNING'
    }
