...
try {
    ...
    # Tailored display: winsock catalog
    try {
        $catalog = Get-NetWinsockReset -ErrorAction SilentlyContinue 2>&1 | Out-String
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Winsock --" -ForegroundColor DarkGray
        Write-Host "  netsh winsock reset simulated - reboot may be required" -ForegroundColor Cyan
    } catch {
