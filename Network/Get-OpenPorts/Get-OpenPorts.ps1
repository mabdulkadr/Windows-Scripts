<#
.TITLE
    Get-OpenPorts - Enumerate open TCP and UDP listening ports

.SYNOPSIS
    Enumerate open TCP and UDP listening ports

.DESCRIPTION
    Combines Get-NetTCPConnection (Listen) and Get-NetUDPEndpoint to surface local listening ports.
...
.EXAMPLE
    .\Get-OpenPorts.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Network,Ports
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Enumerate ports')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $null = Get-NetTCPConnection -State Listen -ErrorAction SilentlyContinue | Select-Object -First 1
...
try {
    ...
    # Tailored display: listening ports
    try {
        $ports = Get-NetTCPConnection -State Listen -ErrorAction SilentlyContinue | Select-Object LocalAddress, LocalPort, OwningProcess -First 10
        $udp = Get-NetUDPEndpoint -ErrorAction SilentlyContinue | Select-Object LocalAddress, LocalPort -First 5
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Listening Ports (TCP top 10) --" -ForegroundColor DarkGray
        if ($ports) { $ports | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
        if ($udp) { Write-Host "  -- UDP (top 5) --" -ForegroundColor DarkGray; $udp | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Gray } }
    } catch {
        Write-Log -Message "Could not enumerate ports: $($_.Exception.Message)" -Level 'WARNING'
    }
    # Export professional HTML report
    try {
        $ports = Get-NetTCPConnection -State Listen -ErrorAction SilentlyContinue | Select-Object LocalAddress, LocalPort, State, OwningProcess -First 20
        $portCount = @($ports).Count
        $portKpis = @(@{value=$portCount; label='Listening Ports'; color='#0f62fe'}, @{value=$ok; label='Checks OK'; color='#24a148'}, @{value=$failed; label='Failed'; color='#da1e28'}, @{value=$env:COMPUTERNAME; label='Device'; color='#8a3ffc'})
        $portRows = if($ports){ $h=($ports[0].PSObject.Properties | ForEach-Object { "<th>$($_.Name)</th>" }) -join ""; $b=($ports | ForEach-Object { $cs=($_.PSObject.Properties | ForEach-Object { "<td>$([System.Net.WebUtility]::HtmlEncode("$($_.Value)"))</td>" }) -join ""; "<tr>$cs</tr>" }) -join "`n"; "<table><thead><tr>$h</tr></thead><tbody>$b</tbody></table>" } else { "<p style=`"color:#8d8d8d;padding:16px`">No listening ports found</p>" }
        $portBody = "<div class=`"section-title`">Open Ports Inventory</div><div class=`"card`"><h2>Listening Ports ($portCount)</h2>$portRows</div>"
        $portBase = if ($PSScriptRoot) { $PSScriptRoot } elseif ($PSCommandPath) { Split-Path -Parent $PSCommandPath } else { (Get-Location).Path }
        $portHtml = Join-Path $portBase "Reports\OpenPorts_$(Get-Date -Format yyyyMMdd_HHmmss).html"
        Export-ProfessionalHtmlReport -OutputPath $portHtml -Title 'Open Ports' -Subtitle "Listening TCP ports inventory" -Kpis $portKpis -Body $portBody -ReportName 'Open Ports' -Version '2.1.0'
        Write-Log -Message "HTML report: $portHtml" -Level 'SUCCESS'
