...
try {
    ...
    try {
        $keys = @('HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\TLS 1.0\Server','HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\TLS 1.2\Server','HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\SCHANNEL\Protocols\TLS 1.3\Server')
        $rows = foreach ($k in $keys) { $v = Get-ItemProperty -Path $k -ErrorAction SilentlyContinue; [PSCustomObject]@{ Key=Split-Path $k -Leaf; Enabled=(if ($null -ne $v.Enabled) { $v.Enabled } else { 'NotSet' }); DisabledByDefault=(if ($null -ne $v.DisabledByDefault) { $v.DisabledByDefault } else { 'NotSet' }) } }
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- TLS Server Settings --" -ForegroundColor DarkGray
