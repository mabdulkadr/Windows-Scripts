...
try {
    ...
    # Tailored display: DNS cache count before/after
    try {
        ...
        Write-Host "  -- DNS Cache --" -ForegroundColor DarkGray
        [PSCustomObject]@{ Before=$before; After=$after } | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
    } catch {
