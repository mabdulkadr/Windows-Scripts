<#
.TITLE
    Disable-WindowsFirewall - Disable Windows Firewall on all profiles (testing only)

.SYNOPSIS
    Disable Windows Firewall on all profiles (testing only)

.DESCRIPTION
    Calls Set-NetFirewallProfile with -Enabled False on every profile.
...
.EXAMPLE
    .\Disable-WindowsFirewall.ps1 -TargetName "Server01","Server02"
...
$SolutionName = 'Disable-WindowsFirewall'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Disable firewall')) {
...
try {
    ...
    # Tailored display: comprehensive firewall diagnostics
    try {
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Firewall Profiles (after disable) --" -ForegroundColor DarkGray
        ...
        if ($fw) {
            ...
            } | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor $(if ($_ -match "False") {'Yellow'} else {'Cyan'}) }

            $disabledCount = ($fw | Where-Object { -not $_.Enabled }).Count
            $total = $fw.Count
            Write-Host ("  Disabled: {0}/{1} profiles" -f $disabledCount, $total) -ForegroundColor $(if ($disabledCount -eq $total) {'Yellow'} else {'Cyan'})
            if ($disabledCount -eq $total) {
                Write-Host "  WARNING: All firewall profiles are DISABLED - device is exposed!" -ForegroundColor Red
                Write-Host "  Re-enable: Enable-NetFirewallProfile -All  or  .\Reset-WindowsFirewall.ps1" -ForegroundColor Yellow
            }
        }

        Write-Host "  -- Firewall Rules (sample) --" -ForegroundColor DarkGray
        $rules = Get-NetFirewallRule -ErrorAction SilentlyContinue | Select-Object -First 5
        if ($rules) {
            $rules | Select-Object DisplayName, Direction, Action, Enabled | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Gray }
            $totalRules = (Get-NetFirewallRule -ErrorAction SilentlyContinue | Measure-Object).Count
            Write-Host "  Total rules: $totalRules" -ForegroundColor Gray
        }

        Write-Host "  -- Security Warning --" -ForegroundColor DarkGray
        Write-Host "  This script disables firewall for testing only - do not use on production!" -ForegroundColor Red
        Write-Host "  Recommended: Use Reset-WindowsFirewall.ps1 to restore defaults after testing" -ForegroundColor Yellow

    } catch {
        Write-Log -Message "Could not query firewall: $($_.Exception.Message)" -Level 'WARNING'
    }
