<#
.TITLE
    Sync-IntuneDevice - Trigger an immediate Intune device sync

.SYNOPSIS
    Trigger an immediate Intune device sync

.DESCRIPTION
    Restarts the IntuneManagementExtension service to force IME resync.

.TAGS
    Endpoint,Intune,Sync
...
.EXAMPLE
    .\Sync-IntuneDevice.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Endpoint,Intune,Sync
...
$SolutionName = 'Sync-IntuneDevice'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Sync IME')) {
...
try {
    ...
    # Tailored display: last sync time
    try {
        $key = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Enrollments\*' -ErrorAction SilentlyContinue | Select-Object -First 1
        $task = Get-ScheduledTask -TaskName "PushLaunch" -ErrorAction SilentlyContinue
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Intune Sync Context --" -ForegroundColor DarkGray
        if ($task) { $task | Select-Object TaskName, State, LastRunTime | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
        else { Write-Log -Message "PushLaunch task not found" -Level 'WARNING' }
    } catch {
        Write-Log -Message "Could not query sync context: $($_.Exception.Message)" -Level 'WARNING'
    }
