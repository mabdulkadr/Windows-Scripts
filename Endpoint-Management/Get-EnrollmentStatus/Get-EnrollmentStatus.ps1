...
.SYNOPSIS
    Report Intune enrollment state

.DESCRIPTION
    Queries MDM enrollment keys and emits a structured summary of each enrollment state, provider, and last check time.

.TAGS
    Endpoint,Intune,Enrollment
...
.EXAMPLE
    .\Get-EnrollmentStatus.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Endpoint,Intune,Enrollment
...
$SolutionName = 'Get-EnrollmentStatus'
$ScriptMode   = 'run'
...
function Export-ProfessionalHtmlReport {
    ...
    param(
        ...
        [Parameter(Mandatory = $false)][string]$Disclaimer = 'This report is auto-generated by Windows-Scripts. Test in staging before relying on it for production decisions.',
        [Parameter(Mandatory = $false)][string]$RunId = ([guid]::NewGuid().ToString().Substring(0, 8))
    )
    $kpiHtml = ''
    if ($Kpis -and $Kpis.Count -gt 0) {
        $cards = foreach ($k in $Kpis) {
            $v = if ($null -ne $k.value) { [string]$k.value } else { '' }
            $l = if ($k.label) { [string]$k.label } else { '' }
            $c = if ($k.color) { [string]$k.color } else { '#0f62fe' }
            $t = if ($k.trend) { [string]$k.trend } else { '' }
            $i = if ($k.icon) { [string]$k.icon } else { '' }
            $tH = if ($t) { "<div class=`"kpi-trend`">$t</div>" } else { '' }
            $iH = if ($i) { "<div class=`"kpi-icon`" style=`"color:$c`">$i</div>" } else { '' }
            "<div class=`"kpi-card`" style=`"border-left:4px solid $c`">$iH<div class=`"kpi-value`" style=`"color:$c`">$v</div><div class=`"kpi-label`">$l</div>$tH</div>"
        }
        $kpiHtml = "<div class=`"kpi-row`">$($cards -join "`n")</div>"
    }
    $gradeHtml = ''
    if ($Grade) {
        $gc = if ($GradeColor) { $GradeColor } else { '#0f62fe' }
        $gt = if ($GradeTip) { $GradeTip } else { '' }
        $gr = if ($GradeRate) { " &#183; $GradeRate" } else { '' }
        $gradeHtml = "<div class=`"grade-badge`" style=`"background:$gc`"><div class=`"grade-letter`">$Grade</div><div class=`"grade-meta`">$gr</div><div class=`"grade-tip`">$gt</div></div>"
    }
    $chartsHtml = ''; $chartScripts = ''
    if ($Charts) {
        try {
            $cd = $Charts | ConvertFrom-Json -ErrorAction SilentlyContinue
            $dId = 'chart-donut'; $hasDonut = $cd.donut -and @($cd.donut).Count -gt 0; $hasBars = $cd.bars -and @($cd.bars).Count -gt 0
            if ($hasDonut -or $hasBars) {
                $blks = @()
                if ($hasDonut) {
                    $leg = ($cd.donut | ForEach-Object { $cc = if ($_.color) { $_.color } else { '#0f62fe' }; "<li><span class=`"legend-swatch`" style=`"background:$cc`"></span>$($_.label) <strong>$($_.value)</strong></li>" }) -join ''
                    $blks += "<div class=`"card`"><h2>Distribution</h2><div class=`"chart-wrap`"><canvas id=`"$dId`" width=`"320`" height=`"320`"></canvas></div><ul class=`"legend`">$leg</ul></div>"
                }
                if ($hasBars) {
                    $mx = [Math]::Max((@($cd.bars | ForEach-Object { [double]$_.value } | Measure-Object -Maximum).Maximum), 1)
                    $bis = ($cd.bars | ForEach-Object { $pc = [Math]::Round(([double]$_.value / $mx) * 100, 1); $cc = if ($_.color) { $_.color } else { '#0f62fe' }; "<div class=`"bar-row`"><div class=`"bar-label`">$($_.label)</div><div class=`"bar-track`"><div class=`"bar-fill`" style=`"width:${pc}%;background:$cc`"></div></div><div class=`"bar-value`">$($_.value)</div></div>" }) -join ''
                    $blks += "<div class=`"card`"><h2>Top Items</h2><div class=`"bar-chart`">$bis</div></div>"
                }
                $chartsHtml = "<div class=`"grid-2`">$($blks -join "`n")</div>"
                if ($hasDonut) {
                    $dData = ($cd.donut | ForEach-Object { $cc = if ($_.color) { $_.color } else { '#0f62fe' }; "{label:'$($_.label)',value:$([double]$_.value),color:'$cc'}" }) -join ','
...
@media print{body{background:#fff;color:#000}.card,.kpi-card,details{background:#fff;border-color:#ddd}thead th{background:#f5f5f5;color:#000;border-color:#999}tbody td{color:#222;border-color:#eee}.header{border-color:#999}.footer{border-color:#999;color:#666}.section-title::before{background:#0f62fe}.legend-swatch{border:1px solid #999}.bar-track{background:#eee}.page{padding:16px}.grade-badge{box-shadow:none;border:2px solid #000}}
</style></head><body><div class="page">
<div class="header"><div class="header-main"><h1>$Title</h1><div class="subtitle">$hdrSub</div></div>$grBar<div class="header-meta"><div class="meta-row"><span class="meta-label">Generated:</span><span class="meta-value">$GeneratedAt</span></div><div class="meta-row"><span class="meta-label">Computer:</span><span class="meta-value">$ComputerName</span></div><div class="meta-row"><span class="meta-label">Operator:</span><span class="meta-value">$Operator</span></div><div class="meta-row"><span class="meta-label">Run ID:</span><span class="meta-value">$RunId</span></div></div></div>
<div class="search-bar"><input type="text" class="search-input" id="reportSearch" placeholder="Filter tables and sections..."><span class="search-meta" id="searchMeta"></span></div>
$kpiHtml
$chartsHtml
$Body
...
"@
    $par = Split-Path -Parent $OutputPath
    if ($par -and -not (Test-Path -LiteralPath $par)) { $null = [System.IO.Directory]::CreateDirectory($par) }
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Query enrollment')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $null = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Enrollments\*' -ErrorAction SilentlyContinue | Select-Object -First 1
...
try {
    ...
    # Tailored display: show actual enrollment data + export professional HTML
    try {
        $enrollments = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Enrollments\*' -ErrorAction SilentlyContinue | Select-Object PSChildName, EnrollmentState, ProviderId, DeviceId -First 20
        if ($enrollments) {
            Write-Host "" -ForegroundColor Gray
            Write-Host "  -- Enrollment Details --" -ForegroundColor DarkGray
            $enrollments | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
        } else {
            Write-Log -Message "No MDM enrollments found on this device" -Level 'WARNING'
        }
        try {
            $enRows = $enrollments | Select-Object -First 100
            $enCount = @($enRows).Count; $enActive = @($enRows | Where-Object { $_.EnrollmentState -eq 1 }).Count
            $kpis = @(
                @{value=$enCount; label='Enrollments'; color='#0f62fe'},
                @{value=$enActive; label='Active (State=1)'; color='#24a148'},
                @{value=($enCount - $enActive); label='Inactive / Other'; color='#da1e28'},
                @{value=$env:COMPUTERNAME; label='Device'; color='#8a3ffc'}
            )
            $stateChart = @{donut=@(@{label='Active';value=$enActive;color='#24a148'},@{label='Inactive';value=($enCount-$enActive);color='#da1e28'})} | ConvertTo-Json -Compress
            $tableHtml = if ($enRows) {
                $h = ($enRows[0].PSObject.Properties | ForEach-Object { "<th>$($_.Name)</th>" }) -join ""
                $b = ($enRows | ForEach-Object { $cs = ($_.PSObject.Properties | ForEach-Object { $v=[System.Net.WebUtility]::HtmlEncode("$($_.Value)"); if ($_.Name -eq 'EnrollmentState') { $badge=if ($_.Value -eq '1') {'success'}else{'neutral'}; "<td><span class=`"badge $badge`">$v</span></td>" } else { "<td>$v</td>" } }) -join ""; "<tr>$cs</tr>" }) -join "`n"
                "<table><thead><tr>$h</tr></thead><tbody>$b</tbody></table>"
            } else { "<p style=`"color:#8d8d8d;padding:16px`">No enrollments found</p>" }
            $bodyHtml = "<div class=`"section-title`">Enrollment Details</div><div class=`"card`"><h2>Enrollments ($enCount)</h2>$tableHtml</div>"
            $scriptBase2 = if ($PSScriptRoot) { $PSScriptRoot } elseif ($PSCommandPath) { Split-Path -Parent $PSCommandPath } elseif ($MyInvocation.MyCommand.Path) { Split-Path -Parent $MyInvocation.MyCommand.Path } else { (Get-Location).Path }
            $htmlPath = Join-Path $scriptBase2 "Reports\EnrollmentStatus_$(Get-Date -Format yyyyMMdd_HHmmss).html"
            Export-ProfessionalHtmlReport -OutputPath $htmlPath -Title 'Enrollment Status' -Subtitle "Intune enrollment report" -Kpis $kpis -Body $bodyHtml -Charts $stateChart -ReportName 'Enrollment Status' -Version '2.1.0'
            Write-Log -Message "HTML report: $htmlPath" -Level 'SUCCESS'
            Write-Host "  HTML: $htmlPath" -ForegroundColor Green
        } catch { Write-Log -Message "HTML export failed: $($_.Exception.Message)" -Level 'WARNING' }
    } catch {
        Write-Log -Message "Could not query enrollments: $($_.Exception.Message)" -Level 'WARNING'
    }
