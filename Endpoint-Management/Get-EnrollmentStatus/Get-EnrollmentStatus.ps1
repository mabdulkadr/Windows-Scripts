...
try {
    ...
    # Tailored display: show actual enrollment data
    try {
        $enrollments = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Enrollments\*' -ErrorAction SilentlyContinue | Select-Object PSChildName, EnrollmentState, ProviderId, DeviceId -First 10
        if ($enrollments) {
