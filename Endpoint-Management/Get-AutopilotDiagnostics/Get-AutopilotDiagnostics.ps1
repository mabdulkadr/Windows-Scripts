<#
.TITLE
    Get-AutopilotDiagnostics - Diagnose Autopilot readiness

.SYNOPSIS
    Diagnose Autopilot readiness

.DESCRIPTION
    Checks Autopilot prerequisites: TPM, Autopilot profile registry, enrollment status, and hardware hash availability. Reports ready vs not ready for OOBE.

.TAGS
    Endpoint,Autopilot,Diagnostics
...
.EXAMPLE
    .\Get-AutopilotDiagnostics.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Endpoint,Autopilot,Diagnostics
...
$SolutionName = 'Get-AutopilotDiagnostics'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Check Autopilot')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $tpm = try { Get-Tpm -ErrorAction SilentlyContinue } catch { $null }
        $profile = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Provisioning\Diagnostics\AutoPilot' -ErrorAction SilentlyContinue
        if ($null -eq $tpm -and $null -eq $profile) {
            Write-Log -Message "No Autopilot diagnostics data found" -Level 'INFO'
        }
...
try {
    ...
    # Tailored display: Autopilot
    try {
        $tpm = try { Get-Tpm -ErrorAction SilentlyContinue } catch { $null }
        $profile = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Provisioning\Diagnostics\AutoPilot' -ErrorAction SilentlyContinue
        $enr = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Enrollments\*' -ErrorAction SilentlyContinue | Select-Object -First 1
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Autopilot Diagnostics --" -ForegroundColor DarkGray
        [PSCustomObject]@{
            TPM_Present= if ($tpm) { $tpm.TpmPresent } else { 'unknown' }
            TPM_Ready= if ($tpm) { $tpm.TpmReady } else { 'unknown' }
            HasProfile= ($null -ne $profile)
            EnrollmentFound= ($null -ne $enr)
        } | Format-List | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
        if ($profile) { $profile | Format-List | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Gray } }
    } catch {
        Write-Log -Message "Could not query Autopilot: $($_.Exception.Message)" -Level 'WARNING'
    }
