<#
.TITLE
    Test-EndpointConnectivity - Test connectivity to common Microsoft endpoints

.SYNOPSIS
    Test connectivity to common Microsoft endpoints

.DESCRIPTION
    Opens TCP sockets to login.microsoftonline.com, manage.microsoft.com, graph.microsoft.com, and portal.azure.com and reports status per host.

.TAGS
    Endpoint,Network,M365
...
.EXAMPLE
    .\Test-EndpointConnectivity.ps1
    Runs with default targets.

.EXAMPLE
    .\Test-EndpointConnectivity.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Endpoint,Network,M365
...
$SolutionName = 'Test-EndpointConnectivity'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Test TCP')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $tcp = New-Object Net.Sockets.TcpClient; $tcp.Connect('login.microsoftonline.com',443); $tcp.Close()
...
try {
    ...
    # Tailored display: endpoint ping results
    try {
        $endpoints = @('manage.microsoft.com','graph.microsoft.com','login.microsoftonline.com','enterpriseregistration.windows.net')
        $res = foreach ($ep in $endpoints) {
            $ok = Test-Connection -ComputerName $ep -Count 1 -Quiet -ErrorAction SilentlyContinue
            [PSCustomObject]@{ Endpoint=$ep; Reachable=$ok }
        }
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Endpoint Connectivity --" -ForegroundColor DarkGray
        $res | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
    } catch {
        Write-Log -Message "Could not test endpoints: $($_.Exception.Message)" -Level 'WARNING'
    }
