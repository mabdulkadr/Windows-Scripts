...
.DESCRIPTION
    Restarts IntuneManagementExtension to recover IME.

.TAGS
    Endpoint,Intune,Service
...
.EXAMPLE
    .\Restart-IntuneService.ps1
    Runs with default targets.

.EXAMPLE
    .\Restart-IntuneService.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Endpoint,Intune,Service
...
$SolutionName = 'Restart-IntuneService'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Restart IME')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        Restart-Service -Name IntuneManagementExtension -Force -ErrorAction Stop | Out-Null
...
try {
    ...
    # Tailored display: comprehensive IME restart diagnostics
    try {
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- IntuneManagementExtension Service (after restart) --" -ForegroundColor DarkGray
        $svc = Get-Service -Name IntuneManagementExtension -ErrorAction SilentlyContinue
        if ($svc) {
            $wmi = Get-CimInstance -ClassName Win32_Service -Filter "Name='IntuneManagementExtension'" -ErrorAction SilentlyContinue
            [PSCustomObject]@{
                Name      = $svc.Name
                Status    = $svc.Status
            ...
            if (Test-Path $imeLogs) {
                ...
                Get-ChildItem -Path $imeLogs -Filter "*.log" -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object Name, Length, LastWriteTime -First 3 | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Gray }
            }
        } else {
            Write-Log -Message "IntuneManagementExtension not found - not enrolled or extension not installed" -Level 'WARNING'
            Write-Host "  Action: Enroll device in Intune or trigger IME install via Company Portal" -ForegroundColor Yellow
        }

        Write-Host "  -- Restart Summary --" -ForegroundColor DarkGray
        $resultText = if ($svc -and $svc.Status -eq 'Running') { "Restart succeeded - service is Running" } elseif ($svc) { "Service status: $($svc.Status) - check IME logs" } else { "Service not found" }
        $color = if ($svc -and $svc.Status -eq 'Running') { "Green" } else { "Yellow" }
        Write-Host "  $resultText" -ForegroundColor $color
        Write-Host "  Next: Check Intune portal -> Devices -> Monitor -> Device actions -> Sync" -ForegroundColor Gray

    } catch {
        Write-Log -Message "Could not query IME service: $($_.Exception.Message)" -Level 'WARNING'
    }
