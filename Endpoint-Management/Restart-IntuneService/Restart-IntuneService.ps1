...
try {
    ...
    # Tailored display: IME service status
    try {
        $svc = Get-Service -Name IntuneManagementExtension -ErrorAction SilentlyContinue
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- IntuneManagementExtension Service --" -ForegroundColor DarkGray
        if ($svc) { $svc | Select-Object Name, Status, StartType | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
        else { Write-Log -Message "IntuneManagementExtension not found" -Level 'WARNING' }
    } catch {
