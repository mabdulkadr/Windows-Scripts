...
try {
    ...
    # Tailored display: Win32 apps from Intune
    try {
        $apps = Get-ChildItem -Path 'HKLM:\SOFTWARE\Microsoft\IntuneManagementExtension\Win32Apps' -ErrorAction SilentlyContinue | Select-Object -First 10
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Win32 Apps (IME) --" -ForegroundColor DarkGray
        if ($apps) { $apps | Select-Object PSChildName, PSPath | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
        else { Write-Log -Message "No Win32Apps keys under IME" -Level 'WARNING' }
    } catch {
