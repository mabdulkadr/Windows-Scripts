<#
.TITLE
    Install-ConfigMgrClient - Install the ConfigMgr client from a management point

.SYNOPSIS
    Install the ConfigMgr client from a management point

.DESCRIPTION
    Detects CcmExec; if missing, triggers ccmsetup from the assigned MP and verifies the post-install service state.

.TAGS
    Endpoint,ConfigMgr,Deployment
...
.EXAMPLE
    .\Install-ConfigMgrClient.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Endpoint,ConfigMgr,Deployment
...
$SolutionName = 'Install-ConfigMgrClient'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Check CcmExec')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $svc = Get-Service -Name CcmExec -ErrorAction SilentlyContinue; if (-not $svc) { throw 'CcmExec not found - install required' }
...
try {
    ...
    # Tailored display: CcmExec service + version
    try {
        $svc = Get-Service -Name CcmExec -ErrorAction SilentlyContinue
        $ver = (Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\SMS\Mobile Client' -ErrorAction SilentlyContinue).ClientVersion
        $site = (Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\SMS\Mobile Client' -ErrorAction SilentlyContinue).AssignedSiteCode
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- ConfigMgr Client Details --" -ForegroundColor DarkGray
        if ($svc) {
            [PSCustomObject]@{ Service=$svc.Name; Status=$svc.Status; Version=(if ($ver) { $ver } else { 'unknown' }); SiteCode=(if ($site) { $site } else { 'unknown' }) } | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
        } else {
            Write-Log -Message "CcmExec service not found - client not installed" -Level 'WARNING'
        }
    } catch {
        Write-Log -Message "Could not query ConfigMgr client: $($_.Exception.Message)" -Level 'WARNING'
    }
