<#
.TITLE
    Invoke-IntuneReenrollment - Re-enroll the device into Intune

.SYNOPSIS
    Re-enroll the device into Intune

.DESCRIPTION
    Clears the Autopilot cache and forces a re-enrollment at the next ESP run.
...
.EXAMPLE
    .\Invoke-IntuneReenrollment.ps1
    Runs with default targets.

.EXAMPLE
    .\Invoke-IntuneReenrollment.ps1 -TargetName "Server01","Server02"
...
$SolutionName = 'Invoke-IntuneReenrollment'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Check enrollment')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $null = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Provisioning\OMADM\Accounts\*' -ErrorAction SilentlyContinue | Select-Object -First 1
...
try {
    ...
    # Tailored display: enrollment + scheduled tasks
    try {
        $enr = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Enrollments\*' -ErrorAction SilentlyContinue | Select-Object PSChildName, EnrollmentState -First 5
        $task = Get-ScheduledTask -TaskName "*PushLaunch*" -ErrorAction SilentlyContinue | Select-Object TaskName, State -First 5
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Intune Re-enrollment Context --" -ForegroundColor DarkGray
        if ($enr) { $enr | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
        else { Write-Log -Message "No enrollment keys found" -Level 'WARNING' }
        if ($task) { $task | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Gray } }
    } catch {
        Write-Log -Message "Could not query re-enrollment context: $($_.Exception.Message)" -Level 'WARNING'
    }
    # Export professional HTML report
    try {
        $reenrEnrs = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Enrollments\*' -ErrorAction SilentlyContinue | Select-Object PSChildName, EnrollmentState -First 10
        $reenrCount = @($reenrEnrs).Count
        $reenrKpis = @(@{value=$reenrCount; label='Enrollments'; color='#0f62fe'}, @{value=$ok; label='Checks OK'; color='#24a148'}, @{value=$failed; label='Failed'; color='#da1e28'}, @{value=$env:COMPUTERNAME; label='Device'; color='#8a3ffc'})
        $reenrRows = if ($reenrEnrs) { $h=($reenrEnrs[0].PSObject.Properties | ForEach-Object { "<th>$($_.Name)</th>" }) -join ""; $b=($reenrEnrs | ForEach-Object { $cs=($_.PSObject.Properties | ForEach-Object { $v=[System.Net.WebUtility]::HtmlEncode("$($_.Value)"); if($_.Name -eq 'EnrollmentState'){ $bg=if($_.Value -eq '1'){'success'}else{'neutral'}; "<td><span class=`"badge $bg`">$v</span></td>" } else { "<td>$v</td>" } }) -join ""; "<tr>$cs</tr>" }) -join "`n"; "<table><thead><tr>$h</tr></thead><tbody>$b</tbody></table>" } else { "<p style=`"color:#8d8d8d;padding:16px`">No enrollments</p>" }
        $reenrBody = "<div class=`"section-title`">Intune Re-enrollment Status</div><div class=`"card`"><h2>Enrollment State ($reenrCount)</h2>$reenrRows</div>"
        $reenrBase = if ($PSScriptRoot) { $PSScriptRoot } elseif ($PSCommandPath) { Split-Path -Parent $PSCommandPath } else { (Get-Location).Path }
        $reenrHtml = Join-Path $reenrBase "Reports\IntuneReenrollment_$(Get-Date -Format yyyyMMdd_HHmmss).html"
        Export-ProfessionalHtmlReport -OutputPath $reenrHtml -Title 'Intune Re-enrollment' -Subtitle "Intune re-enrollment readiness report" -Kpis $reenrKpis -Body $reenrBody -ReportName 'Intune Re-enrollment' -Version '2.1.0'
        Write-Log -Message "HTML report: $reenrHtml" -Level 'SUCCESS'
