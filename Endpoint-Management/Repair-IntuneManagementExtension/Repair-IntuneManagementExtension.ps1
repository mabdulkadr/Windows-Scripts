...
try {
    ...
    # Tailored display: IME service + folder
    try {
        ...
        Write-Host "  -- IME Health --" -ForegroundColor DarkGray
        [PSCustomObject]@{ Service=(if ($svc.Status) { $svc.Status } else { 'NotFound' }); PathExists=$exists; Path=$path } | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
    } catch {
