...
try {
    ...
    # Tailored display: client actions via WMI
    try {
        $actions = Get-WmiObject -Namespace root\ccm -Class SMS_Client -ErrorAction SilentlyContinue | Select-Object -First 1
        $inv = Get-CimInstance -Namespace root\ccm\invagt -ClassName InventoryActionStatus -ErrorAction SilentlyContinue | Select-Object InventoryActionID, LastCycleStartedDate -First 5
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- ConfigMgr Client Actions --" -ForegroundColor DarkGray
        if ($inv) { $inv | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
        else { Write-Log -Message "No InventoryActionStatus found (client may be idle)" -Level 'INFO' }
    } catch {
