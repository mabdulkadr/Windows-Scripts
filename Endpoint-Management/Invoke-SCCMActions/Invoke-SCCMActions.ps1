<#
.TITLE
    Invoke-SCCMActions - Trigger a ConfigMgr client action

.SYNOPSIS
    Trigger a ConfigMgr client action

.DESCRIPTION
    Invokes the requested WMI client action (e.g. Machine Policy, App Deployment) on the local ConfigMgr client.
...
.EXAMPLE
    .\Invoke-SCCMActions.ps1
    Runs with default targets.

.EXAMPLE
    .\Invoke-SCCMActions.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Endpoint,ConfigMgr
...
$SolutionName = 'Invoke-SCCMActions'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Trigger SCCM')) {
...
try {
    ...
    # Tailored display: comprehensive ConfigMgr actions & health
    try {
        Write-Host "" -ForegroundColor Gray
        Write-Host "  ── ConfigMgr Client Health ──" -ForegroundColor DarkGray
        $svc = Get-Service -Name CcmExec -ErrorAction SilentlyContinue
        $regVer = (Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\SMS\Mobile Client' -ErrorAction SilentlyContinue).ClientVersion
        [PSCustomObject]@{
            Service = if ($svc) { $svc.Status } else { "NotFound" }
            Version = if ($regVer) { $regVer } else { "unknown" }
            SiteCode= (Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\SMS\Mobile Client' -ErrorAction SilentlyContinue).AssignedSiteCode
        } | Format-List | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }

        Write-Host "  ── Client Actions (GUID → Friendly Name) ──" -ForegroundColor DarkGray
        $actionMap = @{
            '{00000000-0000-0000-0000-000000000021}'='Machine Policy Retrieval & Evaluation'
            '{00000000-0000-0000-0000-000000000022}'='Discovery Data Collection'
            '{00000000-0000-0000-0000-000000000027}'='Hardware Inventory'
            '{00000000-0000-0000-0000-000000000031}'='Software Inventory'
            '{00000000-0000-0000-0000-000000000032}'='Update Deployment (Scan)'
            '{00000000-0000-0000-0000-000000000108}'='Configuration Baseline Evaluation'
            '{00000000-0000-0000-0000-000000000121}'='Application Deployment Evaluation'
        }
        $inv = Get-CimInstance -Namespace root\ccm\invagt -ClassName InventoryActionStatus -ErrorAction SilentlyContinue | Select-Object InventoryActionID, LastCycleStartedDate, LastMajorReportVersion -First 7
        if ($inv) {
            $rows = $inv | ForEach-Object {
                [PSCustomObject]@{
                    Action = $actionMap[$_.InventoryActionID] ?? $_.InventoryActionID
                    LastRun = $_.LastCycleStartedDate
                    Version = $_.LastMajorReportVersion
                }
            }
            $rows | Format-Table -AutoSize -Wrap | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
        } else {
            Write-Host "  No InventoryActionStatus (client idle or not installed)" -ForegroundColor Yellow
            Write-Host "  Available actions that TriggerSchedule can invoke:" -ForegroundColor Gray
            $actionMap.GetEnumerator() | ForEach-Object { Write-Host ("  {0} → {1}" -f $_.Key, $_.Value) -ForegroundColor Gray }
        }

        Write-Host "  ── Last Policy & CCM Logs ──" -ForegroundColor DarkGray
        try {
