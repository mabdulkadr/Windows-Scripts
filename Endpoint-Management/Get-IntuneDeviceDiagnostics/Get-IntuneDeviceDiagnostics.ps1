<#
.TITLE
    Get-IntuneDeviceDiagnostics - Collect Intune device diagnostics (local)

.SYNOPSIS
    Collect Intune device diagnostics (local)

.DESCRIPTION
    Collects local Intune diagnostics: enrollment, IME logs, Win32Apps, last sync, and exports CSV + HTML Carbon to Reports. No Graph required.

.TAGS
    Endpoint,Intune,Diagnostics
...
.EXAMPLE
    .\Get-IntuneDeviceDiagnostics.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Endpoint,Intune,Diagnostics
...
$SolutionName = 'Get-IntuneDeviceDiagnostics'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Collect Diagnostics')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $enr = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Enrollments\*' -ErrorAction SilentlyContinue
        if (-not $enr) { Write-Log -Message "No enrollments found - still collecting other diagnostics" -Level 'INFO' }
...
try {
    ...
    # Tailored display: diagnostics
    try {
        $enrCount = (Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Enrollments\*' -ErrorAction SilentlyContinue | Measure-Object).Count
        $ime = Get-Service -Name IntuneManagementExtension -ErrorAction SilentlyContinue
        $apps = (Get-ChildItem -Path 'HKLM:\SOFTWARE\Microsoft\IntuneManagementExtension\Win32Apps' -ErrorAction SilentlyContinue | Measure-Object).Count
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Intune Device Diagnostics --" -ForegroundColor DarkGray
        [PSCustomObject]@{
            Enrollments=$enrCount
            IME_Service= if ($ime) { $ime.Status } else { 'NotFound' }
            Win32Apps=$apps
