<#
.TITLE
    Get-HardwareInventorySingle - Single-device hardware inventory

.SYNOPSIS
    Single-device hardware inventory

.DESCRIPTION
    CIM-gathers hardware info for the supplied device and writes Reports/HardwareInventorySingle_<ts>.csv.
...
.EXAMPLE
    .\Get-HardwareInventorySingle.ps1
    Runs with default targets.

.EXAMPLE
    .\Get-HardwareInventorySingle.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Reporting,Hardware
...
$SolutionName = 'Get-HardwareInventorySingle'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Single HW')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $null = Get-CimInstance Win32_ComputerSystem -ErrorAction Stop | Select-Object -First 1
...
try {
    ...
    # Tailored display: single device hardware
    try {
        ...
        Write-Host "  -- Single Device Inventory --" -ForegroundColor DarkGray
        [PSCustomObject]@{ Computer=$cs.Name; Manufacturer=$cs.Manufacturer; BIOS=$bios.SMBIOSBIOSVersion; DiskFreeGB=[math]::Round($disk.FreeSpace/1GB,1) } | Format-List | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
    } catch {
        Write-Log -Message "Could not query single hardware: $($_.Exception.Message)" -Level 'WARNING'
    }
