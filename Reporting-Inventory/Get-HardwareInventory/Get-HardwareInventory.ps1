<#
.TITLE
    Get-HardwareInventory - Bulk hardware inventory for a list of devices

.SYNOPSIS
    Bulk hardware inventory for a list of devices

.DESCRIPTION
    CIM-gathers CPU, memory, OS, and user from each device in the target list and writes one CSV.
...
.EXAMPLE
    .\Get-HardwareInventory.ps1 -TargetName "Server01","Server02"
...
$SolutionName = 'Get-HardwareInventory'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'HW inventory')) {
...
try {
    ...
    # Tailored display: remote hardware (local sample)
    try {
        $cs = Get-CimInstance -ClassName Win32_ComputerSystem -ErrorAction SilentlyContinue
        $cpu = Get-CimInstance -ClassName Win32_Processor -ErrorAction SilentlyContinue | Select-Object -First 1
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Hardware (local sample) --" -ForegroundColor DarkGray
        [PSCustomObject]@{ Model=$cs.Model; RAM_GB=[math]::Round($cs.TotalPhysicalMemory/1GB,1); CPU=$cpu.Name; Cores=$cpu.NumberOfCores } | Format-List | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
    } catch {
        Write-Log -Message "Could not query hardware: $($_.Exception.Message)" -Level 'WARNING'
    }
