<#
.TITLE
    Get-BatteryHealth - Report laptop battery health

.SYNOPSIS
    Report laptop battery health

.DESCRIPTION
    Queries Win32_Battery to report DesignCapacity vs FullChargeCapacity, health %, and estimated runtime. Skips desktops without battery.
...
.EXAMPLE
    .\Get-BatteryHealth.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Reporting,Battery,Health
...
$SolutionName = 'Get-BatteryHealth'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Check Battery')) {
        ...
        if (-not $bat) {
            Write-Log -Message "No battery found (desktop or VM)" -Level 'INFO'
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
...
try {
    ...
    # Tailored display: battery
    try {
        $bat = Get-CimInstance -ClassName Win32_Battery -ErrorAction SilentlyContinue
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Battery Health --" -ForegroundColor DarkGray
        if ($bat) {
            $b = $bat | Select-Object -First 1
            $design = $b.DesignCapacity; $full = $b.FullChargeCapacity
            $health = if ($design -and $full) { [math]::Round(($full/$design)*100,1) } else { "unknown" }
            [PSCustomObject]@{ Name=$b.Name; Status=$b.Status; HealthPercent="$health%"; Design_mWh=$design; Full_mWh=$full } | Format-List | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
        } else { Write-Host "  No battery (desktop/VM) - skipped" -ForegroundColor Green }
    } catch {
        Write-Log -Message "Could not query battery: $($_.Exception.Message)" -Level 'WARNING'
    }
    # Export professional HTML report
    try {
        $batt = Get-CimInstance -ClassName Win32_Battery -ErrorAction SilentlyContinue | Select-Object Name, BatteryStatus, EstimatedChargeRemaining -First 5
        $battCount = @($batt).Count
        $avgCharge = if($batt){ [math]::Round(($batt | Measure-Object EstimatedChargeRemaining -Average).Average,0) } else { 0 }
        $battKpis = @(@{value="$avgCharge%"; label='Avg Charge'; color='#0f62fe'}, @{value=$battCount; label='Batteries'; color='#24a148'}, @{value=$ok; label='Checks OK'; color='#8a3ffc'}, @{value=$env:COMPUTERNAME; label='Device'; color='#c6c6c6'})
        $battRows = if($batt){ $h=($batt[0].PSObject.Properties | ForEach-Object { "<th>$($_.Name)</th>" }) -join ""; $b=($batt | ForEach-Object { $cs=($_.PSObject.Properties | ForEach-Object { $v=[System.Net.WebUtility]::HtmlEncode("$($_.Value)"); if($_.Name -eq 'EstimatedChargeRemaining'){ $pct=[int]$_.Value; $bg=if($pct -gt 80){'success'}elseif($pct -gt 40){'warning'}else{'error'}; "<td><span class=`"badge $bg`">$v%</span></td>" } else { "<td>$v</td>" } }) -join ""; "<tr>$cs</tr>" }) -join "`n"; "<table><thead><tr>$h</tr></thead><tbody>$b</tbody></table>" } else { "<p style=`"color:#8d8d8d;padding:16px`">No battery detected (desktop/VM)</p>" }
        $battBody = "<div class=`"section-title`">Battery Health</div><div class=`"card`"><h2>Battery Status</h2>$battRows</div>"
        $battBase = if ($PSScriptRoot) { $PSScriptRoot } elseif ($PSCommandPath) { Split-Path -Parent $PSCommandPath } else { (Get-Location).Path }
        $battHtml = Join-Path $battBase "Reports\BatteryHealth_$(Get-Date -Format yyyyMMdd_HHmmss).html"
        Export-ProfessionalHtmlReport -OutputPath $battHtml -Title 'Battery Health' -Subtitle "Battery health report" -Kpis $battKpis -Body $battBody -ReportName 'Battery Health' -Version '2.1.0'
        Write-Log -Message "HTML report: $battHtml" -Level 'SUCCESS'
