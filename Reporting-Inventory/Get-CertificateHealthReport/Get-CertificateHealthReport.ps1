<#
.TITLE
    Get-CertificateHealthReport - Report certificates nearing expiry

.SYNOPSIS
    Report certificates nearing expiry

.DESCRIPTION
    Scans LocalMachine stores and writes certificates expiring within 60 days to Reports/CertHealth_<ts>.csv.

.TAGS
    Reporting,Certificate,Security
...
.EXAMPLE
    .\Get-CertificateHealthReport.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Reporting,Certificate,Security
...
$SolutionName = 'Get-CertificateHealthReport'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Cert health')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $null = Get-ChildItem Cert:\LocalMachine\My -ErrorAction SilentlyContinue | Select-Object -First 1
...
try {
    ...
    # Tailored display: expiring certs
    try {
        $now = Get-Date; $soon = $now.AddDays(30)
        $certs = Get-ChildItem -Path Cert:\LocalMachine\My -ErrorAction SilentlyContinue | Where-Object { $_.NotAfter -lt $soon } | Select-Object Subject, NotAfter, Thumbprint -First 10
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Certs Expiring within 30 days --" -ForegroundColor DarkGray
        if ($certs) { $certs | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Yellow } }
        else { Write-Host "  No certs expiring within 30 days" -ForegroundColor Green }
    } catch {
        Write-Log -Message "Could not check cert health: $($_.Exception.Message)" -Level 'WARNING'
    }
    # Export professional HTML report
    try {
        $certs = Get-ChildItem -Path Cert:\LocalMachine\My -ErrorAction SilentlyContinue | Select-Object Subject, NotAfter, Thumbprint -First 20
        $certTotal = @($certs).Count; $certExp = @($certs | Where-Object { $_.NotAfter -lt (Get-Date).AddDays(30) }).Count; $certHealthy = $certTotal - $certExp
        $certKpis = @(@{value=$certTotal; label='Certificates'; color='#0f62fe'}, @{value=$certHealthy; label='Healthy'; color='#24a148'}, @{value=$certExp; label='Expiring <30d'; color='#da1e28'}, @{value=$env:COMPUTERNAME; label='Device'; color='#8a3ffc'})
        $certRows = if($certs){ $h="<th>Subject</th><th>Expiry</th><th>Status</th>"; $b=($certs | Select-Object -First 30 | ForEach-Object { $days=[math]::Round(($_.NotAfter - (Get-Date)).TotalDays,0); $bg=if($days -lt 0){'error'}elseif($days -lt 30){'warning'}else{'success'}; $st=if($days -lt 0){'Expired'}elseif($days -lt 30){"$days days"}else{"$days days"}; "<tr><td>$([System.Net.WebUtility]::HtmlEncode($_.Subject))</td><td>$($_.NotAfter.ToString('yyyy-MM-dd'))</td><td><span class=`"badge $bg`">$st</span></td></tr>" }) -join "`n"; "<table><thead><tr>$h</tr></thead><tbody>$b</tbody></table>" } else { "<p style=`"color:#8d8d8d;padding:16px`">No certificates found</p>" }
        $certCharts = @{donut=@(@{label='Healthy';value=$certHealthy;color='#24a148'},@{label='Expiring <30d';value=$certExp;color='#da1e28'})} | ConvertTo-Json -Compress
        $certBody = "<div class=`"section-title`">Certificate Health</div><div class=`"card`"><h2>Certificate Status</h2>$certRows</div>"
        $certBase = if ($PSScriptRoot) { $PSScriptRoot } elseif ($PSCommandPath) { Split-Path -Parent $PSCommandPath } else { (Get-Location).Path }
        $certHtml = Join-Path $certBase "Reports\CertHealth_$(Get-Date -Format yyyyMMdd_HHmmss).html"
        Export-ProfessionalHtmlReport -OutputPath $certHtml -Title 'Certificate Health' -Subtitle "Certificate expiry and validity report" -Kpis $certKpis -Body $certBody -Charts $certCharts -ReportName 'Certificate Health' -Version '2.1.0'
        Write-Log -Message "HTML report: $certHtml" -Level 'SUCCESS'
