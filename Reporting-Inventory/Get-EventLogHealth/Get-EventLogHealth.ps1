<#
.TITLE
    Get-EventLogHealth - Report System and Application log health for the last 7 days

.SYNOPSIS
    Report System and Application log health for the last 7 days

.DESCRIPTION
    Counts error/warning events with Get-WinEvent over the last 7 days.
...
.EXAMPLE
    .\Get-EventLogHealth.ps1
    Runs with default targets.

.EXAMPLE
    .\Get-EventLogHealth.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Reporting,EventLog
...
$SolutionName = 'Get-EventLogHealth'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Event log')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $cutoff = (Get-Date).AddDays(-7); $null = Get-WinEvent -FilterHashtable @{LogName='System'; StartTime=$cutoff} -MaxEvents 1 -ErrorAction SilentlyContinue
...
try {
    ...
    # Tailored display: event logs
    try {
        $logs = @('System','Application')
        $rows = foreach ($lg in $logs) {
            $log = Get-WinEvent -ListLog $lg -ErrorAction SilentlyContinue
            $errs = (Get-WinEvent -FilterHashtable @{LogName=$lg; Level=2} -MaxEvents 100 -ErrorAction SilentlyContinue | Measure-Object).Count
            [PSCustomObject]@{ Log=$lg; RecordCount=$log.RecordCount; ErrorsLast100=$errs }
        }
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Event Log Health (last 7 days errors) --" -ForegroundColor DarkGray
        $rows | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
    } catch {
        Write-Log -Message "Could not check event logs: $($_.Exception.Message)" -Level 'WARNING'
    }
