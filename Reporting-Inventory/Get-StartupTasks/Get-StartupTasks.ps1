<#
.TITLE
    Get-StartupTasks - Enumerate startup items from Run / scheduled tasks / startup folder

.SYNOPSIS
    Enumerate startup items from Run / scheduled tasks / startup folder

.DESCRIPTION
    Combines Win32_StartupCommand and Scheduled Task data to a Reports/Startup_<ts>.csv.
...
.EXAMPLE
    .\Get-StartupTasks.ps1
    Runs with default targets.

.EXAMPLE
    .\Get-StartupTasks.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Reporting,Startup
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Startup tasks')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $null = Get-CimInstance Win32_StartupCommand -ErrorAction SilentlyContinue | Select-Object -First 1
...
try {
    ...
    # Tailored display: startup items
    try {
        $run = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run' -ErrorAction SilentlyContinue
        $tasks = Get-ScheduledTask -ErrorAction SilentlyContinue | Where-Object { $_.TaskName -notlike "*Optimize*" } | Select-Object TaskName, State -First 10
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Startup Run Keys --" -ForegroundColor DarkGray
        if ($run) { $run.PSObject.Properties | Where-Object { $_.Name -notlike "PS*" } | Select-Object Name, Value -First 5 | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
        Write-Host "  -- Scheduled Tasks (sample) --" -ForegroundColor DarkGray
        if ($tasks) { $tasks | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Gray } }
    } catch {
        Write-Log -Message "Could not enumerate startup: $($_.Exception.Message)" -Level 'WARNING'
    }
    # Export professional HTML report
    try {
        $tasks = Get-ScheduledTask -ErrorAction SilentlyContinue | Where-Object { $_.TaskName -notlike "*Microsoft*" } | Select-Object TaskName, TaskPath, State -First 20
        $taskCount = @($tasks).Count; $taskReady = @($tasks | Where-Object State -eq 'Ready').Count
        $taskKpis = @(@{value=$taskCount; label='Tasks Listed'; color='#0f62fe'}, @{value=$taskReady; label='Ready'; color='#24a148'}, @{value=($taskCount-$taskReady); label='Other'; color='#f1c21b'}, @{value=$env:COMPUTERNAME; label='Device'; color='#8a3ffc'})
        $taskRows = if($tasks){ $h="<th>Task</th><th>Path</th><th>State</th>"; $b=($tasks | ForEach-Object { $bg=if($_.State -eq 'Ready'){'success'}else{'neutral'}; "<tr><td>$([System.Net.WebUtility]::HtmlEncode($_.TaskName))</td><td>$([System.Net.WebUtility]::HtmlEncode($_.TaskPath))</td><td><span class=`"badge $bg`">$($_.State)</span></td></tr>" }) -join "`n"; "<table><thead><tr>$h</tr></thead><tbody>$b</tbody></table>" } else { "<p style=`"color:#8d8d8d;padding:16px`">No tasks found</p>" }
        $taskBody = "<div class=`"section-title`">Startup &amp; Scheduled Tasks</div><div class=`"card`"><h2>Scheduled Tasks ($taskCount)</h2>$taskRows</div>"
        $taskBase = if ($PSScriptRoot) { $PSScriptRoot } elseif ($PSCommandPath) { Split-Path -Parent $PSCommandPath } else { (Get-Location).Path }
        $taskHtml = Join-Path $taskBase "Reports\StartupTasks_$(Get-Date -Format yyyyMMdd_HHmmss).html"
        Export-ProfessionalHtmlReport -OutputPath $taskHtml -Title 'Startup Tasks' -Subtitle "Scheduled task inventory" -Kpis $taskKpis -Body $taskBody -ReportName 'Startup Tasks' -Version '2.1.0'
        Write-Log -Message "HTML report: $taskHtml" -Level 'SUCCESS'
