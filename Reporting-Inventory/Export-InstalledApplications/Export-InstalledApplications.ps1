<#
.TITLE
    Export-InstalledApplications - Export installed applications to CSV

.SYNOPSIS
    Export installed applications to CSV

.DESCRIPTION
    Reads HKLM Uninstall keys and writes the result to Reports/InstalledApps_<ts>.csv.

.TAGS
    Reporting,Inventory,Apps
...
.EXAMPLE
    .\Export-InstalledApplications.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Reporting,Inventory,Apps
...
$SolutionName = 'Export-InstalledApplications'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Export apps')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $null = Get-ItemProperty 'HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*' -ErrorAction SilentlyContinue | Select-Object -First 1
...
try {
    ...
    # Tailored display: exported CSV
    try {
        $scriptBase = if ($PSScriptRoot) { $PSScriptRoot } else { Split-Path -Parent $PSCommandPath }
        $reports = Join-Path $scriptBase "Reports"
        $csv = Get-ChildItem -Path $reports -Filter "*.csv" -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 3
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Installed Apps Export --" -ForegroundColor DarkGray
        Write-Host "  Reports: $reports" -ForegroundColor Gray
        if ($csv) { $csv | Select-Object Name, Length, LastWriteTime | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
        $count = (Get-ItemProperty 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*' -ErrorAction SilentlyContinue | Measure-Object).Count
        Write-Host "  Registry Uninstall entries: $count" -ForegroundColor Cyan
    } catch {
        Write-Log -Message "Could not check app export: $($_.Exception.Message)" -Level 'WARNING'
    }
    # Export professional HTML report
    try {
        $apps = Get-ItemProperty HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\* -ErrorAction SilentlyContinue | Select-Object DisplayName, DisplayVersion, Publisher -First 30 | Where-Object DisplayName
        $appCount = @($apps).Count
        $appKpis = @(@{value=$appCount; label='Installed Apps'; color='#0f62fe'}, @{value=$ok; label='Checks OK'; color='#24a148'}, @{value=$failed; label='Failed'; color='#da1e28'}, @{value=$env:COMPUTERNAME; label='Device'; color='#8a3ffc'})
        $appRows = if($apps){ $h=($apps[0].PSObject.Properties | ForEach-Object { "<th>$($_.Name)</th>" }) -join ""; $b=($apps | Select-Object -First 50 | ForEach-Object { $cs=($_.PSObject.Properties | ForEach-Object { "<td>$([System.Net.WebUtility]::HtmlEncode("$($_.Value)"))</td>" }) -join ""; "<tr>$cs</tr>" }) -join "`n"; "<table><thead><tr>$h</tr></thead><tbody>$b</tbody></table>" } else { "<p style=`"color:#8d8d8d;padding:16px`">No applications found</p>" }
        $pubCharts = if($apps){ $pubs=$apps | Group-Object Publisher | Sort-Object Count -Descending | Select-Object -First 6; $bars=@($pubs | ForEach-Object { @{label=if($_.Name){"$($_.Name)"}else{"Unknown"};value=$_.Count;color='#0f62fe'} }) | ConvertTo-Json -Compress; @{bars=$bars} | ConvertTo-Json -Compress } else { "" }
        $appBody = "<div class=`"section-title`">Installed Applications</div><div class=`"card`"><h2>Application Inventory ($appCount apps)</h2>$appRows</div>"
        $appBase = if ($PSScriptRoot) { $PSScriptRoot } elseif ($PSCommandPath) { Split-Path -Parent $PSCommandPath } else { (Get-Location).Path }
        $appHtml = Join-Path $appBase "Reports\InstalledApps_$(Get-Date -Format yyyyMMdd_HHmmss).html"
        if($pubCharts -and $pubCharts.Contains('bars')){ $ch2 = @{bars=(($apps | Group-Object Publisher | Sort-Object Count -Descending | Select-Object -First 6) | ForEach-Object { @{label=if($_.Name){"$($_.Name)"}else{"Unknown"};value=$_.Count;color='#0f62fe'} }) } | ConvertTo-Json -Compress; Export-ProfessionalHtmlReport -OutputPath $appHtml -Title 'Installed Applications' -Subtitle "Software inventory report" -Kpis $appKpis -Body $appBody -Charts $ch2 -ReportName 'Installed Applications' -Version '2.1.0' } else { Export-ProfessionalHtmlReport -OutputPath $appHtml -Title 'Installed Applications' -Subtitle "Software inventory report" -Kpis $appKpis -Body $appBody -ReportName 'Installed Applications' -Version '2.1.0' }
        Write-Log -Message "HTML report: $appHtml" -Level 'SUCCESS'
