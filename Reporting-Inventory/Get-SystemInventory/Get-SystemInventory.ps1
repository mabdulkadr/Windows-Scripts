...
try {
    ...
    # Tailored display: system inventory
    try {
        ...
        Write-Host "  -- System Inventory --" -ForegroundColor DarkGray
        [PSCustomObject]@{ OS=$os.Caption; Build=$os.BuildNumber; Model=$cs.Model; RAM_GB=[math]::Round($cs.TotalPhysicalMemory/1GB,1) } | Format-List | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
