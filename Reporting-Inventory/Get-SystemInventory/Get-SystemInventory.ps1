<#
.TITLE
    Get-SystemInventory - Collect full system inventory and write CSV

.SYNOPSIS
    Collect full system inventory and write CSV

.DESCRIPTION
    CIM-gathers hostname, manufacturer, CPU, RAM, OS into Reports/SystemInventory_<ts>.csv.
...
.EXAMPLE
    .\Get-SystemInventory.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Reporting,Inventory
...
$SolutionName = 'Get-SystemInventory'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Collect inventory')) {
        ...
        $net = Get-CimInstance -ClassName Win32_NetworkAdapterConfiguration -Filter 'IPEnabled=True' -ErrorAction SilentlyContinue | Select-Object -First 1
        $row = [PSCustomObject]@{
            ...
            GPU=$gpu.Name
            GPU_RAM_GB=[math]::Round($gpu.AdapterRAM/1GB,2)
            Domain=$cs.Domain
            PrimaryIPv4=$net.IPAddress | Where-Object { $_ -match '^\d+\.\d+\.\d+\.\d+$' } | Select-Object -First 1
        }
        ...
        $csvPath = Join-Path $reports "SystemInventory_$(Get-Date -Format yyyyMMdd_HHmmss).csv"
        $row | Export-Csv -Path $csvPath -NoTypeInformation -Encoding UTF8

        # Export professional HTML report (IBM Carbon Dark) with KPIs + charts
        try {
            $htmlPath = $csvPath -replace '\.csv$', '.html'
            $rows = Import-Csv -Path $csvPath -ErrorAction SilentlyContinue | Select-Object -First 100
            $tableHtml = if ($rows) {
                $header = ($rows[0].PSObject.Properties | ForEach-Object { "<th>$($_.Name)</th>" }) -join ""
                $body = ($rows | ForEach-Object {
                    $cells = ($_.PSObject.Properties | ForEach-Object { "<td>$([System.Net.WebUtility]::HtmlEncode($_.Value))</td>" }) -join ""
                    "<tr>$cells</tr>"
                }) -join "`n"
            ...
            $kpis = @(
                @{value=$ramGb; label='RAM (GB)'; color='#0f62fe'},
                @{value=$os.BuildNumber; label='OS Build'; color='#24a148'},
                @{value=if ($cs.Manufacturer) { $cs.Manufacturer } else { 'N/A' }; label='Manufacturer'; color='#8a3ffc'},
                @{value="$($cpu.NumberOfCores)c / $($cpu.NumberOfLogicalProcessors)t"; label='CPU'; color='#f1c21b'}
            )
            # Disk breakdown
            $diskRows = ''
            foreach ($d in $disk) {
                ...
                $diskBg = if ($usedPct -gt 90) { 'error' } elseif ($usedPct -gt 75) { 'warning' } else { 'success' }
                $diskRows += "<tr><td>$($d.DeviceID)</td><td>$($d.FileSystem)</td><td>$sizeGB GB</td><td>$freeGB GB</td><td><span class=`"badge $diskBg`">$usedPct%</span></td></tr>"
            }
            $diskTable = if ($diskRows) { "<table><thead><tr><th>Drive</th><th>FS</th><th>Size</th><th>Free</th><th>Used</th></tr></thead><tbody>$diskRows</tbody></table>" } else { '<p style="color:#8d8d8d">No disks reported</p>' }
            # Component summary (label/value pairs)
            $compRows = "<tr><td>Manufacturer</td><td>$([System.Net.WebUtility]::HtmlEncode($cs.Manufacturer))</td></tr><tr><td>Model</td><td>$([System.Net.WebUtility]::HtmlEncode($cs.Model))</td></tr><tr><td>Serial Number</td><td><code>$([System.Net.WebUtility]::HtmlEncode($cs.SerialNumber))</code></td></tr><tr><td>BIOS Version</td><td><code>$([System.Net.WebUtility]::HtmlEncode($bios.SMBIOSBIOSVersion))</code></td></tr><tr><td>BIOS Date</td><td>$($bios.ReleaseDate.ToString('yyyy-MM-dd'))</td></tr><tr><td>Domain / Workgroup</td><td>$([System.Net.WebUtility]::HtmlEncode($cs.Domain))</td></tr><tr><td>OS Architecture</td><td>$($os.OSArchitecture)</td></tr><tr><td>OS Install Date</td><td>$($os.InstallDate.ToString('yyyy-MM-dd HH:mm'))</td></tr><tr><td>Last Boot</td><td>$($os.LastBootUpTime.ToString('yyyy-MM-dd HH:mm'))</td></tr><tr><td>CPU</td><td>$([System.Net.WebUtility]::HtmlEncode($cpu.Name))</td></tr><tr><td>GPU</td><td>$([System.Net.WebUtility]::HtmlEncode($gpu.Name))</td></tr><tr><td>GPU RAM</td><td>$([math]::Round($gpu.AdapterRAM/1GB, 2)) GB</td></tr>"
            $compTable = "<table><thead><tr><th>Component</th><th>Detail</th></tr></thead><tbody>$compRows</tbody></table>"
            $bodyHtml = "<div class=`"section-title`">System Inventory</div><div class=`"card`"><h2>Device Inventory (CSV export)</h2><p style=`"color:#c6c6c6`">Per-target summary, also exported to <code>$([System.IO.Path]::GetFileName($csvPath))</code>.</p>$tableHtml</div><div class=`"grid-2`"><div class=`"card`"><h2>Component Detail</h2>$compTable</div><div class=`"card`"><h2>Logical Disks</h2>$diskTable</div></div>"
            Export-ProfessionalHtmlReport -OutputPath $htmlPath -Title $SolutionName -Subtitle "Full system inventory report" -Kpis $kpis -Body $bodyHtml -ReportName 'System Inventory' -Version '2.1.0'
            Write-Log -Message "HTML report: $htmlPath" -Level 'INFO'
        } catch {
            Write-Log -Message "HTML export failed: $($_.Exception.Message)" -Level 'WARNING'
        }
        Write-Log -Message "System inventory CSV: $csvPath" -Level 'INFO'
...
try {
    ...
    # Tailored display: comprehensive system inventory deep dive
    try {
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- System Inventory Deep Dive --" -ForegroundColor DarkGray
        $os = Get-CimInstance -ClassName Win32_OperatingSystem -ErrorAction SilentlyContinue
        $cs = Get-CimInstance -ClassName Win32_ComputerSystem -ErrorAction SilentlyContinue
        $cpu = Get-CimInstance -ClassName Win32_Processor -ErrorAction SilentlyContinue | Select-Object -First 1
        ...
        Write-Host "  [OS & CPU]" -ForegroundColor Cyan
        [PSCustomObject]@{
            OS            = $os.Caption
            Build         = $os.BuildNumber
            Version       = $os.Version
        ...
        Write-Host "  [Memory & Disk]" -ForegroundColor Cyan
        $mem = Get-CimInstance -ClassName Win32_PhysicalMemory -ErrorAction SilentlyContinue
        $disks = Get-CimInstance -ClassName Win32_LogicalDisk -Filter "DriveType=3" -ErrorAction SilentlyContinue
        [PSCustomObject]@{
            RAM_GB        = [math]::Round($cs.TotalPhysicalMemory/1GB,1)
            DIMMs         = ($mem | Measure-Object).Count
            PageFile_GB   = [math]::Round($os.SizeStoredInPagingFiles/1KB/1KB/1KB,1)
            Disks         = ($disks | ForEach-Object { "$($_.DeviceID) $([math]::Round($_.FreeSpace/1GB,1))/$([math]::Round($_.Size/1GB,1)) GB" }) -join " | "
        } | Format-List | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Gray }
        $disks | Select-Object DeviceID, @{N='SizeGB';E={[math]::Round($_.Size/1GB,1)}}, @{N='FreeGB';E={[math]::Round($_.FreeSpace/1GB,1)}}, FileSystem | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Gray }

        Write-Host "  [GPU & Network]" -ForegroundColor Cyan
        Write-Host ("  GPU: {0} ({1} MB)" -f $gpu.Name, [math]::Round($gpu.AdapterRAM/1MB)) -ForegroundColor Gray
        Get-NetAdapter -ErrorAction SilentlyContinue | Where-Object { $_.Status -eq 'Up' } | Select-Object Name, LinkSpeed, MacAddress -First 3 | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Gray }

        $scriptBase = if ($PSScriptRoot) { $PSScriptRoot } else { Split-Path -Parent $PSCommandPath }
        $reports = Join-Path $scriptBase "Reports"
        Write-Host "  Reports: $reports" -ForegroundColor Gray
        $csv = Get-ChildItem -Path $reports -Filter "SystemInventory*.csv" -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
        $html = Get-ChildItem -Path $reports -Filter "SystemInventory*.html" -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
        if ($csv) { Write-Host "  CSV: $($csv.FullName) ($($csv.Length) bytes)" -ForegroundColor Cyan }
        if ($html) { Write-Host "  HTML: $($html.FullName) ($($html.Length) bytes)" -ForegroundColor Green }

    } catch {
        Write-Log -Message "Could not collect inventory: $($_.Exception.Message)" -Level 'WARNING'
    }
