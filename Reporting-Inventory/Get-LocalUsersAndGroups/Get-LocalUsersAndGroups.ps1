<#
.TITLE
    Get-LocalUsersAndGroups - Enumerate local users and administrators group

.SYNOPSIS
    Enumerate local users and administrators group

.DESCRIPTION
    Combines Get-LocalUser and Get-LocalGroupMember into Reports/LocalUsersGroups_<ts>.csv.

.TAGS
    Reporting,LocalUsers
...
.EXAMPLE
    .\Get-LocalUsersAndGroups.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Reporting,LocalUsers
...
$SolutionName = 'Get-LocalUsersAndGroups'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Local users')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $null = Get-LocalUser -ErrorAction SilentlyContinue | Select-Object -First 1
...
try {
    ...
    # Tailored display: local users + admins
    try {
        $users = Get-LocalUser -ErrorAction SilentlyContinue | Select-Object Name, Enabled -First 10
        $admins = Get-LocalGroupMember -Group "Administrators" -ErrorAction SilentlyContinue | Select-Object Name, ObjectClass -First 10
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Local Users --" -ForegroundColor DarkGray
        if ($users) { $users | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
        Write-Host "  -- Administrators Group --" -ForegroundColor DarkGray
        if ($admins) { $admins | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Yellow } }
    } catch {
        Write-Log -Message "Could not enumerate users/groups: $($_.Exception.Message)" -Level 'WARNING'
    }
# Export professional HTML report
    try {
        $locUsrs = try { Get-LocalUser -ErrorAction SilentlyContinue | Select-Object Name, Enabled, Description, SID, PasswordRequired, PasswordLastSet -First 25 } catch { @() }
        $locGrp = try { Get-LocalGroup -ErrorAction SilentlyContinue | Select-Object Name, Description, SID -First 25 } catch { @() }
        $locAdmins = try { Get-LocalGroupMember -Group 'Administrators' -ErrorAction SilentlyContinue | Select-Object Name, ObjectClass, PrincipalSource -First 25 } catch { @() }
        $locUserCount = @($locUsrs).Count
        $locEnabled = @($locUsrs | Where-Object Enabled).Count
        $locDisabled = $locUserCount - $locEnabled
        $locGrpCount = @($locGrp).Count
        $locKpis = @(@{value=$locUserCount; label='Local Users'; color='#0f62fe'}, @{value="$locEnabled / $locDisabled"; label='Enabled / Disabled'; color='#24a148'}, @{value=$locGrpCount; label='Local Groups'; color='#8a3ffc'}, @{value=$env:COMPUTERNAME; label='Device'; color='#f1c21b'})
        # Users table (5 cols)
        $locUserRows = if($locUsrs){ $b=($locUsrs | ForEach-Object {
            $sid = if ($_.SID) { ($_.SID.ToString().Substring(0, [Math]::Min(20, $_.SID.ToString().Length))) + '...' } else { '' }
            $pwLastSet = if ($_.PasswordLastSet) { $_.PasswordLastSet.ToString('yyyy-MM-dd') } else { 'Never' }
            $desc = if ($_.Description) { [System.Net.WebUtility]::HtmlEncode($_.Description) } else { '' }
            $bg=if($_.Enabled){'success'}else{'neutral'}
            $pwReqBg = if ($_.PasswordRequired) { 'success' } else { 'warning' }
            "<tr><td>$([System.Net.WebUtility]::HtmlEncode($_.Name))</td><td><span class=`"badge $bg`">$(if($_.Enabled){'Enabled'}else{'Disabled'})</span></td><td>$desc</td><td><span class=`"badge $pwReqBg`">$(if($_.PasswordRequired){'Required'}else{'None'})</span></td><td>$pwLastSet</td><td><code>$sid</code></td></tr>"
        }) -join "`n"; "<table><thead><tr><th>User</th><th>Status</th><th>Description</th><th>Password</th><th>Last Set</th><th>SID</th></tr></thead><tbody>$b</tbody></table>" } else { "<p style=`"color:#8d8d8d;padding:16px`">No local users</p>" }
        # Groups table
        $locGrpRows = if($locGrp){ $b=($locGrp | ForEach-Object {
            $sid = if ($_.SID) { ($_.SID.ToString().Substring(0, [Math]::Min(20, $_.SID.ToString().Length))) + '...' } else { '' }
            $desc = if ($_.Description) { [System.Net.WebUtility]::HtmlEncode($_.Description) } else { '—' }
            $memberCount = try { @(Get-LocalGroupMember -Group $_.Name -ErrorAction SilentlyContinue).Count } catch { 0 }
            "<tr><td>$([System.Net.WebUtility]::HtmlEncode($_.Name))</td><td>$desc</td><td><code>$sid</code></td><td>$memberCount</td></tr>"
        }) -join "`n"; "<table><thead><tr><th>Group</th><th>Description</th><th>SID</th><th>Members</th></tr></thead><tbody>$b</tbody></table>" } else { "<p style=`"color:#8d8d8d;padding:16px`">No local groups</p>" }
        # Administrators table (high-privilege focus)
        $locAdminRows = if($locAdmins){ $b=($locAdmins | ForEach-Object {
            $src = if ($_.PrincipalSource) { $_.PrincipalSource.ToString() } else { 'Local' }
            $srcBg = switch ($src) { 'Local' { 'success' } 'ActiveDirectory' { 'info' } 'AzureAD' { 'info' } default { 'neutral' } }
            "<tr><td>$([System.Net.WebUtility]::HtmlEncode($_.Name))</td><td>$($_.ObjectClass)</td><td><span class=`"badge $srcBg`">$src</span></td></tr>"
        }) -join "`n"; "<table><thead><tr><th>Member</th><th>Object Class</th><th>Source</th></tr></thead><tbody>$b</tbody></table>" } else { "<p style=`"color:#8d8d8d;padding:16px`">No administrator accounts found</p>" }
        $locBody = "<div class=`"section-title`">Local Users &amp; Groups</div><div class=`"card`"><h2>Local User Accounts ($locUserCount)</h2><p style=`"color:#c6c6c6`">Local accounts on <code>$env:COMPUTERNAME</code>. Disabled accounts are excluded from interactive logon but remain on the system.</p>$locUserRows</div><div class=`"grid-2`"><div class=`"card`"><h2>Local Groups ($locGrpCount)</h2>$locGrpRows</div><div class=`"card`"><h2>Administrators ($(if($locAdmins){@($locAdmins).Count}else{0}) members)</h2><p style=`"color:#c6c6c6`">Members of the local Administrators group. <strong>Audit this regularly.</strong></p>$locAdminRows</div></div>"
        $locBase = if ($PSScriptRoot) { $PSScriptRoot } elseif ($PSCommandPath) { Split-Path -Parent $PSCommandPath } else { (Get-Location).Path }
        $locHtml = Join-Path $locBase "Reports\LocalUsers_$(Get-Date -Format yyyyMMdd_HHmmss).html"
        Export-ProfessionalHtmlReport -OutputPath $locHtml -Title 'Local Users & Groups' -Subtitle "Local account inventory" -Kpis $locKpis -Body $locBody -ReportName 'Local Users & Groups' -Version '2.1.0'
        Write-Log -Message "HTML report: $locHtml" -Level 'SUCCESS'
