<#
.TITLE
    Get-CertificateSummary - Summarize certificates in LocalMachine stores

.SYNOPSIS
    Summarize certificates in LocalMachine stores

.DESCRIPTION
    Enumerates My/Root/CA and writes Reports/CertificateSummary_<ts>.csv.
...
.EXAMPLE
    .\Get-CertificateSummary.ps1
    Runs with default targets.

.EXAMPLE
    .\Get-CertificateSummary.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Reporting,Certificate
...
$SolutionName = 'Get-CertificateSummary'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Cert summary')) {
...
try {
    ...
    # Tailored display: cert counts by store
    try {
        $stores = @('Cert:\LocalMachine\My','Cert:\LocalMachine\Root','Cert:\CurrentUser\My')
        $rows = foreach ($s in $stores) { $c = (Get-ChildItem -Path $s -ErrorAction SilentlyContinue | Measure-Object).Count; [PSCustomObject]@{ Store=$s; Count=$c } }
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Certificate Stores --" -ForegroundColor DarkGray
        $rows | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
    } catch {
        Write-Log -Message "Could not summarize certs: $($_.Exception.Message)" -Level 'WARNING'
    }
    # Export professional HTML report
    try {
        $certs2 = Get-ChildItem -Path Cert:\LocalMachine\My -ErrorAction SilentlyContinue | Select-Object Subject, NotAfter -First 20
        $cTotal2 = @($certs2).Count; $cExp2 = @($certs2 | Where-Object { $_.NotAfter -lt (Get-Date).AddDays(30) }).Count; $cHlth2 = $cTotal2 - $cExp2
        $cKpis2 = @(@{value=$cTotal2; label='Certificates'; color='#0f62fe'}, @{value=$cHlth2; label='Healthy'; color='#24a148'}, @{value=$cExp2; label='Expiring <30d'; color='#da1e28'}, @{value=$env:COMPUTERNAME; label='Device'; color='#8a3ffc'})
        $cRows2 = if($certs2){ $h="<th>Subject</th><th>Expiry</th><th>Days</th>"; $b=($certs2 | Select-Object -First 20 | ForEach-Object { $days=[math]::Round(($_.NotAfter - (Get-Date)).TotalDays,0); $bg=if($days -lt 0){'error'}elseif($days -lt 30){'warning'}else{'success'}; "<tr><td>$([System.Net.WebUtility]::HtmlEncode($_.Subject))</td><td>$($_.NotAfter.ToString('yyyy-MM-dd'))</td><td><span class=`"badge $bg`">$days days</span></td></tr>" }) -join "`n"; "<table><thead><tr>$h</tr></thead><tbody>$b</tbody></table>" } else { "<p style=`"color:#8d8d8d;padding:16px`">No certificates</p>" }
        $cCharts2 = @{donut=@(@{label='Healthy';value=$cHlth2;color='#24a148'},@{label='Expiring';value=$cExp2;color='#da1e28'})} | ConvertTo-Json -Compress
        $cBody2 = "<div class=`"section-title`">Certificate Summary</div><div class=`"card`"><h2>LocalMachine\My Store</h2>$cRows2</div>"
        $cBase2 = if ($PSScriptRoot) { $PSScriptRoot } elseif ($PSCommandPath) { Split-Path -Parent $PSCommandPath } else { (Get-Location).Path }
        $cHtml2 = Join-Path $cBase2 "Reports\CertSummary_$(Get-Date -Format yyyyMMdd_HHmmss).html"
        Export-ProfessionalHtmlReport -OutputPath $cHtml2 -Title 'Certificate Summary' -Subtitle "Certificate store overview" -Kpis $cKpis2 -Body $cBody2 -Charts $cCharts2 -ReportName 'Certificate Summary' -Version '2.1.0'
        Write-Log -Message "HTML report: $cHtml2" -Level 'SUCCESS'
