<#
.TITLE
    Get-ComplianceReport - Generate a device compliance HTML report

.SYNOPSIS
    Generate a device compliance HTML report

.DESCRIPTION
    Combines hotfix, Defender, and disk info into a single Reports/Compliance_<ts>.html.
...
.EXAMPLE
    .\Get-ComplianceReport.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Reporting,Compliance
...
$SolutionName = 'Get-ComplianceReport'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Compliance report')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $null = Get-HotFix -ErrorAction SilentlyContinue | Select-Object -First 1
...
try {
    ...
    # Tailored display: compliance HTML
    try {
        $scriptBase = if ($PSScriptRoot) { $PSScriptRoot } else { Split-Path -Parent $PSCommandPath }
        $reports = Join-Path $scriptBase "Reports"
        $html = Get-ChildItem -Path $reports -Filter "*.html" -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 3
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Compliance Reports --" -ForegroundColor DarkGray
        ...
        Write-Log -Message "Could not list compliance reports: $($_.Exception.Message)" -Level 'WARNING'
    }
    # Export professional HTML report
    try {
        $compOs = Get-CimInstance -ClassName Win32_OperatingSystem -ErrorAction SilentlyContinue
        $compCs = Get-CimInstance -ClassName Win32_ComputerSystem -ErrorAction SilentlyContinue
        # Same data as console: 8-domain compliance checks from tailored display above ($checks, $score, $grade)
        $compChkRows = @()
        try {
            $compHf = Get-HotFix -ErrorAction SilentlyContinue | Sort-Object InstalledOn -Descending | Select-Object -First 1
            $compChkRows += [PSCustomObject]@{ Domain="Hotfix"; Check="Latest hotfix"; Result=if($compHf){$compHf.HotFixID}else{"unknown"}; Pass=($null -ne $compHf) }
            $compDef = try { (Get-MpComputerStatus -ErrorAction Stop).AntivirusEnabled } catch { $false }
            $compChkRows += [PSCustomObject]@{ Domain="Defender"; Check="RealTime"; Result=$compDef; Pass=$compDef }
            $compDisk = Get-PSDrive C -ErrorAction SilentlyContinue; $compDiskOk = $compDisk.Free -gt 5GB
            $compChkRows += [PSCustomObject]@{ Domain="Disk"; Check="C: free >5GB"; Result="$([math]::Round($compDisk.Free/1GB,1)) GB"; Pass=$compDiskOk }
            $compBl = try { (Get-BitLockerVolume -MountPoint "C:" -ErrorAction Stop).ProtectionStatus -eq 'On' } catch { $false }
            $compChkRows += [PSCustomObject]@{ Domain="BitLocker"; Check="C: encrypted"; Result=$compBl; Pass=$compBl }
            $compSb = try { Confirm-SecureBootUEFI -ErrorAction Stop } catch { $false }
            $compChkRows += [PSCustomObject]@{ Domain="SecureBoot"; Check="Enabled"; Result=$compSb; Pass=$compSb }
            $compFw2 = try { (Get-NetFirewallProfile -ErrorAction Stop | Where-Object Enabled).Count -gt 0 } catch { $false }
            $compChkRows += [PSCustomObject]@{ Domain="Firewall"; Check="Enabled"; Result=$compFw2; Pass=$compFw2 }
            $compLaps2 = Test-Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\LAPS'
            $compChkRows += [PSCustomObject]@{ Domain="LAPS"; Check="Configured"; Result=$compLaps2; Pass=$compLaps2 }
            $compUp = (Get-Date) - (Get-CimInstance -ClassName Win32_OperatingSystem -ErrorAction SilentlyContinue).LastBootUpTime
            $compChkRows += [PSCustomObject]@{ Domain="Uptime"; Check="Days"; Result=[math]::Round($compUp.TotalDays,1); Pass=($compUp.TotalDays -lt 30) }
        } catch [System.Exception] { Write-Log -Message $_.Exception.Message -Level 'DEBUG' }
        if (-not $compChkRows -and $checks) { $compChkRows = $checks }
        $compPass2 = @($compChkRows | Where-Object { $_.Pass }).Count; $compScore2 = if($compChkRows.Count -gt 0){[math]::Round(($compPass2/$compChkRows.Count)*100,1)}else{0}
        $compGrade2 = if($compScore2 -ge 90){'A'}elseif($compScore2 -ge 80){'B'}elseif($compScore2 -ge 70){'C'}else{'F'}; $compGColor2 = if($compScore2 -ge 80){'#24a148'}elseif($compScore2 -ge 60){'#f1c21b'}else{'#da1e28'}
        $compKpis = @(@{value="$compPass2/$($compChkRows.Count)"; label='Passed'; color='#0f62fe'}, @{value="$compScore2%"; label='Score'; color=$compGColor2}, @{value=$compGrade2; label='Grade'; color=$compGColor2}, @{value=$env:COMPUTERNAME; label='Device'; color='#c6c6c6'})
        $compTable2 = if ($compChkRows) {
            $compH2 = "<th>Domain</th><th>Check</th><th>Result</th><th>Status</th>"
            $compB2 = ($compChkRows | ForEach-Object {
                $compBg = if ($_.Pass) { 'success' } else { 'error' }; $compSt = if ($_.Pass) { 'Pass' } else { 'Fail' }
                "<tr><td>$([System.Net.WebUtility]::HtmlEncode($_.Domain))</td><td>$([System.Net.WebUtility]::HtmlEncode($_.Check))</td><td>$([System.Net.WebUtility]::HtmlEncode("$($_.Result)"))</td><td><span class=`"badge $compBg`">$compSt</span></td></tr>"
            }) -join "`n"
            "<table><thead><tr>$compH2</tr></thead><tbody>$compB2</tbody></table>"
        } else { "<p style=`"color:#8d8d8d;padding:16px`">No compliance data</p>" }
        $compCharts2 = @{donut=@(@{label='Passed';value=$compPass2;color='#24a148'},@{label='Failed';value=($compChkRows.Count-$compPass2);color='#da1e28'})} | ConvertTo-Json -Compress
        $compBody = "<div class=`"section-title`">Compliance Checks (same as console)</div><div class=`"card`"><h2>Compliance Summary - Grade $compGrade2 ($compScore2%)</h2><p style=`"color:#c6c6c6`">System: <strong>$($compOs.Caption)</strong> Build $($compOs.BuildNumber) - Model $($compCs.Model)</p>$compTable2</div>"
        $compBase = if ($PSScriptRoot) { $PSScriptRoot } elseif ($PSCommandPath) { Split-Path -Parent $PSCommandPath } else { (Get-Location).Path }
        $compHtml = Join-Path $compBase "Reports\ComplianceReport_$(Get-Date -Format yyyyMMdd_HHmmss).html"
        Export-ProfessionalHtmlReport -OutputPath $compHtml -Title 'Compliance Report' -Subtitle "Device compliance overview" -Kpis $compKpis -Body $compBody -Charts $compCharts2 -Grade $compGrade2 -GradeRate "$compScore2%" -GradeColor $compGColor2 -ReportName 'Compliance Report' -Version '2.1.0'
        Write-Log -Message "HTML report: $compHtml" -Level 'SUCCESS'
