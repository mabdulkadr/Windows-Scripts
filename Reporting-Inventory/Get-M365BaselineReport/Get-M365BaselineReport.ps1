<#
.TITLE
    Get-M365BaselineReport - M365 baseline audit (ScubaGear-lite, Carbon Dark)

.SYNOPSIS
    M365 baseline audit (ScubaGear-lite, Carbon Dark)

.DESCRIPTION
    Audits local M365-relevant signals (Defender, BitLocker, LAPS, SecureBoot, TLS) and exports Carbon Dark HTML baseline report to Reports. For full CISA ScubaGear, run the dedicated ScubaGear module.

.TAGS
    Reporting,M365,Compliance,Carbon
...
.EXAMPLE
    .\Get-M365BaselineReport.ps1
    Runs with default targets.

.EXAMPLE
    .\Get-M365BaselineReport.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Reporting,M365,Compliance,Carbon
...
$SolutionName = 'Get-M365BaselineReport'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Audit M365 Baseline')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $checks = @()
        $checks += [PSCustomObject]@{ Check="Defender"; Pass=(try {(Get-MpComputerStatus -ErrorAction Stop).RealTimeProtectionEnabled} catch {$false}) }
        ...
        Write-Log -Message "M365 baseline: $pass/$($checks.Count) passed" -Level 'INFO'
        ...
        $htmlPath = Join-Path $reports "M365Baseline_$(Get-Date -Format yyyyMMdd_HHmmss).html"
        $html = "<html><head><style>body{background:#161616;color:#f4f4f4;font-family:Segoe UI;padding:24px}.card{background:#262626;padding:16px;border-radius:8px}</style></head><body><h1>M365 Baseline (ScubaGear-lite)</h1><p>Passed $pass/$($checks.Count)</p></body></html>"
        Set-Content -LiteralPath $htmlPath -Value $html -Encoding UTF8
        Write-Log -Message "M365 HTML: $htmlPath" -Level 'SUCCESS'
...
try {
    ...
    # Tailored display: M365 baseline
    try {
        $checks = @()
        $checks += [PSCustomObject]@{ Check="Defender"; Pass=(try {(Get-MpComputerStatus -ErrorAction Stop).RealTimeProtectionEnabled} catch {$false}) }
        $checks += [PSCustomObject]@{ Check="BitLocker"; Pass=(try {(Get-BitLockerVolume -MountPoint "C:" -ErrorAction Stop).ProtectionStatus -eq 'On'} catch {$false}) }
        $checks += [PSCustomObject]@{ Check="SecureBoot"; Pass=(try { Confirm-SecureBootUEFI -ErrorAction Stop } catch {$false}) }
        $pass = ($checks | Where-Object Pass).Count
        $score = [math]::Round(($pass/$checks.Count)*100,1)
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- M365 Baseline (ScubaGear-lite) --" -ForegroundColor DarkGray
        Write-Host "  Score: $score% ($pass/$($checks.Count))" -ForegroundColor $(if ($score -ge 80){'Green'} else {'Yellow'})
        $checks | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
        $scriptBase = if ($PSScriptRoot) { $PSScriptRoot } else { Split-Path -Parent $PSCommandPath }
        $reports = Join-Path $scriptBase "Reports"
        $html = Get-ChildItem -Path $reports -Filter "M365Baseline*.html" -ErrorAction SilentlyContinue | Select-Object -First 1
        if ($html) { Write-Host "  HTML: $($html.FullName)" -ForegroundColor Green }
    } catch {
        Write-Log -Message "Could not show M365 baseline: $($_.Exception.Message)" -Level 'WARNING'
    }
