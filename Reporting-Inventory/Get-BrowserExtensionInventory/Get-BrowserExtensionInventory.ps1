<#
.TITLE
    Get-BrowserExtensionInventory - Inventory Edge/Chrome extensions

.SYNOPSIS
    Inventory Edge/Chrome extensions

.DESCRIPTION
    Scans current user Edge and Chrome profile Preferences JSON for installed extensions, reports ID, name, version, and enabled state. No elevation needed.

.TAGS
    Reporting,Browser,Inventory
...
.EXAMPLE
    .\Get-BrowserExtensionInventory.ps1
    Runs with default targets.

.EXAMPLE
    .\Get-BrowserExtensionInventory.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Reporting,Browser,Inventory
...
$SolutionName = 'Get-BrowserExtensionInventory'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Inventory Extensions')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $edgePref = Join-Path $env:LOCALAPPDATA "Microsoft\Edge\User Data\Default\Preferences"
        $chromePref = Join-Path $env:LOCALAPPDATA "Google\Chrome\User Data\Default\Preferences"
        if (-not (Test-Path $edgePref) -and -not (Test-Path $chromePref)) {
            Write-Log -Message "No Edge/Chrome Preferences found - no extensions to inventory" -Level 'INFO'
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
...
try {
    ...
    # Tailored display: browser extensions
    try {
        $prefs = @(
            @{ Browser='Edge'; Path=(Join-Path $env:LOCALAPPDATA "Microsoft\Edge\User Data\Default\Preferences") },
            @{ Browser='Chrome'; Path=(Join-Path $env:LOCALAPPDATA "Google\Chrome\User Data\Default\Preferences") }
        )
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Browser Extensions --" -ForegroundColor DarkGray
        $found = $false
        foreach ($p in $prefs) {
            if (Test-Path $p.Path) {
                $found = $true
                $json = Get-Content -LiteralPath $p.Path -Raw -ErrorAction SilentlyContinue | ConvertFrom-Json -ErrorAction SilentlyContinue
                $exts = $json.extensions.settings
                if ($exts) {
                    Write-Host "  $($p.Browser): $($exts.PSObject.Properties.Count) extensions" -ForegroundColor Cyan
                    $exts.PSObject.Properties | Select-Object -First 5 | ForEach-Object {
                        $id = $_.Name; $val = $_.Value
                        [PSCustomObject]@{ Browser=$p.Browser; ID=$id.Substring(0,[Math]::Min(12,$id.Length)); State=(if ($val.state) { $val.state } else { 'unknown' }) } | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Gray }
                    }
                } else { Write-Host "  $($p.Browser): no extensions node" -ForegroundColor Yellow }
            } else { Write-Host "  $($p.Browser): Preferences not found" -ForegroundColor Gray }
        }
        if (-not $found) { Write-Log -Message "No browser Preferences found" -Level 'WARNING' }
    } catch {
        Write-Log -Message "Could not inventory extensions: $($_.Exception.Message)" -Level 'WARNING'
    }
    # Export professional HTML report
    try {
        $extCount = try { (Get-ChildItem -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Browser Helper Objects' -ErrorAction SilentlyContinue | Measure-Object).Count } catch { 0 }
        $extKpis = @(@{value=$extCount; label='BHOs Found'; color='#0f62fe'}, @{value=$ok; label='Checks OK'; color='#24a148'}, @{value=$failed; label='Failed'; color='#da1e28'}, @{value=$env:COMPUTERNAME; label='Device'; color='#8a3ffc'})
        # Same data as console: browser preferences extensions from $prefs / $exts
        $extTableHtml = if ($prefs) {
            $extRows2 = @()
            foreach ($extP in $prefs) {
                if (Test-Path $extP.Path) {
                    $extJson2 = Get-Content -LiteralPath $extP.Path -Raw -ErrorAction SilentlyContinue | ConvertFrom-Json -ErrorAction SilentlyContinue
                    $extSet2 = $extJson2.extensions.settings
                    if ($extSet2) {
                        foreach ($extProp in ($extSet2.PSObject.Properties | Select-Object -First 10)) {
                            $extId2 = $extProp.Name.Substring(0, [Math]::Min(12, $extProp.Name.Length))
                            $extSt2 = if ($extProp.Value.state) { $extProp.Value.state } else { 'unknown' }
                            $extBg2 = if ($extSt2 -eq 'enabled') { 'success' } elseif ($extSt2 -eq 'disabled') { 'neutral' } else { 'info' }
                            $extRows2 += "<tr><td>$extP.Browser</td><td>$extId2</td><td><span class=`"badge $extBg2`">$extSt2</span></td></tr>"
                        }
                    }
                }
            }
            if ($extRows2) { "<table><thead><tr><th>Browser</th><th>ID</th><th>State</th></tr></thead><tbody>$($extRows2 -join "`n")</tbody></table>" } else { "<p style=`"color:#8d8d8d;padding:16px`">No extensions found (same as console - $extCount BHOs)</p>" }
        } else { "<p style=`"color:#8d8d8d;padding:16px`">No browser data (same as console)</p>" }
        $extBody = "<div class=`"section-title`">Browser Extension Inventory</div><div class=`"card`"><h2>Browser Extensions &amp; BHOs</h2>$extTableHtml</div>"
        $extBase = if ($PSScriptRoot) { $PSScriptRoot } elseif ($PSCommandPath) { Split-Path -Parent $PSCommandPath } else { (Get-Location).Path }
        $extHtml = Join-Path $extBase "Reports\BrowserExtensions_$(Get-Date -Format yyyyMMdd_HHmmss).html"
        Export-ProfessionalHtmlReport -OutputPath $extHtml -Title 'Browser Extensions' -Subtitle "Browser extension inventory" -Kpis $extKpis -Body $extBody -ReportName 'Browser Extensions' -Version '2.1.0'
        Write-Log -Message "HTML report: $extHtml" -Level 'SUCCESS'
