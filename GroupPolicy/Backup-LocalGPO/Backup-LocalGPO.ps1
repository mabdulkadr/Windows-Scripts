<#
.TITLE
    Backup-LocalGPO - Backup Local Group Policy with LGPO

.SYNOPSIS
    Backup Local Group Policy with LGPO

.DESCRIPTION
    Runs LGPO.exe /b to save a Local GPO backup to the supplied path.

.TAGS
    GroupPolicy,LGPO,Backup
...
.EXAMPLE
    .\Backup-LocalGPO.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - GroupPolicy,LGPO,Backup
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Backup LGPO')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $null = Get-Command LGPO.exe -ErrorAction SilentlyContinue; if (-not $null) { throw 'LGPO.exe not found' }
...
try {
    ...
    # Tailored display: comprehensive GPO backup diagnostics
    try {
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- LGPO Tool --" -ForegroundColor DarkGray
        $lgpo = Get-Command LGPO.exe -ErrorAction SilentlyContinue
        if ($lgpo) {
            $ver = try { (Get-Item $lgpo.Source).VersionInfo.FileVersion } catch { "unknown" }
            Write-Host "  LGPO.exe: $($lgpo.Source) (v$ver)" -ForegroundColor Cyan
        } else {
            Write-Host "  LGPO.exe not found in PATH - download from Microsoft Security Compliance Toolkit" -ForegroundColor Yellow
            Write-Host "  Fallback: secedit /export and registry export will be used" -ForegroundColor Gray
        }

        Write-Host "  -- Local GPO Paths --" -ForegroundColor DarkGray
        $gpoPaths = @("$env:SystemRoot\System32\GroupPolicy", "$env:SystemRoot\System32\GroupPolicyUsers")
        foreach ($gp in $gpoPaths) {
            $exists = Test-Path $gp
            $count = if ($exists) { (Get-ChildItem -Path $gp -Recurse -ErrorAction SilentlyContinue | Measure-Object).Count } else { 0 }
            $size = if ($exists) { [math]::Round(((Get-ChildItem -Path $gp -Recurse -ErrorAction SilentlyContinue | Measure-Object -Property Length -Sum).Sum / 1KB),1) } else { 0 }
            Write-Host ("  {0} : Exists={1} Files={2} Size={3} KB" -f $gp, $exists, $count, $size) -ForegroundColor $(if ($exists) {'Cyan'} else {'Yellow'})
        }

        Write-Host "  -- Backup Location --" -ForegroundColor DarkGray
        $backupRoot = Join-Path $env:TEMP "GPOBackup"
        $scriptBase = if ($PSScriptRoot) { $PSScriptRoot } else { Split-Path -Parent $PSCommandPath }
        $reports = Join-Path $scriptBase "Reports"
        Write-Host "  Temp Backup: $backupRoot" -ForegroundColor Gray
        Write-Host "  Reports: $reports" -ForegroundColor Gray
        $items = Get-ChildItem -Path $backupRoot -ErrorAction SilentlyContinue | Select-Object Name, LastWriteTime, @{N='SizeKB';E={[math]::Round($_.Length/1KB,1)}} -First 10
        if ($items) {
            Write-Host "  Backups (last 10):" -ForegroundColor Gray
            $items | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
            $latest = $items | Sort-Object LastWriteTime -Descending | Select-Object -First 1
            Write-Host "  Latest: $($latest.Name) ($($latest.SizeKB) KB) at $($latest.LastWriteTime)" -ForegroundColor Green
        } else {
            Write-Host "  No backups yet - run LGPO.exe /b $backupRoot" -ForegroundColor Yellow
        }

        # Also check Reports for previous backups copied there
        if (Test-Path $reports) {
            $repItems = Get-ChildItem -Path $reports -Filter "*GPO*" -ErrorAction SilentlyContinue | Select-Object -First 5
            if ($repItems) {
                Write-Host "  Reports backups:" -ForegroundColor Gray
                $repItems | Select-Object Name, LastWriteTime | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Gray }
            }
        }

    } catch {
        Write-Log -Message "Could not list GPO backup: $($_.Exception.Message)" -Level 'WARNING'
    }
