...
try {
    ...
    # Tailored display: backup folder
    try {
        $backupRoot = Join-Path $env:TEMP "GPOBackup"
        $items = Get-ChildItem -Path $backupRoot -ErrorAction SilentlyContinue | Select-Object Name, LastWriteTime, Length -First 10
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- GPO Backup Location --" -ForegroundColor DarkGray
        Write-Host "  Path: $backupRoot" -ForegroundColor Gray
        if ($items) { $items | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
        else { Write-Log -Message "No backup files yet in $backupRoot" -Level 'INFO' }
    } catch {
