<#
.TITLE
    Invoke-RegistryPolicyAudit - Compare registry policy keys against a baseline

.SYNOPSIS
    Compare registry policy keys against a baseline

.DESCRIPTION
    Imports a CliXML baseline and reports drift in HKLM\SOFTWARE\Policies keys.

.TAGS
    GroupPolicy,Registry,Audit
...
.EXAMPLE
    .\Invoke-RegistryPolicyAudit.ps1
    Runs with default targets.

.EXAMPLE
    .\Invoke-RegistryPolicyAudit.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - GroupPolicy,Registry,Audit
...
$SolutionName = 'Invoke-RegistryPolicyAudit'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Audit registry')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $null = Get-ItemProperty 'HKLM:\SOFTWARE\Policies' -ErrorAction SilentlyContinue | Select-Object -First 1
...
try {
    ...
    # Tailored display: sample policy keys
    try {
        $keys = @('HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate','HKLM:\SOFTWARE\Policies\Microsoft\WindowsStore','HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System')
        $found = foreach ($k in $keys) { if (Test-Path $k) { Get-ItemProperty -Path $k -ErrorAction SilentlyContinue | Select-Object -First 1 } }
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Registry Policy Keys Sample --" -ForegroundColor DarkGray
        if ($found) { $found | Format-List | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
        else { Write-Log -Message "No sampled policy keys found" -Level 'INFO' }
    } catch {
        Write-Log -Message "Could not audit registry policies: $($_.Exception.Message)" -Level 'WARNING'
    }
    # Export professional HTML report
    try {
        # Same specific subkeys the console reads via Get-ItemProperty
        $regSamplePaths = @('HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate','HKLM:\SOFTWARE\Policies\Microsoft\WindowsStore','HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System','HKLM:\SOFTWARE\Policies\Microsoft\Windows\Safer','HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Explorer')
        $regDtlRows = @()
        $regTot2 = 0
        foreach ($regPath in $regSamplePaths) {
            $regItem = try { Get-Item -Path $regPath -ErrorAction Stop } catch { $null }
            if ($regItem) {
                $regPropNames = $regItem.Property
                if ($regPropNames) {
                    $regTot2 += @($regPropNames).Count
                    foreach ($regProp in @($regPropNames | Select-Object -First 50)) {
                        $regVal = try { (Get-ItemProperty -Path $regPath -Name $regProp -ErrorAction Stop).$regProp } catch { '<access denied>' }
                        $regDtlRows += [PSCustomObject]@{ Key = Split-Path $regPath -Leaf; Name = $regProp; Value = if($null -eq $regVal){''}else{"$regVal"} }
                    }
                }
            }
        }
        $regKpis = @(@{value=$regTot2; label='Policy Values'; color='#0f62fe'}, @{value=$ok; label='Checks OK'; color='#24a148'}, @{value=$failed; label='Failed'; color='#da1e28'}, @{value=$env:COMPUTERNAME; label='Device'; color='#8a3ffc'})
        $regDtlTable = if ($regDtlRows) {
            $regDtlH = "<th>Key</th><th>Name</th><th>Value</th>"
            $regDtlB = ($regDtlRows | ForEach-Object {
                $regDtlK = [System.Net.WebUtility]::HtmlEncode("$($_.Key)")
                $regDtlN = [System.Net.WebUtility]::HtmlEncode("$($_.Name)")
                $regDtlV = [System.Net.WebUtility]::HtmlEncode("$($_.Value)")
                "<tr><td style=`"font-family:monospace;font-size:11px`">$regDtlK</td><td style=`"font-family:monospace;font-size:11px`">$regDtlN</td><td style=`"word-break:break-all;font-family:monospace;font-size:11px`">$regDtlV</td></tr>"
            }) -join "`n"
            "<table><thead><tr>$regDtlH</tr></thead><tbody>$regDtlB</tbody></table>"
        } else { "<p style=`"color:#8d8d8d;padding:16px`">No registry policy values found (same as console)</p>" }
        $regBody = "<div class=`"section-title`">Registry Policy Audit</div><div class=`"card`"><h2>Policy Configuration ($regTot2 values - same as console)</h2>$regDtlTable</div>"
        $regBase = if ($PSScriptRoot) { $PSScriptRoot } elseif ($PSCommandPath) { Split-Path -Parent $PSCommandPath } else { (Get-Location).Path }
        $regHtml = Join-Path $regBase "Reports\RegistryAudit_$(Get-Date -Format yyyyMMdd_HHmmss).html"
        Export-ProfessionalHtmlReport -OutputPath $regHtml -Title 'Registry Policy Audit' -Subtitle "Group Policy registry audit" -Kpis $regKpis -Body $regBody -ReportName 'Registry Policy Audit' -Version '2.1.0'
        Write-Log -Message "HTML report: $regHtml" -Level 'SUCCESS'
