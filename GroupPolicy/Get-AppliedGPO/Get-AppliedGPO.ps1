...
try {
    ...
    # Tailored display: applied GPOs via gpresult /r
    try {
        $out = gpresult /r 2>&1 | Out-String
        $gpos = $out -split "`r`n" | Where-Object { $_ -match "Applied Group Policy Objects|GPO Name" } | Select-Object -First 20
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Applied GPOs (gpresult /r) --" -ForegroundColor DarkGray
        if ($gpos) { $gpos | ForEach-Object { Write-Host "  $_" -ForegroundColor Cyan } }
        else { Write-Host ($out | Select-Object -First 30) -ForegroundColor Gray }
    } catch {
