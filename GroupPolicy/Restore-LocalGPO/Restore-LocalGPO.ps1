<#
.TITLE
    Restore-LocalGPO - Restore Local Group Policy from a backup

.SYNOPSIS
    Restore Local Group Policy from a backup

.DESCRIPTION
    Runs LGPO.exe /r against a previously saved backup folder.

.TAGS
    GroupPolicy,LGPO,Restore
...
.EXAMPLE
    .\Restore-LocalGPO.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - GroupPolicy,LGPO,Restore
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Restore LGPO')) {
...
try {
    ...
    # Tailored display: comprehensive restore diagnostics
    try {
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- LGPO Tool --" -ForegroundColor DarkGray
        $lgpo = Get-Command LGPO.exe -ErrorAction SilentlyContinue
        if ($lgpo) { Write-Host "  LGPO.exe: $($lgpo.Source)" -ForegroundColor Cyan }
        else { Write-Host "  LGPO.exe not found - download from Microsoft Security Compliance Toolkit" -ForegroundColor Yellow }

        Write-Host "  -- Backup Source --" -ForegroundColor DarkGray
        $backupRoots = @(
            (Join-Path $env:TEMP "GPOBackup"),
            (Join-Path (if ($PSScriptRoot) { $PSScriptRoot } else { Split-Path -Parent $PSCommandPath }) "Reports"),
            "C:\GPOBackup"
        )
        $foundBackup = $false
        foreach ($br in $backupRoots) {
            if (Test-Path $br) {
                $items = Get-ChildItem -Path $br -Directory -ErrorAction SilentlyContinue | Select-Object -First 5
                if ($items) {
                    $foundBackup = $true
                    Write-Host "  Source: $br" -ForegroundColor Cyan
                    foreach ($it in $items) {
                        $size = [math]::Round(((Get-ChildItem -Path $it.FullName -Recurse -ErrorAction SilentlyContinue | Measure-Object -Property Length -Sum).Sum / 1KB),1)
                        Write-Host ("    {0,-30} {1,6} KB  {2}" -f $it.Name, $size, $it.LastWriteTime) -ForegroundColor Gray
                        # Check for key files
                        $hasPol = Test-Path (Join-Path $it.FullName "Machine\Registry.pol")
                        $hasInf = Test-Path (Join-Path $it.FullName "Machine\Microsoft\Windows NT\SecEdit\GptTmpl.inf")
                        Write-Host ("      Registry.pol: {0} | SecEdit: {1}" -f $hasPol, $hasInf) -ForegroundColor Gray
                    }
                }
            }
        }
        if (-not $foundBackup) {
            Write-Log -Message "No backup found in searched paths" -Level 'WARNING'
            Write-Host "  Searched: $($backupRoots -join ', ')" -ForegroundColor Yellow
            Write-Host "  Create backup first: .\Backup-LocalGPO.ps1" -ForegroundColor Gray
        }

        Write-Host "  -- Current GPO State (before restore) --" -ForegroundColor DarkGray
        ...
        foreach ($gp in $gpoPaths) {
            ...
            Write-Host ("  {0} : {1} files" -f $gp, $count) -ForegroundColor $(if ($exists -and $count -gt 0) {'Yellow'} else {'Green'})
        }

        Write-Host "  -- Restore Command --" -ForegroundColor DarkGray
        Write-Host "  LGPO.exe /r <BackupFolder> /n <BackupName>" -ForegroundColor Gray
        Write-Host "  Or: LGPO.exe /m <Backup>\DomainSysvol\GPO\Machine\registry.pol" -ForegroundColor Gray
        Write-Host "  Verify: gpresult /R  and  gpresult /H Reports\GPResult.html" -ForegroundColor Gray

    } catch {
        Write-Log -Message "Could not list restore source: $($_.Exception.Message)" -Level 'WARNING'
    }
