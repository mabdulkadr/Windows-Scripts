...
try {
    ...
    # Tailored display: restore source
    try {
        $backupRoot = Join-Path $env:TEMP "GPOBackup"
        $items = Get-ChildItem -Path $backupRoot -ErrorAction SilentlyContinue | Select-Object Name, LastWriteTime -First 10
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Restore Source ($backupRoot) --" -ForegroundColor DarkGray
        if ($items) { $items | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
        else { Write-Log -Message "No backup found in $backupRoot" -Level 'WARNING' }
    } catch {
