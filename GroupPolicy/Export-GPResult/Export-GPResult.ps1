...
function Invoke-TargetAction {
    ...
    try {
        ...
        & gpresult /h $env:TEMP\gpresult.html /f | Out-Null
...
try {
    ...
    # Tailored display: exported HTML report
    try {
        $scriptBase = if ($PSScriptRoot) { $PSScriptRoot } else { Split-Path -Parent $PSCommandPath }
        $reportsDir = Join-Path $scriptBase "Reports"
        $html = Get-ChildItem -Path $reportsDir -Filter "*.html" -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 5
        $tempHtml = Join-Path $env:TEMP "gpresult.html"
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- GPResult Report --" -ForegroundColor DarkGray
        if (Test-Path $tempHtml) {
            $sz = (Get-Item $tempHtml).Length
            Write-Host "  Temp HTML: $tempHtml ($sz bytes)" -ForegroundColor Cyan
        }
        ...
        else { Write-Host "  Reports folder: $reportsDir (no HTML yet - will be created on next run beside script)" -ForegroundColor Gray }
    } catch {
