<#
.TITLE
    Reset-LocalGPO - Reset Local Group Policy to default

.SYNOPSIS
    Reset Local Group Policy to default

.DESCRIPTION
    Runs LGPO /refreshpolicy to reset local policy to defaults.

.TAGS
    GroupPolicy,LGPO,Reset
...
.EXAMPLE
    .\Reset-LocalGPO.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - GroupPolicy,LGPO,Reset
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Reset LGPO')) {
...
try {
    ...
    # Tailored display: comprehensive GPO reset diagnostics
    try {
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Local GPO Paths (before reset) --" -ForegroundColor DarkGray
        $paths = @("$env:SystemRoot\System32\GroupPolicy", "$env:SystemRoot\System32\GroupPolicyUsers")
        foreach ($p in $paths) {
            $exists = Test-Path $p
            if ($exists) {
                $files = Get-ChildItem -Path $p -Recurse -ErrorAction SilentlyContinue
                $count = ($files | Measure-Object).Count
                $sizeKB = [math]::Round((($files | Measure-Object -Property Length -Sum).Sum / 1KB),1)
                $lastMod = try { (Get-ChildItem -Path $p -Recurse -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1).LastWriteTime } catch { "unknown" }
                Write-Host ("  {0}" -f $p) -ForegroundColor Cyan
                Write-Host ("    Files: {0} | Size: {1} KB | Last: {2}" -f $count, $sizeKB, $lastMod) -ForegroundColor Gray
                # Check Registry.pol
                $regPol = Join-Path $p "Machine\Registry.pol"
                if (Test-Path $regPol) {
                    $regSize = (Get-Item $regPol).Length
                    Write-Host ("    Registry.pol: {0} bytes" -f $regSize) -ForegroundColor Yellow
                }
                # Check GptTmpl.inf
                $gpt = Join-Path $p "Machine\Microsoft\Windows NT\SecEdit\GptTmpl.inf"
                if (Test-Path $gpt) { Write-Host "    SecEdit: $gpt exists" -ForegroundColor Gray }
            } else {
                Write-Host ("  {0} : Not Found (already reset?)" -f $p) -ForegroundColor Green
            }
        }

        Write-Host "  -- What Will Be Reset --" -ForegroundColor DarkGray
        Write-Host "  secedit /configure /cfg %windir%\inf\defltbase.inf /db defltbase.sdb /verbose" -ForegroundColor Gray
        Write-Host "  RD /S /Q ""%SystemRoot%\System32\GroupPolicy""" -ForegroundColor Gray
        Write-Host "  RD /S /Q ""%SystemRoot%\System32\GroupPolicyUsers""" -ForegroundColor Gray
        Write-Host "  gpupdate /force (after reboot)" -ForegroundColor Gray
        Write-Host "  Note: Domain GPOs will reapply at next refresh if machine is domain-joined" -ForegroundColor Yellow

        Write-Host "  -- Domain Membership --" -ForegroundColor DarkGray
        $cs = Get-CimInstance -ClassName Win32_ComputerSystem -ErrorAction SilentlyContinue
        Write-Host ("  Domain: {0} | PartOfDomain: {1}" -f $cs.Domain, $cs.PartOfDomain) -ForegroundColor Cyan
        Write-Host "  Backup recommendation: Run Backup-LocalGPO.ps1 before reset" -ForegroundColor Gray

    } catch {
        Write-Log -Message "Could not check GPO paths: $($_.Exception.Message)" -Level 'WARNING'
    }
