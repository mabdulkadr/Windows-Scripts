<#
.TITLE
    Install-WinGetApp - Bulk install apps via WinGet (WhatIf-safe)

.SYNOPSIS
    Bulk install apps via WinGet (WhatIf-safe)

.DESCRIPTION
    Installs one or more apps via WinGet (winget install --silent) with bulk support. Checks winget availability, skips already-installed packages, and supports -WhatIf preview. Reports installed vs skipped.

.TAGS
    Software,WinGet,Deployment

.PLATFORM
    Windows 10/11/Server 2019+

.PERMISSIONS
    Administrator recommended; Standard user for per-user installs
...
.EXAMPLE
    .\Install-WinGetApp.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Software,WinGet,Deployment
...
$SolutionName = 'Install-WinGetApp'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Install via WinGet')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $winget = Get-Command winget -ErrorAction SilentlyContinue
        if (-not $winget) { throw "winget not found - install App Installer from Microsoft Store" }
...
try {
    ...
    # Tailored display: WinGet version + search sample
    try {
        $ver = (& winget --version 2>&1 | Out-String).Trim()
        $list = & winget list --accept-source-agreements 2>&1 | Select-Object -First 20 | Out-String
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- WinGet --" -ForegroundColor DarkGray
        Write-Host "  Version: $ver" -ForegroundColor Cyan
        if ($list) { Write-Host "  Installed (sample 20 lines):" -ForegroundColor Gray; Write-Host $list -ForegroundColor Gray }
    } catch {
        Write-Log -Message "Could not query winget: $($_.Exception.Message)" -Level 'WARNING'
    }
