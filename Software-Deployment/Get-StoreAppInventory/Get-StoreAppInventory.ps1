<#
.TITLE
    Get-StoreAppInventory - Inventory Microsoft Store (AppX) packages

.SYNOPSIS
    Inventory Microsoft Store (AppX) packages

.DESCRIPTION
    Enumerates AppX packages via Get-AppxPackage for all users, reports Name, Version, Publisher, and InstallLocation. Exports CSV to Reports beside script.

.TAGS
    Software,Store,AppX,Inventory
...
.EXAMPLE
    .\Get-StoreAppInventory.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Software,Store,AppX,Inventory
...
$SolutionName = 'Get-StoreAppInventory'
$ScriptMode   = 'run'
...
try {
    ...
    # Tailored display: Store apps
    try {
        $apps = Get-AppxPackage -ErrorAction SilentlyContinue | Select-Object Name, Version, Publisher -First 15
        $count = (Get-AppxPackage -ErrorAction SilentlyContinue | Measure-Object).Count
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Microsoft Store Apps --" -ForegroundColor DarkGray
        Write-Host "  Total AppX packages: $count" -ForegroundColor Cyan
        if ($apps) { $apps | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
        # Export CSV beside script
        try {
            ...
            $csv = Join-Path $reports "StoreAppInventory_$(Get-Date -Format yyyyMMdd_HHmmss).csv"
            Get-AppxPackage -ErrorAction SilentlyContinue | Select-Object Name, Version, Publisher, InstallLocation | Export-Csv -Path $csv -NoTypeInformation -Encoding UTF8
            Write-Host "  CSV: $csv" -ForegroundColor Green
        } catch { Write-Log -Message "CSV export failed: $($_.Exception.Message)" -Level 'WARNING' }
    } catch {
        Write-Log -Message "Could not inventory Store apps: $($_.Exception.Message)" -Level 'WARNING'
    }
    # Export professional HTML report
    try {
        $apps2 = try { Get-AppxPackage -ErrorAction SilentlyContinue | Select-Object Name, Version, Publisher -First 30 } catch { @() }
        $apps2Count = @($apps2).Count
        $apps2Kpis = @(@{value=$apps2Count; label='Store Apps'; color='#0f62fe'}, @{value=$ok; label='Checks OK'; color='#24a148'}, @{value=$failed; label='Failed'; color='#da1e28'}, @{value=$env:COMPUTERNAME; label='Device'; color='#8a3ffc'})
        $apps2Rows = if($apps2){ $h=($apps2[0].PSObject.Properties | ForEach-Object { "<th>$($_.Name)</th>" }) -join ""; $b=($apps2 | Select-Object -First 30 | ForEach-Object { $cs=($_.PSObject.Properties | ForEach-Object { "<td>$([System.Net.WebUtility]::HtmlEncode("$($_.Value)"))</td>" }) -join ""; "<tr>$cs</tr>" }) -join "`n"; "<table><thead><tr>$h</tr></thead><tbody>$b</tbody></table>" } else { "<p style=`"color:#8d8d8d;padding:16px`">No store apps found</p>" }
        $apps2Body = "<div class=`"section-title`">Store App Inventory</div><div class=`"card`"><h2>Microsoft Store Apps ($apps2Count)</h2>$apps2Rows</div>"
        $apps2Base = if ($PSScriptRoot) { $PSScriptRoot } elseif ($PSCommandPath) { Split-Path -Parent $PSCommandPath } else { (Get-Location).Path }
        $apps2Html = Join-Path $apps2Base "Reports\StoreApps_$(Get-Date -Format yyyyMMdd_HHmmss).html"
        Export-ProfessionalHtmlReport -OutputPath $apps2Html -Title 'Store Apps' -Subtitle "Microsoft Store app inventory" -Kpis $apps2Kpis -Body $apps2Body -ReportName 'Store Apps' -Version '2.1.0'
        Write-Log -Message "HTML report: $apps2Html" -Level 'SUCCESS'
