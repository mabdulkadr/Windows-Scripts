...
try {
    ...
    # Tailored display: fonts folder
    try {
        $fontsDir = Join-Path $PSScriptRoot "Fonts"
        if (-not (Test-Path $fontsDir)) { $fontsDir = $PSScriptRoot }
        $fonts = Get-ChildItem -Path $fontsDir -Filter "*.ttf" -ErrorAction SilentlyContinue | Select-Object Name, Length -First 10
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Fonts to Install --" -ForegroundColor DarkGray
        Write-Host "  Source: $fontsDir" -ForegroundColor Gray
        if ($fonts) { $fonts | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
        else { Write-Log -Message "No .ttf found in $fontsDir" -Level 'WARNING' }
    } catch {
