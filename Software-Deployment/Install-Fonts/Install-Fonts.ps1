<#
.TITLE
    Install-Fonts - Register fonts from a folder as system fonts

.SYNOPSIS
    Register fonts from a folder as system fonts

.DESCRIPTION
    Copies every TTF/OTF in the supplied folder to the Fonts directory via Shell.Application.
...
.EXAMPLE
    .\Install-Fonts.ps1 -TargetName "Server01","Server02"
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Install fonts')) {
...
try {
    ...
    # Tailored display: comprehensive font inventory
    try {
        ...
        if ($fonts) {
            $fonts | ForEach-Object {
                ...
                [PSCustomObject]@{ File=$_.Name; Type=$type; SizeKB=[math]::Round($_.Length/1KB,1); Modified=$_.LastWriteTime.ToString('yyyy-MM-dd') }
            } | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
            Write-Host ("  Total to install: {0} (showing 10) | Total size: {1} KB" -f $fonts.Count, [math]::Round(($fonts | Measure-Object -Property Length -Sum).Sum/1KB,1)) -ForegroundColor Green
        } else {
            Write-Host "  No .ttf/.otf found in $fontsDir" -ForegroundColor Yellow
            Write-Host "  Place font files in .\Fonts\ and re-run" -ForegroundColor Gray
        }

        Write-Host "  -- Already Installed Fonts (sample) --" -ForegroundColor DarkGray
        ...
        if ($installed) {
            ...
            Write-Host "  Total installed fonts: $totalInstalled" -ForegroundColor Gray
        }

        Write-Host "  -- Registry (Fonts) --" -ForegroundColor DarkGray
        ...
        Write-Host "  Registered fonts in registry: $regCount" -ForegroundColor Cyan
        if ($regFonts) {
            $regFonts.PSObject.Properties | Where-Object { $_.Name -notlike "PS*" } | Select-Object -First 3 | ForEach-Object {
                Write-Host ("    {0} = {1}" -f $_.Name, $_.Value) -ForegroundColor Gray
            }
        }

        Write-Host "  -- Install Method --" -ForegroundColor DarkGray
        Write-Host "  Uses Shell.Application Namespace(0x14) CopyHere with 0x10 (no dialog)" -ForegroundColor Gray
        Write-Host "  Or: Copy to C:\Windows\Fonts + New-ItemProperty in HKLM\...\Fonts" -ForegroundColor Gray
