...
.SYNOPSIS
    Remove fonts from the Fonts folder

.DESCRIPTION
    Deletes every TTF/OTF in the Fonts folder and clears the registry entries.
...
.EXAMPLE
    .\Uninstall-Fonts.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Software,Fonts
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Remove fonts')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $null = Get-ChildItem 'C:\Windows\Fonts' -ErrorAction SilentlyContinue | Select-Object -First 1
...
try {
    ...
    # Tailored display: comprehensive font uninstall diagnostics
    try {
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Fonts Marked for Removal (Fonts folder) --" -ForegroundColor DarkGray
        $fontsDir = Join-Path $scriptBase "Fonts"
        if (-not (Test-Path $fontsDir)) { $fontsDir = $scriptBase }
        $toRemove = Get-ChildItem -Path $fontsDir -Include "*.ttf","*.otf" -Recurse -ErrorAction SilentlyContinue | Select-Object -First 10
        if ($toRemove) {
            $toRemove | ForEach-Object {
                $type = $_.Extension.ToUpper().TrimStart('.')
                [PSCustomObject]@{ File=$_.Name; Type=$type; SizeKB=[math]::Round($_.Length/1KB,1); Path=$_.FullName }
            } | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Yellow }
            Write-Host ("  Total to remove: {0} (showing 10)" -f $toRemove.Count) -ForegroundColor Yellow
        } else {
            Write-Host "  No .ttf/.otf found in $fontsDir - specify Fonts folder with fonts to uninstall" -ForegroundColor Gray
            Write-Host "  Example: .\Uninstall-Fonts.ps1 -TargetName 'MyFont.ttf'" -ForegroundColor Gray
        }

        Write-Host "  -- Currently Installed Fonts (registry) --" -ForegroundColor DarkGray
        $regFonts = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Fonts' -ErrorAction SilentlyContinue
        $regCount = ($regFonts.PSObject.Properties | Where-Object { $_.Name -notlike "PS*" } | Measure-Object).Count
        Write-Host "  Total registered fonts: $regCount" -ForegroundColor Cyan
        if ($regFonts) {
            $regFonts.PSObject.Properties | Where-Object { $_.Name -notlike "PS*" } | Select-Object -First 5 | ForEach-Object {
                Write-Host ("    {0,-35} = {1}" -f $_.Name, $_.Value) -ForegroundColor Gray
            }
            if ($regCount -gt 5) { Write-Host "  ... and $($regCount - 5) more (see C:\Windows\Fonts)" -ForegroundColor Gray }
        }

        Write-Host "  -- Fonts Folder (C:\Windows\Fonts) --" -ForegroundColor DarkGray
        $installed = Get-ChildItem -Path "C:\Windows\Fonts" -ErrorAction SilentlyContinue | Select-Object -First 5
        if ($installed) {
            $installed | Select-Object Name, Length, LastWriteTime | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Gray }
            $totalInstalled = (Get-ChildItem -Path "C:\Windows\Fonts" -ErrorAction SilentlyContinue | Measure-Object).Count
            Write-Host "  Total files in Fonts: $totalInstalled" -ForegroundColor Gray
        }

        Write-Host "  -- Uninstall Method --" -ForegroundColor DarkGray
        Write-Host "  1. Delete file from C:\Windows\Fonts (requires ownership)" -ForegroundColor Gray
        Write-Host "  2. Remove registry value from HKLM\...\Fonts" -ForegroundColor Gray
        Write-Host "  3. Broadcast WM_FONTCHANGE via AddFontResource" -ForegroundColor Gray
        Write-Host "  Verify: Get-ChildItem C:\Windows\Fonts | Where Name -like '*<FontName>*'" -ForegroundColor Gray
        Write-Host "  Backup: Copy fonts to .\Fonts-Backup\ before removal" -ForegroundColor Yellow

    } catch {
        Write-Log -Message "Could not list fonts: $($_.Exception.Message)" -Level 'WARNING'
    }
