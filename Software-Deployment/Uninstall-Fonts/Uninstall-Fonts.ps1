...
try {
    ...
    # Tailored display: installed fonts sample
    try {
        $regFonts = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Fonts' -ErrorAction SilentlyContinue
        $sample = $regFonts.PSObject.Properties | Where-Object { $_.Name -notlike "PS*" } | Select-Object Name, Value -First 10
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Installed Fonts (registry sample) --" -ForegroundColor DarkGray
