...
try {
    ...
    # Tailored display: BitLocker C: after action
    try {
        $vol = Get-BitLockerVolume -MountPoint "C:" -ErrorAction SilentlyContinue
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- BitLocker C: --" -ForegroundColor DarkGray
        if ($vol) {
            [PSCustomObject]@{ MountPoint=$vol.MountPoint; Protection=$vol.ProtectionStatus; Encrypted="$($vol.EncryptionPercentage)%"; Protectors=(($vol.KeyProtector.KeyProtectorType) -join ', ') } | Format-List | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
        }
    } catch {
