<#
.TITLE
    Get-AttackSurfaceReduction - Audit ASR rules and exclusions

.SYNOPSIS
    Audit ASR rules and exclusions

.DESCRIPTION
    Enumerates Attack Surface Reduction (ASR) rules via Get-MpPreference (AttackSurfaceReductionRules_Ids/Actions) and Defender exclusions. Reports audit vs block vs disabled.

.TAGS
    Security,ASR,Defender,Hardening
...
.EXAMPLE
    .\Get-AttackSurfaceReduction.ps1
    Runs with default targets.

.EXAMPLE
    .\Get-AttackSurfaceReduction.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Security,ASR,Defender,Hardening
...
$SolutionName = 'Get-AttackSurfaceReduction'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Audit ASR')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $pref = Get-MpPreference -ErrorAction SilentlyContinue
        if (-not $pref) { throw "Get-MpPreference unavailable" }
...
try {
    ...
    # Tailored display: ASR rules
    try {
        $pref = Get-MpPreference -ErrorAction SilentlyContinue
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Attack Surface Reduction Rules --" -ForegroundColor DarkGray
        if ($pref -and $pref.AttackSurfaceReductionRules_Ids) {
            for ($i=0; $i -lt $pref.AttackSurfaceReductionRules_Ids.Count; $i++) {
                ...
                $state = switch ($act) { 0 {'Disabled'} 1 {'Block'} 2 {'Audit'} 6 {'Warn'} default {"$act"} }
                [PSCustomObject]@{ Rule=$id.Substring(0,8); State=$state; GUID=$id } | Format-Table -AutoSize -Wrap | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
            }
        } else { Write-Log -Message "No ASR rules configured" -Level 'INFO' }
    } catch {
        Write-Log -Message "Could not query ASR: $($_.Exception.Message)" -Level 'WARNING'
    }
