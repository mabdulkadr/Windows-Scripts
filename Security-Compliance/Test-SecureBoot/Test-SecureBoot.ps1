<#
.TITLE
    Test-SecureBoot - Verify Secure Boot and TPM readiness

.SYNOPSIS
    Verify Secure Boot and TPM readiness

.DESCRIPTION
    Checks Secure Boot state (Confirm-SecureBootUEFI), TPM presence (Get-Tpm), and reports compliant vs not compliant for Conditional Access.

.TAGS
    Security,SecureBoot,TPM,Compliance
...
.EXAMPLE
    .\Test-SecureBoot.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Security,SecureBoot,TPM,Compliance
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Check SecureBoot')) {
        ...
        if ($null -eq $sb -and $null -eq $tpm) { throw "SecureBoot/TPM query failed" }
...
try {
    ...
    # Tailored display: SecureBoot + TPM
    try {
        $sb = try { Confirm-SecureBootUEFI -ErrorAction Stop } catch { "Unsupported/VM" }
        $tpm = try { Get-Tpm -ErrorAction SilentlyContinue } catch { $null }
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Secure Boot & TPM --" -ForegroundColor DarkGray
        [PSCustomObject]@{
            SecureBoot=$sb
            TPM_Present=(if ($tpm.TpmPresent) { $tpm.TpmPresent.ToString() } else { 'unknown' })
            TPM_Ready=(if ($tpm.TpmReady) { $tpm.TpmReady.ToString() } else { 'unknown' })
            TPM_Enabled=(if ($tpm.TpmEnabled) { $tpm.TpmEnabled.ToString() } else { 'unknown' })
        } | Format-List | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
    } catch {
        Write-Log -Message "Could not query SecureBoot/TPM: $($_.Exception.Message)" -Level 'WARNING'
    }
# Export professional HTML report
    try {
        $sb2 = try { Confirm-SecureBootUEFI -ErrorAction Stop } catch { $false }
        $tpm2 = try { Get-Tpm -ErrorAction Stop } catch { $null }
        $biosMode = try { (Get-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\SecureBoot\State' -Name UEFISecureBootEnabled -ErrorAction Stop).UEFISecureBootEnabled } catch { $null }
        $sbKpis = @(@{value=if($sb2){'Enabled'}else{'Disabled'}; label='SecureBoot'; color=if($sb2){'#24a148'}else{'#da1e28'}}, @{value=if($tpm2 -and $tpm2.TpmReady){'Ready'}else{'Not Ready'}; label='TPM'; color=if($tpm2 -and $tpm2.TpmReady){'#24a148'}else{'#f1c21b'}}, @{value=$env:COMPUTERNAME; label='Device'; color='#8a3ffc'}, @{value=$ok; label='Checks OK'; color='#c6c6c6'})
        # TPM detail table (every meaningful TPM property from Get-Tpm)
        $tpmRows = ''
        if ($tpm2) {
            $tpmFields = @(
                @{n='TpmPresent'; l='TPM Present'; ok=$tpm2.TpmPresent},
                @{n='TpmReady'; l='TPM Ready'; ok=$tpm2.TpmReady},
                @{n='TpmEnabled'; l='TPM Activated'; ok=$tpm2.TpmEnabled},
                @{n='TpmOwned'; l='TPM Owned'; ok=$tpm2.TpmOwned},
                @{n='TpmActivated'; l='TPM Enabled in Boot'; ok=$tpm2.TpmActivated},
                @{n='LockedOut'; l='Dictionary Attack Lockout'; ok=-not $tpm2.LockedOut},
                @{n='AutoProvisioning'; l='Auto Provisioning'; ok=$null}
            )
            $tpmRows = ($tpmFields | ForEach-Object {
                $val = $tpm2."$($_.n)"
                $badge = if ($_.n -eq 'AutoProvisioning') { $val = $tpm2.AutoProvisioning; ($val ? 'success' : 'neutral') }
                         elseif ($_.n -eq 'LockedOut') { if ($val) { 'success' } else { 'error' } }
                         else { if ($val) { 'success' } else { 'error' } }
                $disp = if ($null -eq $val) { 'unknown' } elseif ($val -is [bool]) { if ($val) { 'Yes' } else { 'No' } } else { "$val" }
                "<tr><td>$($_.l)</td><td><span class=`"badge $badge`">$disp</span></td></tr>"
            }) -join "`n"
        } else {
            $tpmRows = '<tr><td colspan="2" style="color:#8d8d8d">No TPM detected (VM or legacy BIOS)</td></tr>'
        }
        # TPM info table (manufacturer, version, etc.)
        $tpmInfoRows = ''
        if ($tpm2) {
            $tpmInfo = @(
                @{l='Manufacturer'; v=if ($tpm2.ManufacturerId) { "0x{0:X}" -f $tpm2.ManufacturerId } else { 'unknown' }},
                @{l='Manufacturer Name'; v=try { (Get-TpmEndorsementKeyInfo -ErrorAction SilentlyContinue).ManufacturerName } catch { 'unknown' }},
                @{l='Spec Version'; v=if ($tpm2.SpecVersion) { ($tpm2.SpecVersion -join ', ') } else { 'unknown' }},
                @{l='Manufacturer Version'; v=if ($null -ne $tpm2.ManufacturerVersion) { "$($tpm2.ManufacturerVersion)" } else { 'unknown' }},
                @{l='Firmware Version'; v=try { (Get-Tpm -ErrorAction SilentlyContinue).FirmwareVersion } catch { 'n/a' }},
                @{l='PCR0'; v=if ($tpm2.Pcr0) { $tpm2.Pcr0.ToString() } else { 'n/a' }},
                @{l='PCR7 (Secure Boot)'; v=if ($tpm2.Pcr7) { $tpm2.Pcr7.ToString() } else { 'n/a' }},
                @{l='Id'; v=if ($tpm2.Id) { $tpm2.Id.ToString().Substring(0, [Math]::Min(16, $tpm2.Id.ToString().Length)) + '...' } else { 'unknown' }}
            )
            $tpmInfoRows = ($tpmInfo | ForEach-Object { "<tr><td>$($_.l)</td><td><code>$([System.Net.WebUtility]::HtmlEncode($_.v))</code></td></tr>" }) -join "`n"
        }
        # Boot components table
        $bootRows = "<tr><td>UEFI Mode</td><td><span class=`"badge $(if($biosMode){'success'}else{'warning'})`">$(if($biosMode){'UEFI Native'}else{'Legacy/Unknown'})</span></td></tr><tr><td>Secure Boot (firmware)</td><td><span class=`"badge $(if($biosMode -eq 1){'success'}else{'warning'})`">$(if($biosMode -eq 1){'Enabled in firmware'}else{'Disabled/Unknown'})</span></td></tr><tr><td>Secure Boot (Confirm cmdlet)</td><td><span class=`"badge $(if($sb2){'success'}else{'error'})`">$(if($sb2){'Enabled'}else{'Disabled'})</span></td></tr><tr><td>UEFI Variable Services</td><td><span class=`"badge $(if($biosMode -ne $null){'success'}else{'neutral'})`">$(if($null -ne $biosMode){'Available'}else{'Not Available'})</span></td></tr>"
        $sbBody = "<div class=`"section-title`">Boot Integrity Audit</div><div class=`"grid-2`"><div class=`"card`"><h2>Boot Components</h2><table><thead><tr><th>Component</th><th>Status</th></tr></thead><tbody>$bootRows</tbody></table></div><div class=`"card`"><h2>TPM Health</h2><table><thead><tr><th>Check</th><th>Status</th></tr></thead><tbody>$tpmRows</tbody></table></div></div><div class=`"card`"><h2>TPM Hardware Identity</h2><table><thead><tr><th>Property</th><th>Value</th></tr></thead><tbody>$tpmInfoRows</tbody></table></div><div class=`"card`"><h2>Compliance Verdict</h2><p style=`"color:#c6c6c6`">Modern enterprise compliance requires both Secure Boot (UEFI firmware) AND an activated, owned TPM with current measurements. BitLocker relies on TPM PCRs; Measured Boot relies on PCR0/PCR7. Both UEFI native mode and Secure Boot must be enabled.</p></div>"
        $sb2Base = if ($PSScriptRoot) { $PSScriptRoot } elseif ($PSCommandPath) { Split-Path -Parent $PSCommandPath } else { (Get-Location).Path }
        $sbHtml = Join-Path $sb2Base "Reports\SecureBoot_$(Get-Date -Format yyyyMMdd_HHmmss).html"
        Export-ProfessionalHtmlReport -OutputPath $sbHtml -Title 'Secure Boot & TPM' -Subtitle "Boot integrity and TPM hardware report" -Kpis $sbKpis -Body $sbBody -ReportName 'Secure Boot & TPM' -Version '2.1.0'
        Write-Log -Message "HTML report: $sbHtml" -Level 'SUCCESS'
