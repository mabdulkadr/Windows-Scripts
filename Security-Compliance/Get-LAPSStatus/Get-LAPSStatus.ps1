<#
.TITLE
    Get-LAPSStatus - Audit Windows LAPS configuration and password age

.SYNOPSIS
    Audit Windows LAPS configuration and password age

.DESCRIPTION
    Checks LAPS policy (BackupDirectory, PasswordAge, AutomaticAccount), reads LAPS password expiry from AD/Entra where available, and reports compliance. No changes made.

.TAGS
    Security,LAPS,Identity

.PLATFORM
    Windows 10/11/Server 2019+

.PERMISSIONS
    Standard user for local policy; AD read for expiry
...
.CHANGELOG
    2.1.0 (2026-08-31)
    - Initial release - Phase 1 expansion (tailored per-script output)
...
.EXAMPLE
    .\Get-LAPSStatus.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Security,LAPS,Identity
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Query LAPS')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $lapsPol = Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\LAPS' -ErrorAction SilentlyContinue
        $backupDir = $lapsPol.BackupDirectory
        if ($null -eq $backupDir) { $backupDir = (Get-ItemProperty -Path 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\LAPS' -Name BackupDirectory -ErrorAction SilentlyContinue).BackupDirectory }
...
# Tailored display: LAPS policy
    try {
        $keys = @('HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\LAPS','HKLM:\SOFTWARE\Policies\Microsoft\Windows\LAPS')
        $rows = foreach ($k in $keys) {
            if (Test-Path $k) {
                $p = Get-ItemProperty -Path $k -ErrorAction SilentlyContinue
                [PSCustomObject]@{
                    Key=$k
                    BackupDirectory=(if ($p.BackupDirectory) { $p.BackupDirectory } else { 'NotSet' })
                    PasswordAgeDays=(if ($null -ne $p.PasswordAgeDays) { "$($p.PasswordAgeDays) days" } else { 'NotSet' })
                    PasswordAgeDays_Raw=(if ($null -ne $p.PasswordAgeDays) { $p.PasswordAgeDays } else { 0 })
                    PasswordComplexity=(if ($p.PasswordComplexity) { $p.PasswordComplexity } else { 'NotSet' })
                    PasswordLength=(if ($null -ne $p.PasswordLength) { "$($p.PasswordLength) chars" } else { 'NotSet' })
                    AutomaticAccountName=(if ($p.AutomaticAccountName) { $p.AutomaticAccountName } else { 'NotSet' })
                    AdministratorAccountName=(if ($p.AdministratorAccountName) { $p.AdministratorAccountName } else { 'NotSet' })
                    PostAuthenticationResetDelay=(if ($null -ne $p.PostAuthenticationResetDelay) { "$($p.PostAuthenticationResetDelay) hours" } else { 'NotSet' })
                    PostAuthenticationActions=(if ($p.PostAuthenticationActions) { $p.PostAuthenticationActions } else { 'NotSet' })
                    ResetPassword=(if ($null -ne $p.ResetPassword) { "=$($p.ResetPassword)" } else { 'NotSet' })
                    EncryptionEnabled=(if ($null -ne $p.EncryptionEnabled) { if ($p.EncryptionEnabled) { 'Yes' } else { 'No' } } else { 'NotSet' })
                }
            }
        }
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Windows LAPS Policy --" -ForegroundColor DarkGray
        if ($rows) { $rows | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
        else { Write-Log -Message "No LAPS policy keys found - LAPS not configured" -Level 'WARNING' }
    } catch {
        Write-Log -Message "Could not query LAPS: $($_.Exception.Message)" -Level 'WARNING'
    }
    # Export professional HTML report
    try {
        $lapsStatus = if ($rows) { 'Configured' } else { 'Not Found' }
        $lapsPwdAge = if ($rows -and $rows[0].PasswordAgeDays_Raw -gt 0) { "$($rows[0].PasswordAgeDays_Raw) days" } else { 'Not Set' }
        $lapsBackup = if ($rows) { $rows[0].BackupDirectory } else { 'Not Set' }
        $lapsKpis = @(@{value=$lapsStatus; label='LAPS State'; color=if($rows){'#24a148'}else{'#da1e28'}}, @{value=$lapsPwdAge; label='Password Age'; color='#0f62fe'}, @{value=$lapsBackup; label='Backup Directory'; color='#8a3ffc'}, @{value=$env:COMPUTERNAME; label='Device'; color='#f1c21b'})
        # LAPS properties detail table (every property from $rows)
        $lapsTable2 = if ($rows) {
            $lapsH2 = ($rows[0].PSObject.Properties | Where-Object { $_.Name -notlike '*_Raw' } | ForEach-Object { "<th>$($_.Name)</th>" }) -join ""
            $lapsB2 = ($rows | ForEach-Object {
                $lapsCs = ($_.PSObject.Properties | Where-Object { $_.Name -notlike '*_Raw' } | ForEach-Object {
                    $val = "$($_.Value)"
                    $encoded = [System.Net.WebUtility]::HtmlEncode($val)
                    if ($_.Name -eq 'Key') { "<td><code>$encoded</code></td>" }
                    else { "<td>$encoded</td>" }
                }) -join ""
                "<tr>$lapsCs</tr>"
            }) -join "`n"
            "<table><thead><tr>$lapsH2</tr></thead><tbody>$lapsB2</tbody></table>"
        } else { "<p style=`"color:#8d8d8d;padding:16px`">No LAPS policy keys found — LAPS not configured (same as console)</p>" }
        # Backup directory analysis
        $lapsBackupAnalysis = ''
        if ($rows -and $rows[0].BackupDirectory -ne 'NotSet') {
            $bd = $rows[0].BackupDirectory
            $bdLabel = switch ($bd) { 0 { 'Disabled - password not backed up' } 1 { 'Azure AD only' } 2 { 'Active Directory' } default { "Custom value: $bd" } }
            $lapsBackupAnalysis = "<div class=`"card`"><h2>Backup Directory Interpretation</h2><table><thead><tr><th>Code</th><th>Description</th></tr></thead><tbody><tr><td><code>$([System.Net.WebUtility]::HtmlEncode($bd))</code></td><td>$bdLabel</td></tr></tbody></table></div>"
        }
        # Encryption analysis
        $lapsEncAnalysis = ''
        if ($rows -and $rows[0].EncryptionEnabled -ne 'NotSet') {
            $enc = $rows[0].EncryptionEnabled
            $encBadge = if ($enc -eq 'Yes') { 'success' } else { 'error' }
            $encDesc = if ($enc -eq 'Yes') { 'Password encryption enabled (Windows LAPS modern)' } else { 'Password encryption disabled — legacy mode' }
            $lapsEncAnalysis = "<div class=`"card`"><h2>Encryption Status</h2><table><thead><tr><th>Check</th><th>Status</th></tr></thead><tbody><tr><td>Password Encryption</td><td><span class=`"badge $encBadge`">$enc</span> — $encDesc</td></tr></tbody></table></div>"
        }
        $lapsBody = "<div class=`"section-title`">LAPS Configuration</div><div class=`"card`"><h2>Registry Policy ($($rows.Count) policies)</h2><p style=`"color:#c6c6c6`">All LAPS-related registry values from <code>HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\LAPS</code> and <code>HKLM:\SOFTWARE\Policies\Microsoft\Windows\LAPS</code>.</p>$lapsTable2</div><div class=`"grid-2`">$lapsBackupAnalysis$lapsEncAnalysis</div>"
        $lapsBase = if ($PSScriptRoot) { $PSScriptRoot } elseif ($PSCommandPath) { Split-Path -Parent $PSCommandPath } else { (Get-Location).Path }
        $lapsHtml = Join-Path $lapsBase "Reports\LAPSStatus_$(Get-Date -Format yyyyMMdd_HHmmss).html"
        Export-ProfessionalHtmlReport -OutputPath $lapsHtml -Title 'LAPS Status' -Subtitle "Windows Local Admin Password Solution configuration report" -Kpis $lapsKpis -Body $lapsBody -ReportName 'LAPS Status' -Version '2.1.0'
        Write-Log -Message "HTML report: $lapsHtml" -Level 'SUCCESS'
