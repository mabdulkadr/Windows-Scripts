    ...
    try {
        ...
        $rows = foreach ($k in $keys) {
            if (Test-Path $k) {
                ...
                [PSCustomObject]@{ Key=$k; BackupDirectory=(if ($p.BackupDirectory) { $p.BackupDirectory } else { 'NotSet' }); PasswordAgeDays=(if ($p.PasswordAgeDays) { $p.PasswordAgeDays } else { 'NotSet' }); AutomaticAccountName=(if ($p.AutomaticAccountName) { $p.AutomaticAccountName } else { 'NotSet' }) }
            }
        }
