<#
.TITLE
    Get-BitLockerStatus - Audit BitLocker encryption status

.SYNOPSIS
    Audit BitLocker encryption status

.DESCRIPTION
    Queries Get-BitLockerVolume for all fixed volumes and reports ProtectionStatus, EncryptionPercentage, and KeyProtector types. Use for compliance drift checks.

.TAGS
    Security,BitLocker,Encryption

.PLATFORM
    Windows 10/11/Server 2019+

.PERMISSIONS
    Standard user for status; Administrator for protector details
...
.EXAMPLE
    .\Get-BitLockerStatus.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Security,BitLocker,Encryption
...
$SolutionName = 'Get-BitLockerStatus'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Check BitLocker')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $vols = Get-BitLockerVolume -ErrorAction SilentlyContinue
        if (-not $vols) { throw "No BitLocker volumes found or BitLocker not available" }
...
try {
    ...
    # Tailored display: BitLocker volumes
    try {
        $vols = Get-BitLockerVolume -ErrorAction SilentlyContinue | Select-Object MountPoint, VolumeType, ProtectionStatus, EncryptionPercentage, EncryptionMethod, KeyProtector
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- BitLocker Volumes --" -ForegroundColor DarkGray
        if ($vols) {
            $vols | ForEach-Object {
                $kp = ($_.KeyProtector | ForEach-Object { $_.KeyProtectorType }) -join ', '
                [PSCustomObject]@{ MountPoint=$_.MountPoint; Type=$_.VolumeType; Protection=$_.ProtectionStatus; Encrypted="$($_.EncryptionPercentage)%"; Method=$_.EncryptionMethod; Protectors=$kp }
            } | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
        } else { Write-Log -Message "No BitLocker volumes found" -Level 'WARNING' }
    } catch {
        Write-Log -Message "Could not query BitLocker: $($_.Exception.Message)" -Level 'WARNING'
    }
    # Export professional HTML report
    try {
        $blVols = Get-BitLockerVolume -ErrorAction SilentlyContinue | Select-Object MountPoint, ProtectionStatus, EncryptionPercentage -First 10
        $blCount = @($blVols).Count; $blOn = @($blVols | Where-Object ProtectionStatus -eq 'On').Count
        $blKpis = @(@{value=$blCount; label='Volumes'; color='#0f62fe'}, @{value=$blOn; label='Encrypted'; color='#24a148'}, @{value=($blCount-$blOn); label='Unencrypted'; color='#da1e28'}, @{value=$env:COMPUTERNAME; label='Device'; color='#8a3ffc'})
        $blRows = if($blVols){ $h="<th>Mount</th><th>Protection</th><th>Encrypted %</th>"; $b=($blVols | ForEach-Object { $bg=if($_.ProtectionStatus -eq 'On'){'success'}else{'error'}; "<tr><td>$($_.MountPoint)</td><td><span class=`"badge $bg`">$($_.ProtectionStatus)</span></td><td>$($_.EncryptionPercentage)%</td></tr>" }) -join "`n"; "<table><thead><tr>$h</tr></thead><tbody>$b</tbody></table>" } else { "<p style=`"color:#8d8d8d;padding:16px`">No BitLocker volumes</p>" }
        $blCharts = @{donut=@(@{label='Encrypted';value=$blOn;color='#24a148'},@{label='Unencrypted';value=($blCount-$blOn);color='#da1e28'})} | ConvertTo-Json -Compress
        $blBody = "<div class=`"section-title`">BitLocker Volume Status</div><div class=`"card`"><h2>Drive Encryption</h2>$blRows</div>"
        $blBase = if ($PSScriptRoot) { $PSScriptRoot } elseif ($PSCommandPath) { Split-Path -Parent $PSCommandPath } else { (Get-Location).Path }
        $blHtml = Join-Path $blBase "Reports\BitLocker_$(Get-Date -Format yyyyMMdd_HHmmss).html"
        Export-ProfessionalHtmlReport -OutputPath $blHtml -Title 'BitLocker Status' -Subtitle "Volume encryption status" -Kpis $blKpis -Body $blBody -Charts $blCharts -ReportName 'BitLocker Status' -Version '2.1.0'
        Write-Log -Message "HTML report: $blHtml" -Level 'SUCCESS'
