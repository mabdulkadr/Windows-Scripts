<#
.TITLE
    Remove-RevokedRootCertificate - Remove revoked certificates from the LocalMachine Root store

.SYNOPSIS
    Remove revoked certificates from the LocalMachine Root store

.DESCRIPTION
    Verifies every cert in LocalMachine\Root, removes revoked ones, and logs the count.
...
.EXAMPLE
    .\Remove-RevokedRootCertificate.ps1
    Runs with default targets.

.EXAMPLE
    .\Remove-RevokedRootCertificate.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Security,Certificate
...
$SolutionName = 'Remove-RevokedRootCertificate'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Remove revoked')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $null = Get-ChildItem Cert:\LocalMachine\Root -ErrorAction SilentlyContinue | Select-Object -First 1
...
try {
    ...
    # Tailored display: root store before
    try {
        $roots = Get-ChildItem -Path Cert:\LocalMachine\Root -ErrorAction SilentlyContinue | Select-Object Subject, Thumbprint -First 5
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Root Store Sample (top 5) --" -ForegroundColor DarkGray
        if ($roots) { $roots | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
    } catch {
        Write-Log -Message "Could not list root certs: $($_.Exception.Message)" -Level 'WARNING'
    }
    # Export professional HTML report
    try {
        $rootCerts = Get-ChildItem -Path Cert:\LocalMachine\Root -ErrorAction SilentlyContinue | Select-Object Subject, NotAfter -First 20
        $rootTotal = @($rootCerts).Count; $rootExp = @($rootCerts | Where-Object { $_.NotAfter -lt (Get-Date) }).Count; $rootHlth = $rootTotal - $rootExp
        $rootKpis = @(@{value=$rootTotal; label='Root Certs'; color='#0f62fe'}, @{value=$rootHlth; label='Valid'; color='#24a148'}, @{value=$rootExp; label='Expired'; color='#da1e28'}, @{value=$env:COMPUTERNAME; label='Device'; color='#8a3ffc'})
        $rootRows = if($rootCerts){ $h="<th>Subject</th><th>Expiry</th><th>Status</th>"; $b=($rootCerts | Select-Object -First 20 | ForEach-Object { $bg=if($_.NotAfter -lt (Get-Date)){'error'}elseif($_.NotAfter -lt (Get-Date).AddDays(30)){'warning'}else{'success'}; $st=if($_.NotAfter -lt (Get-Date)){'Expired'}else{'Valid'}; "<tr><td>$([System.Net.WebUtility]::HtmlEncode($_.Subject))</td><td>$($_.NotAfter.ToString('yyyy-MM-dd'))</td><td><span class=`"badge $bg`">$st</span></td></tr>" }) -join "`n"; "<table><thead><tr>$h</tr></thead><tbody>$b</tbody></table>" } else { "<p style=`"color:#8d8d8d;padding:16px`">No root certs</p>" }
        $rootCharts = @{donut=@(@{label='Valid';value=$rootHlth;color='#24a148'},@{label='Expired';value=$rootExp;color='#da1e28'})} | ConvertTo-Json -Compress
        $rootBody = "<div class=`"section-title`">Root Certificate Store</div><div class=`"card`"><h2>Root Certificates</h2>$rootRows</div>"
        $rootBase = if ($PSScriptRoot) { $PSScriptRoot } elseif ($PSCommandPath) { Split-Path -Parent $PSCommandPath } else { (Get-Location).Path }
        $rootHtml = Join-Path $rootBase "Reports\RootCerts_$(Get-Date -Format yyyyMMdd_HHmmss).html"
        Export-ProfessionalHtmlReport -OutputPath $rootHtml -Title 'Root Certificates' -Subtitle "Root CA certificate inventory" -Kpis $rootKpis -Body $rootBody -Charts $rootCharts -ReportName 'Root Certificates' -Version '2.1.0'
        Write-Log -Message "HTML report: $rootHtml" -Level 'SUCCESS'
