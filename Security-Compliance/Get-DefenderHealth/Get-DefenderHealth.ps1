<#
.TITLE
    Get-DefenderHealth - Audit Microsoft Defender AV health

.SYNOPSIS
    Audit Microsoft Defender AV health

.DESCRIPTION
    Checks WinDefend service, Get-MpComputerStatus fields (AntivirusEnabled, RealTimeProtection, SignatureAge, QuickScanAge), and reports healthy vs degraded.
...
.EXAMPLE
    .\Get-DefenderHealth.ps1
    Runs with default targets.

.EXAMPLE
    .\Get-DefenderHealth.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Security,Defender,Antivirus
...
$SolutionName = 'Get-DefenderHealth'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Check Defender')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $svc = Get-Service -Name WinDefend -ErrorAction SilentlyContinue
        if (-not $svc) { throw "WinDefend service not found - Defender not installed" }
        $mp = Get-MpComputerStatus -ErrorAction SilentlyContinue
        if (-not $mp) { throw "Get-MpComputerStatus unavailable" }
...
try {
    ...
    # Tailored display: Defender health
    try {
        $svc = Get-Service -Name WinDefend -ErrorAction SilentlyContinue
        $mp = Get-MpComputerStatus -ErrorAction SilentlyContinue
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Microsoft Defender Health --" -ForegroundColor DarkGray
        if ($svc -and $mp) {
            $issues = @()
            if ($svc.Status -ne 'Running') { $issues += 'ServiceNotRunning' }
            if (-not $mp.AntivirusEnabled) { $issues += 'AntivirusDisabled' }
            if (-not $mp.RealTimeProtectionEnabled) { $issues += 'RealTimeDisabled' }
            if ($mp.AntivirusSignatureAge -gt 7) { $issues += "Signatures:$($mp.AntivirusSignatureAge)d" }
            if ($mp.QuickScanAge -gt 14) { $issues += "QuickScan:$($mp.QuickScanAge)d" }
            [PSCustomObject]@{
                Service=$svc.Status
                AntivirusEnabled=$mp.AntivirusEnabled
                RealTime=$mp.RealTimeProtectionEnabled
                SigAgeDays=$mp.AntivirusSignatureAge
                QuickScanDays=$mp.QuickScanAge
                Issues= if ($issues) { $issues -join ', ' } else { 'None' }
                Healthy=($issues.Count -eq 0)
            } | Format-List | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor $(if ($issues.Count -eq 0) { 'Green' } else { 'Yellow' }) }
        } else { Write-Log -Message "Defender status unavailable" -Level 'WARNING' }
    } catch {
        Write-Log -Message "Could not query Defender: $($_.Exception.Message)" -Level 'WARNING'
    }
    # Export professional HTML report
    try {
        $defSvc = Get-Service -Name WinDefend -ErrorAction SilentlyContinue
        $defMp = try { Get-MpComputerStatus -ErrorAction Stop } catch { $null }
        $defKpis = @(@{value=if($defSvc){$defSvc.Status}else{'Not Found'}; label='Service'; color='#0f62fe'}, @{value=if($defMp -and $defMp.AntivirusEnabled){'Enabled'}else{'Disabled'}; label='Antivirus'; color='#24a148'}, @{value=if($defMp){"$($defMp.AntivirusSignatureAge) days"}else{'N/A'}; label='Signature Age'; color='#8a3ffc'}, @{value=if($defMp){"$($defMp.QuickScanAge) days"}else{'N/A'}; label='Quick Scan'; color='#f1c21b'})
        $defRows = if($defMp){ "<table><thead><tr><th>Field</th><th>Value</th></tr></thead><tbody><tr><td>Service</td><td><span class=`"badge $(if($defSvc.Status -eq 'Running'){'success'}else{'error'})`">$($defSvc.Status)</span></td></tr><tr><td>Antivirus Enabled</td><td>$($defMp.AntivirusEnabled)</td></tr><tr><td>Real-Time Protection</td><td>$($defMp.RealTimeProtectionEnabled)</td></tr><tr><td>Signature Age</td><td>$($defMp.AntivirusSignatureAge) days</td></tr><tr><td>Quick Scan Age</td><td>$($defMp.QuickScanAge) days</td></tr><tr><td>Full Scan Age</td><td>$($defMp.FullScanAge) days</td></tr></tbody></table>" } else { "<p style=`"color:#8d8d8d;padding:16px`">Get-MpComputerStatus unavailable</p>" }
        $defBody = "<div class=`"section-title`">Defender Health</div><div class=`"card`"><h2>Microsoft Defender Status</h2>$defRows</div>"
        $defBase = if ($PSScriptRoot) { $PSScriptRoot } elseif ($PSCommandPath) { Split-Path -Parent $PSCommandPath } else { (Get-Location).Path }
        $defHtml = Join-Path $defBase "Reports\DefenderHealth_$(Get-Date -Format yyyyMMdd_HHmmss).html"
        Export-ProfessionalHtmlReport -OutputPath $defHtml -Title 'Defender Health' -Subtitle "Microsoft Defender health audit" -Kpis $defKpis -Body $defBody -ReportName 'Defender Health' -Version '2.1.0'
        Write-Log -Message "HTML report: $defHtml" -Level 'SUCCESS'
