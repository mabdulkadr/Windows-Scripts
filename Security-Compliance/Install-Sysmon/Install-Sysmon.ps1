...
try {
    ...
    # Tailored display: Sysmon status
    try {
        ...
        $hash = if (Test-Path $cfg) { (Get-FileHash -Path $cfg -Algorithm SHA256 -ErrorAction SilentlyContinue).Hash.Substring(0,12) } else { "not-found" }
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Sysmon --" -ForegroundColor DarkGray
        [PSCustomObject]@{ Service=(if ($svc.Status) { $svc.Status } else { 'NotInstalled' }); ConfigHash=$hash; ConfigPath=$cfg } | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
    } catch {
