...
try {
    ...
    # Tailored display: w32time
    try {
        $svc = Get-Service -Name W32Time -ErrorAction SilentlyContinue
        $time = w32tm /query /status 2>&1 | Out-String
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- W32Time Status --" -ForegroundColor DarkGray
        if ($svc) { $svc | Select-Object Name, Status | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
        Write-Host ($time | Select-Object -First 20) -ForegroundColor Gray
    } catch {
