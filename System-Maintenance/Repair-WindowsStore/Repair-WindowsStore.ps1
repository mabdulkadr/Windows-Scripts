...
try {
    ...
    try {
        ...
        if ($pkg) { $pkg | Select-Object Name, Version | Format-List | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
        else { Write-Log -Message "Windows Store package not found" -Level 'WARNING' }
    } catch {
