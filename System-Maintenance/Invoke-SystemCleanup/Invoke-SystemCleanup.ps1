...
.DESCRIPTION
    Launches cleanmgr with all categories selected for the system drive.
...
.EXAMPLE
    .\Invoke-SystemCleanup.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - System,Cleanup
...
$SolutionName = 'Invoke-SystemCleanup'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Run cleanup')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        & cleanmgr.exe /d C /verylowdisk -ErrorAction SilentlyContinue | Out-Null
...
try {
    ...
    # Tailored display: comprehensive disk cleanup diagnostics
    try {
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Disk Space (before cleanup) --" -ForegroundColor DarkGray
        $drive = Get-PSDrive C -ErrorAction SilentlyContinue
        $freeBefore = $drive.Free
        $usedBefore = $drive.Used
        $total = $freeBefore + $usedBefore
        Write-Host ("  C: Free: {0} GB / Total: {1} GB ({2}% free)" -f [math]::Round($freeBefore/1GB,1), [math]::Round($total/1GB,1), [math]::Round(($freeBefore/$total)*100,1)) -ForegroundColor Cyan
        Write-Host ("  Used: {0} GB" -f [math]::Round($usedBefore/1GB,1)) -ForegroundColor Gray

        Write-Host "  -- Cleanup Categories (StateFlags) --" -ForegroundColor DarkGray
        $volumeCaches = Get-ChildItem -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\VolumeCaches' -ErrorAction SilentlyContinue | Select-Object -First 10
        if ($volumeCaches) {
            $volumeCaches | ForEach-Object {
                $state = (Get-ItemProperty -Path $_.PSPath -Name StateFlags0001 -ErrorAction SilentlyContinue).StateFlags0001
                $enabled = if ($state -eq 2) { "Enabled" } else { "Disabled" }
                Write-Host ("  {0,-35} : {1}" -f $_.PSChildName, $enabled) -ForegroundColor $(if ($enabled -eq "Enabled") {'Green'} else {'Gray'})
            }
            Write-Host ("  Total categories: {0} (Sagerun will clean Enabled ones)" -f $volumeCaches.Count) -ForegroundColor Gray
        }

        Write-Host "  -- Temp Locations (pre-cleanup) --" -ForegroundColor DarkGray
        ...
        foreach ($tp in $tempPaths) {
            if (Test-Path $tp) {
                ...
                Write-Host ("  {0,-35} : {1,5} files | {2,6} MB" -f $tp, $count, $size) -ForegroundColor Gray
            }
        }

        Write-Host "  -- Cleanup Action --" -ForegroundColor DarkGray
        Write-Host "  Command: cleanmgr /d C /verylowdisk  and  cleanmgr /sagerun:1" -ForegroundColor Gray
        Write-Host "  Also: Dism /Online /Cleanup-Image /StartComponentCleanup" -ForegroundColor Gray
        $freeAfter = (Get-PSDrive C -ErrorAction SilentlyContinue).Free
        $freed = [math]::Round(($freeAfter - $freeBefore)/1MB,1)
        Write-Host ("  Freed (estimated): {0} MB (before vs after)" -f $freed) -ForegroundColor $(if ($freed -gt 0) {'Green'} else {'Yellow'})
        Write-Host "  Note: Full cleanup may require reboot to clear locked files" -ForegroundColor Gray

    } catch {
        Write-Log -Message "Could not check disk: $($_.Exception.Message)" -Level 'WARNING'
    }
