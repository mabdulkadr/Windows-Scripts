...
try {
    ...
    # Tailored display: cleanmgr
    try {
        $freeBefore = (Get-PSDrive C -ErrorAction SilentlyContinue).Free
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Disk Cleanup --" -ForegroundColor DarkGray
        Write-Host "  Free on C: before: $([math]::Round($freeBefore/1GB,2)) GB" -ForegroundColor Cyan
    } catch {
