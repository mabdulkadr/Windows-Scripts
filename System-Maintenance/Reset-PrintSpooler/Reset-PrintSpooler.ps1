<#
.TITLE
    Reset-PrintSpooler - Reset the print spooler service

.SYNOPSIS
    Reset the print spooler service

.DESCRIPTION
    Stops the Spooler, clears spool files, and restarts the service.
...
.EXAMPLE
    .\Reset-PrintSpooler.ps1 -TargetName "Server01","Server02"
...
$SolutionName = 'Reset-PrintSpooler'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Reset spooler')) {
...
try {
    ...
    # Tailored display: comprehensive spooler diagnostics (after reset)
    try {
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Print Spooler Service (after reset) --" -ForegroundColor DarkGray
        $svc = Get-Service -Name Spooler -ErrorAction SilentlyContinue
        if ($svc) {
            $wmi = Get-CimInstance -ClassName Win32_Service -Filter "Name='Spooler'" -ErrorAction SilentlyContinue
            [PSCustomObject]@{
                Service   = $svc.Name
                Status    = $svc.Status
        ...
        if (Test-Path $spoolFolder) {
            $files = Get-ChildItem -Path $spoolFolder -ErrorAction SilentlyContinue
            $count = ($files | Measure-Object).Count
            $size = [math]::Round((($files | Measure-Object -Property Length -Sum).Sum / 1KB),1)
            Write-Host ("  Path: {0}" -f $spoolFolder) -ForegroundColor Gray
            Write-Host ("  Files: {0} | Size: {1} KB" -f $count, $size) -ForegroundColor $(if ($count -eq 0) {'Green'} else {'Yellow'})
            if ($files) { $files | Select-Object Name, Length, LastWriteTime -First 3 | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Gray } }
            else { Write-Host "  Spool folder is empty - queue clear" -ForegroundColor Green }
        }

        Write-Host "  -- Printers --" -ForegroundColor DarkGray
        $printers = Get-Printer -ErrorAction SilentlyContinue | Select-Object -First 5
        if ($printers) {
            $printers | ForEach-Object {
                $jobs = Get-PrintJob -PrinterName $_.Name -ErrorAction SilentlyContinue | Measure-Object | Select-Object -ExpandProperty Count
                [PSCustomObject]@{ Printer=$_.Name; Driver=$_.DriverName; Jobs=$jobs; Shared=$_.Shared }
            } | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Gray }
        } else { Write-Host "  No printers installed" -ForegroundColor Yellow }

        Write-Host "  -- Reset Summary --" -ForegroundColor DarkGray
        $status = if ($svc -and $svc.Status -eq 'Running' -and ((Get-ChildItem -Path $spoolFolder -ErrorAction SilentlyContinue | Measure-Object).Count -eq 0)) { "SUCCESS - spooler running and queue empty" } elseif ($svc -and $svc.Status -eq 'Running') { "PARTIAL - spooler running but spool files remain" } else { "FAILED - check service" }
        $color = if ($status -match "SUCCESS") {'Green'} elseif ($status -match "PARTIAL") {'Yellow'} else {'Red'}
        Write-Host "  $status" -ForegroundColor $color
        Write-Host "  Verify: Get-Service Spooler | Format-List ; Get-PrintJob | Measure-Object" -ForegroundColor Gray

    } catch {
        Write-Log -Message "Could not query spooler: $($_.Exception.Message)" -Level 'WARNING'
    }
