...
try {
    ...
    # Tailored display: print jobs
    try {
        $jobs = Get-PrintJob -ErrorAction SilentlyContinue | Select-Object PrinterName, JobStatus, DocumentName -First 10
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Print Queue --" -ForegroundColor DarkGray
        if ($jobs) { $jobs | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Yellow } }
        else { Write-Host "  No print jobs in queue" -ForegroundColor Green }
    } catch {
