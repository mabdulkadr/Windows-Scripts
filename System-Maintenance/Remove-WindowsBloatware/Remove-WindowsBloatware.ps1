...
try {
    ...
    # Tailored display: bloatware
    try {
        $bloat = @('Microsoft.Xbox*','Microsoft.Zune*','Microsoft.Bing*')
        $found = foreach ($pat in $bloat) { Get-AppxPackage -Name $pat -ErrorAction SilentlyContinue | Select-Object -First 2 }
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Bloatware Sample --" -ForegroundColor DarkGray
        if ($found) { $found | Select-Object Name, Version | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Yellow } }
        else { Write-Host "  No matching bloatware found" -ForegroundColor Green }
    } catch {
