<#
.TITLE
    Remove-WindowsBloatware - Remove selected UWP bloatware apps

.SYNOPSIS
    Remove selected UWP bloatware apps

.DESCRIPTION
    Uninstalls a curated list of pre-installed UWP apps for the current user.
...
.EXAMPLE
    .\Remove-WindowsBloatware.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - System,Debloat
...
$SolutionName = 'Remove-WindowsBloatware'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Remove bloatware')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        Get-AppxPackage -Name 'Microsoft.BingNews' -AllUsers -ErrorAction SilentlyContinue | Remove-AppxPackage -ErrorAction SilentlyContinue | Out-Null
...
try {
    ...
    # Tailored display: comprehensive bloatware audit
    try {
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Bloatware Audit (curated list) --" -ForegroundColor DarkGray
        $bloatList = @(
            @{ Pat='Microsoft.BingNews'; Desc='Bing News' },
            @{ Pat='Microsoft.BingWeather'; Desc='Bing Weather' },
            @{ Pat='Microsoft.GetHelp'; Desc='Get Help' },
            @{ Pat='Microsoft.Getstarted'; Desc='Get Started' },
            @{ Pat='Microsoft.MicrosoftOfficeHub'; Desc='Office Hub' },
            @{ Pat='Microsoft.MicrosoftSolitaireCollection'; Desc='Solitaire' },
            @{ Pat='Microsoft.People'; Desc='People' },
            @{ Pat='Microsoft.SkypeApp'; Desc='Skype' },
            @{ Pat='Microsoft.WindowsFeedbackHub'; Desc='Feedback Hub' },
            @{ Pat='Microsoft.Xbox*'; Desc='Xbox' },
            @{ Pat='Microsoft.ZuneMusic'; Desc='Groove Music' },
            @{ Pat='Microsoft.ZuneVideo'; Desc='Movies & TV' }
        )
        $found = @()
        $notFound = 0
        foreach ($b in $bloatList) {
            $pkg = Get-AppxPackage -Name $b.Pat -AllUsers -ErrorAction SilentlyContinue | Select-Object -First 1
            if ($pkg) {
                $found += [PSCustomObject]@{ Name=$pkg.Name; Version=$pkg.Version; Description=$b.Desc; Status="Installed" }
            } else {
                $notFound++
            }
        }
        if ($found) {
            $found | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Yellow }
            Write-Host ("  Found: {0} installed | Not found (already removed): {1} | Total curated: {2}" -f $found.Count, $notFound, $bloatList.Count) -ForegroundColor Cyan
        } else {
            Write-Host "  No bloatware from curated list found - already clean!" -ForegroundColor Green
        }

        Write-Host "  -- Provisioned Packages (for new users) --" -ForegroundColor DarkGray
        $prov = Get-AppxProvisionedPackage -Online -ErrorAction SilentlyContinue | Where-Object { $_.DisplayName -like "Microsoft.Bing*" -or $_.DisplayName -like "Microsoft.Xbox*" } | Select-Object -First 5
        if ($prov) {
            $prov | Select-Object DisplayName, Version | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Gray }
            Write-Host "  Tip: Remove provisioned to prevent reinstall for new users: Remove-AppxProvisionedPackage" -ForegroundColor Yellow
        } else { Write-Host "  No provisioned bloatware" -ForegroundColor Green }

        Write-Host "  -- Disk Impact --" -ForegroundColor DarkGray
        $appxSize = try { (Get-AppxPackage -ErrorAction SilentlyContinue | Measure-Object).Count } catch { 0 }
        Write-Host "  Total AppX packages: $appxSize" -ForegroundColor Gray
        Write-Host "  Action: This script removes current user bloatware; add -AllUsers for system-wide" -ForegroundColor Gray
        Write-Host "  Verify: Get-AppxPackage | Where Name -like 'Microsoft.Bing*'" -ForegroundColor Gray

    } catch {
        Write-Log -Message "Could not query bloatware: $($_.Exception.Message)" -Level 'WARNING'
    }
