<#
.TITLE
    Get-USBDeviceHistory - Audit USB device history

.SYNOPSIS
    Audit USB device history

.DESCRIPTION
    Enumerates USB devices from registry (USBSTOR) and PnP (Get-PnpDevice), reports friendly name, last connected, and serial. No changes made.
...
.EXAMPLE
    .\Get-USBDeviceHistory.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - System,USB,Audit
...
$SolutionName = 'Get-USBDeviceHistory'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Audit USB')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $usbs = Get-ChildItem -Path 'HKLM:\SYSTEM\CurrentControlSet\Enum\USBSTOR' -ErrorAction SilentlyContinue
        if (-not $usbs) {
            $usbs = Get-PnpDevice -Class USB -ErrorAction SilentlyContinue
        }
        if (-not $usbs) { throw "No USB history found" }
...
try {
    ...
    # Tailored display: USB history
    try {
        $usbs = Get-ChildItem -Path 'HKLM:\SYSTEM\CurrentControlSet\Enum\USBSTOR' -ErrorAction SilentlyContinue | Select-Object -First 10
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- USB Device History (USBSTOR, top 10) --" -ForegroundColor DarkGray
        if ($usbs) {
            $usbs | ForEach-Object {
                $name = $_.PSChildName
                $props = Get-ItemProperty -Path $_.PSPath -ErrorAction SilentlyContinue
                $friendlyName = if ($props.FriendlyName) { $props.FriendlyName } else { $name }
                [PSCustomObject]@{ Device=$name; FriendlyName=$friendlyName }
            } | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
        } else { Write-Log -Message "No USBSTOR history" -Level 'INFO' }
        $pnp = Get-PnpDevice -Class USB -Status OK -ErrorAction SilentlyContinue | Select-Object FriendlyName, InstanceId -First 5
        if ($pnp) { Write-Host "  -- PnP USB (OK) --" -ForegroundColor DarkGray; $pnp | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Gray } }
    } catch {
        Write-Log -Message "Could not query USB: $($_.Exception.Message)" -Level 'WARNING'
    }
