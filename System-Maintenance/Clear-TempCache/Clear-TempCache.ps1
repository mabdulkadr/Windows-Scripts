...
try {
    ...
    # Tailored display: temp sizes
    try {
        $paths = @($env:TEMP, "$env:SystemRoot\Temp", "$env:SystemRoot\Prefetch")
        $rows = foreach ($p in $paths) { if (Test-Path $p) { $sz = (Get-ChildItem -Path $p -Recurse -ErrorAction SilentlyContinue | Measure-Object -Property Length -Sum).Sum; [PSCustomObject]@{ Path=$p; SizeMB=[math]::Round(($sz/1MB),1) } } }
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Temp Locations Size --" -ForegroundColor DarkGray
