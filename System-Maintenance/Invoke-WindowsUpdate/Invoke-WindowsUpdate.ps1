...
.SYNOPSIS
    Scan and install Windows Updates (WUA API)

.DESCRIPTION
    Uses Microsoft.Update.Session COM to search for pending updates (IsInstalled=0). Reports count and titles. With -WhatIf previews only; without it, downloads and installs. Requires Administrator for install.

.TAGS
    System,WindowsUpdate,WUA
...
.EXAMPLE
    .\Invoke-WindowsUpdate.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - System,WindowsUpdate,WUA
...
$SolutionName = 'Invoke-WindowsUpdate'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Scan Windows Update')) {
        ...
        if (-not $session) { throw "Microsoft.Update.Session COM not available" }
        $searcher = $session.CreateUpdateSearcher()
        $result = $searcher.Search("IsInstalled=0") 2>&1
        $count = if ($result.Updates) { $result.Updates.Count } else { 0 }
        Write-Log -Message "Found $count pending updates" -Level 'INFO'
...
try {
    ...
    # Tailored display: pending updates
    try {
        $session = New-Object -ComObject Microsoft.Update.Session -ErrorAction SilentlyContinue
        $searcher = $session.CreateUpdateSearcher()
        ...
        Write-Host "  -- Pending Windows Updates: $count --" -ForegroundColor DarkGray
        if ($count -gt 0) {
            $result.Updates | Select-Object -First 10 | ForEach-Object {
                [PSCustomObject]@{ Title=$_.Title.Substring(0,[Math]::Min(80,$_.Title.Length)); KB=($_.KBArticleIDs -join ','); Severity=$_.MsrcSeverity } | Format-Table -AutoSize -Wrap | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Yellow }
            }
        } else { Write-Host "  No pending updates - compliant" -ForegroundColor Green }
    } catch {
