...
try {
    ...
    # Tailored display: DISM health
    try {
        $health = & dism.exe /Online /Cleanup-Image /CheckHealth 2>&1 | Out-String
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- DISM CheckHealth --" -ForegroundColor DarkGray
        Write-Host ($health | Select-Object -First 15) -ForegroundColor Cyan
    } catch {
