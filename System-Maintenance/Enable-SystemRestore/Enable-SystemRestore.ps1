...
try {
    ...
    # Tailored display: system restore
    try {
        $sr = Get-ComputerRestorePoint -ErrorAction SilentlyContinue | Select-Object -Last 3
        $enabled = (Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\SystemRestore' -ErrorAction SilentlyContinue).RPSessionInterval
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- System Restore --" -ForegroundColor DarkGray
        if ($sr) { $sr | Select-Object CreationTime, Description | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
        else { Write-Host "  No restore points yet" -ForegroundColor Yellow }
    } catch {
