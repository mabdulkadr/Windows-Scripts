...
.DESCRIPTION
    Sets HiberbootEnabled=0 under Session Manager\Power to disable hybrid hibernate boot.
...
.EXAMPLE
    .\Disable-FastStartup.ps1 -TargetName "Server01","Server02"
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Disable FastStartup')) {
...
try {
    ...
    try {
        ...
        $color = switch ($val) { 0 { "Green" } 1 { "Yellow" } default { "Gray" } }
        ...
        Write-Host ("  Effect: {0}" -f $(if ($val -eq 0) {"Full shutdown (no hibernation) - slower boot, more reliable"} else {"Hybrid shutdown - faster boot, may hide driver issues"})) -ForegroundColor Gray
        ...
        Write-Host "  Action: Disabled Fast Startup - full shutdown now, reboot required for full effect" -ForegroundColor Green
        Write-Host "  Verify: powercfg /a  and  Get-ItemProperty ...HiberbootEnabled" -ForegroundColor Gray
        Write-Host "  Re-enable: .\Enable-FastStartup.ps1  or  Set HiberbootEnabled=1" -ForegroundColor Gray
