<#
.TITLE
    Watch-DiskSpace - Live disk space dashboard

.SYNOPSIS
    Live disk space dashboard

.DESCRIPTION
    Reports all logical disks with Free/Total GB, % free, and color-coded health (Green >20%, Yellow 10-20%, Red <10%). Suitable for monitoring dashboards.
...
.EXAMPLE
    .\Watch-DiskSpace.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - System,Disk,Monitoring
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Check Disk Space')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $disks = Get-CimInstance -ClassName Win32_LogicalDisk -Filter "DriveType=3" -ErrorAction SilentlyContinue
        if (-not $disks) { throw "No fixed disks found" }
...
try {
    ...
    try {
        ...
        foreach ($d in $disks) {
            ...
            Write-Host ("  {0}  {1,6} GB free / {2,6} GB total  ({3,5}% free)" -f $d.DeviceID, $freeGB, $totalGB, $pct) -ForegroundColor $color
        }
    } catch {
        Write-Log -Message "Could not query disks: $($_.Exception.Message)" -Level 'WARNING'
    }
