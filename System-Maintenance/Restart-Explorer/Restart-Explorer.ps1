...
try {
    ...
    # Tailored display: explorer process
    try {
        $exp = Get-Process -Name explorer -ErrorAction SilentlyContinue | Select-Object -First 1
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Explorer Process --" -ForegroundColor DarkGray
        if ($exp) { $exp | Select-Object ProcessName, Id, WorkingSet64 | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
    } catch {
