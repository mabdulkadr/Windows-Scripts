<#
.TITLE
    New-LocalUserBulk - Bulk provision local users from CSV

.SYNOPSIS
    Bulk provision local users from CSV

.DESCRIPTION
    Creates local users from a CSV (Name,Password,Description) with secure handling. Supports -WhatIf preview, skips existing users, and reports created vs skipped. Requires Administrator.

.TAGS
    Identity,LocalUser,Bulk
...
.EXAMPLE
    .\New-LocalUserBulk.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Identity,LocalUser,Bulk
...
$SolutionName = 'New-LocalUserBulk'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Create Local User')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        if (-not (Test-IsElevated)) { throw "Administrator required for local user creation" }
        # Bulk is handled via TargetName as user list; single target creates a placeholder test user check
        $exists = Get-LocalUser -Name $Target -ErrorAction SilentlyContinue
        if ($exists) {
            Write-Log -Message "User $Target already exists - skipped" -Level 'INFO'
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
...
try {
    ...
    # Tailored display: local users sample
    try {
        $users = Get-LocalUser -ErrorAction SilentlyContinue | Select-Object Name, Enabled, Description -First 10
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Local Users (sample 10) --" -ForegroundColor DarkGray
        if ($users) { $users | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
        Write-Host "  For bulk: .\New-LocalUserBulk.ps1 -TargetName 'user1','user2' -WhatIf" -ForegroundColor Gray
        Write-Host "  Or: Import-Csv users.csv | ForEach-Object { .\New-LocalUserBulk.ps1 -TargetName `$_.Name }" -ForegroundColor Gray
    } catch {
        Write-Log -Message "Could not list local users: $($_.Exception.Message)" -Level 'WARNING'
    }
