<#
.TITLE
    Join-Domain - Join the local computer to an AD domain

.SYNOPSIS
    Join the local computer to an AD domain

.DESCRIPTION
    Calls Add-Computer with -DomainName and supplied -Credential, then restarts the computer.
...
.EXAMPLE
    .\Join-Domain.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Identity,AD,Join
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Join domain')) {
...
try {
    ...
    # Tailored display: current domain membership
    try {
        $cs = Get-CimInstance -ClassName Win32_ComputerSystem -ErrorAction SilentlyContinue
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Domain Membership --" -ForegroundColor DarkGray
        $cs | Select-Object Name, Domain, PartOfDomain, Workgroup | Format-List | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
    } catch {
        Write-Log -Message "Could not query domain membership: $($_.Exception.Message)" -Level 'WARNING'
    }
