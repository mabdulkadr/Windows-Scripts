<#
.TITLE
    Get-ADUserDetails - Retrieve a detailed AD user record

.SYNOPSIS
    Retrieve a detailed AD user record

.DESCRIPTION
    Imports ActiveDirectory and retrieves all properties for the supplied SamAccountName.
...
.EXAMPLE
    .\Get-ADUserDetails.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Identity,AD,User
...
$SolutionName = 'Get-ADUserDetails'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Query AD user')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $null = Get-Command Get-ADUser -ErrorAction SilentlyContinue; if (-not $null) { throw 'RSAT AD module not found' }
...
try {
    ...
    # Tailored display: user details sample
    try {
        Import-Module ActiveDirectory -ErrorAction SilentlyContinue | Out-Null
        $sample = Get-ADUser -Filter * -Properties Enabled, LastLogonDate -ErrorAction SilentlyContinue | Select-Object SamAccountName, Enabled, LastLogonDate -First 5
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- AD User Sample --" -ForegroundColor DarkGray
        if ($sample) { $sample | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
        else { Write-Log -Message "No AD users found or module not available" -Level 'WARNING' }
    } catch {
        Write-Log -Message "Could not query AD users: $($_.Exception.Message)" -Level 'WARNING'
    }
