...
try {
    ...
    # Tailored display: secure channel + DC
    try {
        ...
        Write-Host "  -- Trust + DC --" -ForegroundColor DarkGray
        [PSCustomObject]@{ DiscoveredDC=$dc; SecureChannel=$sc } | Format-List | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
    } catch {
