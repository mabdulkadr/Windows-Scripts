<#
.TITLE
    Get-DomainStatus - Report current AD domain / forest / DC info

.SYNOPSIS
    Report current AD domain / forest / DC info

.DESCRIPTION
    Queries Win32_ComputerSystem to surface domain, role, and current user.
...
.EXAMPLE
    .\Get-DomainStatus.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Identity,AD,Status
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Query domain')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $cs = Get-CimInstance Win32_ComputerSystem -ErrorAction Stop | Select-Object -First 1; if (-not $cs.Domain) { throw 'Not domain-joined' }
...
try {
    ...
    # Tailored display: domain / forest
    try {
        $domain = (Get-CimInstance -ClassName Win32_ComputerSystem -ErrorAction SilentlyContinue).Domain
        $forest = try { (Get-ADForest -ErrorAction SilentlyContinue).Name } catch { "AD module not available" }
        $dc = try { (Get-ADDomainController -Discover -ErrorAction SilentlyContinue).HostName } catch { "unknown" }
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Domain Status --" -ForegroundColor DarkGray
        [PSCustomObject]@{ Domain=$domain; Forest=$forest; DiscoveredDC=$dc } | Format-List | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
    } catch {
        Write-Log -Message "Could not query domain status: $($_.Exception.Message)" -Level 'WARNING'
    }
