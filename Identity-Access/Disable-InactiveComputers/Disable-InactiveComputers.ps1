<#
.TITLE
    Disable-InactiveComputers - Disable AD computer accounts inactive for N days

.SYNOPSIS
    Disable AD computer accounts inactive for N days

.DESCRIPTION
    Filters AD computers by LastLogonDate and disables stale accounts.
...
.EXAMPLE
    .\Disable-InactiveComputers.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Identity,AD,Cleanup
...
$SolutionName = 'Disable-InactiveComputers'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Find inactive')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $cutoff = (Get-Date).AddDays(-90); $null = Get-Date -ErrorAction Stop | Out-Null
...
try {
    ...
    # Tailored display: stale computers
    try {
        Import-Module ActiveDirectory -ErrorAction SilentlyContinue | Out-Null
        $days = 90
        $cutoff = (Get-Date).AddDays(-$days)
        $stale = Get-ADComputer -Filter { LastLogonDate -lt $cutoff -and Enabled -eq $true } -Properties LastLogonDate -ErrorAction SilentlyContinue | Select-Object Name, LastLogonDate, DistinguishedName -First 10
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Stale Computers (> $days days) --" -ForegroundColor DarkGray
        if ($stale) { $stale | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
        else { Write-Log -Message "No stale enabled computers found or AD module not available" -Level 'INFO' }
    } catch {
        Write-Log -Message "Could not query AD stale computers: $($_.Exception.Message)" -Level 'WARNING'
    }
    # Export professional HTML report
    try {
        $dcAvail = try { Import-Module ActiveDirectory -ErrorAction SilentlyContinue | Out-Null; $true } catch { $false }
        $inactKpis = @(@{value=$ok; label='Targets OK'; color='#0f62fe'}, @{value=$failed; label='Failed'; color='#da1e28'}, @{value=$env:COMPUTERNAME; label='Device'; color='#8a3ffc'}, @{value=if($dcAvail){'Available'}else{'No RSAT'}; label='AD Module'; color='#24a148'})
        # Same data as console: stale computer sample from tailored display above ($sample or re-queried)
        $inactSamp = try { Import-Module ActiveDirectory -ErrorAction SilentlyContinue | Out-Null; Get-ADComputer -Filter {Enabled -eq $true} -Properties LastLogonDate -ErrorAction SilentlyContinue | Where-Object { $_.LastLogonDate -lt (Get-Date).AddDays(-90) } | Select-Object Name, LastLogonDate, Enabled -First 10 } catch { @() }
        $inactTable = if ($inactSamp) {
            $inactH = ($inactSamp[0].PSObject.Properties | ForEach-Object { "<th>$($_.Name)</th>" }) -join ""
            $inactB = ($inactSamp | ForEach-Object {
                $inactCs = ($_.PSObject.Properties | ForEach-Object {
                    $inactV = [System.Net.WebUtility]::HtmlEncode("$($_.Value)")
                    if ($_.Name -eq 'Enabled') { $inactBg = if ($inactV -eq 'True') { 'warning' } else { 'neutral' }; "<td><span class=`"badge $inactBg`">$inactV</span></td>" } else { "<td>$inactV</td>" }
                }) -join ""
                "<tr>$inactCs</tr>"
            }) -join "`n"
            "<table><thead><tr>$inactH</tr></thead><tbody>$inactB</tbody></table>"
        } elseif ($sample) {
            $inactH2 = ($sample[0].PSObject.Properties | ForEach-Object { "<th>$($_.Name)</th>" }) -join ""
            $inactB2 = ($sample | ForEach-Object { $inactCs2 = ($_.PSObject.Properties | ForEach-Object { "<td>$([System.Net.WebUtility]::HtmlEncode("$($_.Value)"))</td>" }) -join ""; "<tr>$inactCs2</tr>" }) -join "`n"
            "<table><thead><tr>$inactH2</tr></thead><tbody>$inactB2</tbody></table>"
        } else { "<p style=`"color:#8d8d8d;padding:16px`">No stale computers found or AD not available (same as console)</p>" }
        $inactBody = "<div class=`"section-title`">Inactive Computer Scan</div><div class=`"card`"><h2>Stale Computer Detection (same as console)</h2>$inactTable</div>"
        $inactBase = if ($PSScriptRoot) { $PSScriptRoot } elseif ($PSCommandPath) { Split-Path -Parent $PSCommandPath } else { (Get-Location).Path }
        $inactHtml = Join-Path $inactBase "Reports\InactiveComputers_$(Get-Date -Format yyyyMMdd_HHmmss).html"
        Export-ProfessionalHtmlReport -OutputPath $inactHtml -Title 'Inactive Computers' -Subtitle "Stale AD computer report" -Kpis $inactKpis -Body $inactBody -ReportName 'Inactive Computers' -Version '2.1.0'
        Write-Log -Message "HTML report: $inactHtml" -Level 'SUCCESS'
