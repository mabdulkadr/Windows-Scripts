<#
.TITLE
    Get-ADDeviceDetails - Retrieve a detailed AD computer record

.SYNOPSIS
    Retrieve a detailed AD computer record

.DESCRIPTION
    Imports ActiveDirectory and retrieves all properties for the supplied computer name.
...
.EXAMPLE
    .\Get-ADDeviceDetails.ps1
    Runs with default targets.

.EXAMPLE
    .\Get-ADDeviceDetails.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Identity,AD,Device
...
$SolutionName = 'Get-ADDeviceDetails'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Query AD device')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $null = Get-Command Get-ADComputer -ErrorAction SilentlyContinue; if (-not $null) { throw 'RSAT AD module not found' }
...
try {
    ...
    # Tailored display: device details
    try {
        Import-Module ActiveDirectory -ErrorAction SilentlyContinue | Out-Null
        $sample = Get-ADComputer -Filter * -Properties OperatingSystem, LastLogonDate, Enabled -ErrorAction SilentlyContinue | Select-Object Name, OperatingSystem, Enabled, LastLogonDate -First 5
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- AD Device Sample --" -ForegroundColor DarkGray
        if ($sample) { $sample | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan } }
        else { Write-Log -Message "No AD devices found or module not available" -Level 'WARNING' }
    } catch {
        Write-Log -Message "Could not query AD devices: $($_.Exception.Message)" -Level 'WARNING'
    }
