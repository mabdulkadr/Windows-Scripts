<#
.TITLE
    Remove-DomainJoin - Unjoin the local computer from its current AD domain

.SYNOPSIS
    Unjoin the local computer from its current AD domain

.DESCRIPTION
    Calls Remove-Computer with the supplied -Credential and restarts.
...
.EXAMPLE
    .\Remove-DomainJoin.ps1
    Runs with default targets.

.EXAMPLE
    .\Remove-DomainJoin.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Identity,AD,Unjoin
...
$SolutionName = 'Remove-DomainJoin'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Unjoin domain')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $null = Get-WmiObject -Class Win32_ComputerSystem -ErrorAction Stop | Select-Object -First 1
...
try {
    ...
    # Tailored display: current membership before unjoin
    try {
        $cs = Get-CimInstance -ClassName Win32_ComputerSystem -ErrorAction SilentlyContinue
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Current Membership (before unjoin) --" -ForegroundColor DarkGray
        $cs | Select-Object Name, Domain, PartOfDomain | Format-List | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
    } catch {
        Write-Log -Message "Could not query membership: $($_.Exception.Message)" -Level 'WARNING'
    }
