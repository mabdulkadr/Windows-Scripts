...
.SYNOPSIS
    Audit local Administrators members

.DESCRIPTION
    Enumerates local Administrators group members, flags dormant accounts, Guest, and unknown SIDs. Supports remote via WinRM.

.TAGS
    Utilities,Audit,LocalAdmin
...
.EXAMPLE
    .\Get-LocalAdminReport.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Utilities,Audit,LocalAdmin
...
$SolutionName = 'Get-LocalAdminReport'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Audit LocalAdmins')) {
        ...
        if (-not $members) { throw "Cannot enumerate Administrators group" }
...
try {
    ...
    # Tailored display: local admins
    try {
        $members = Get-LocalGroupMember -Group "Administrators" -ErrorAction SilentlyContinue | Select-Object Name, ObjectClass, PrincipalSource
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Local Administrators --" -ForegroundColor DarkGray
        if ($members) { $members | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Yellow } }
        $guest = Get-LocalUser -Name "Guest" -ErrorAction SilentlyContinue
        if ($guest) { Write-Host "  Guest account: Enabled=$($guest.Enabled)" -ForegroundColor $(if ($guest.Enabled) {'Red'} else {'Green'}) }
    } catch {
        Write-Log -Message "Could not audit local admins: $($_.Exception.Message)" -Level 'WARNING'
    }
    # Export professional HTML report
    try {
        $admins = try { Get-LocalGroupMember -Group "Administrators" -ErrorAction SilentlyContinue | Select-Object Name, PrincipalSource -First 20 } catch { @() }
        $adminCount = @($admins).Count
        $adminKpis = @(@{value=$adminCount; label='Admin Members'; color='#0f62fe'}, @{value=$ok; label='Checks OK'; color='#24a148'}, @{value=$failed; label='Failed'; color='#da1e28'}, @{value=$env:COMPUTERNAME; label='Device'; color='#8a3ffc'})
        $adminRows = if($admins){ $h=($admins[0].PSObject.Properties | ForEach-Object { "<th>$($_.Name)</th>" }) -join ""; $b=($admins | ForEach-Object { $cs=($_.PSObject.Properties | ForEach-Object { "<td>$([System.Net.WebUtility]::HtmlEncode("$($_.Value)"))</td>" }) -join ""; "<tr>$cs</tr>" }) -join "`n"; "<table><thead><tr>$h</tr></thead><tbody>$b</tbody></table>" } else { "<p style=`"color:#8d8d8d;padding:16px`">No admin members</p>" }
        $adminBody = "<div class=`"section-title`">Local Administrators</div><div class=`"card`"><h2>Administrators Group ($adminCount members)</h2>$adminRows</div>"
        $adminBase = if ($PSScriptRoot) { $PSScriptRoot } elseif ($PSCommandPath) { Split-Path -Parent $PSCommandPath } else { (Get-Location).Path }
        $adminHtml = Join-Path $adminBase "Reports\LocalAdmins_$(Get-Date -Format yyyyMMdd_HHmmss).html"
        Export-ProfessionalHtmlReport -OutputPath $adminHtml -Title 'Local Admins' -Subtitle "Local Administrators group report" -Kpis $adminKpis -Body $adminBody -ReportName 'Local Administrators' -Version '2.1.0'
        Write-Log -Message "HTML report: $adminHtml" -Level 'SUCCESS'
