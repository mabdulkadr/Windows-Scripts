...
.SYNOPSIS
    Run command on remote machines (WinRM parallel)

.DESCRIPTION
    Runs a PowerShell ScriptBlock on one or more remote machines via WinRM (Invoke-Command). Supports bulk -TargetName list, parallel ThrottleLimit, and -WhatIf preview. Requires WinRM and permissions.

.TAGS
    Utilities,WinRM,Remote
...
.EXAMPLE
    .\Invoke-RemoteCommand.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Utilities,WinRM,Remote
...
$SolutionName = 'Invoke-RemoteCommand'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Invoke Remote')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        try {
            $sb = [scriptblock]::Create("Get-Service -Name WinRM -ErrorAction SilentlyContinue | Select-Object -First 1")
            $res = Invoke-Command -ComputerName $Target -ScriptBlock $sb -ErrorAction Stop
            if (-not $res) { throw "No response from $Target" }
        } catch {
            throw "WinRM to $Target failed: $($_.Exception.Message)"
        }
...
try {
    ...
    # Tailored display: WinRM
    try {
        $local = $env:COMPUTERNAME
        $winrm = Get-Service -Name WinRM -ErrorAction SilentlyContinue
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- WinRM --" -ForegroundColor DarkGray
        Write-Host "  Local: $local  WinRM: $(if ($winrm.Status) { $winrm.Status } else { 'NotFound' })" -ForegroundColor Cyan
        Write-Host "  Targets: $($targets -join ', ')" -ForegroundColor Gray
        Write-Host "  Example: .\Invoke-RemoteCommand.ps1 -TargetName 'PC01','PC02' -WhatIf" -ForegroundColor Gray
        Write-Host "  Or scriptblock: Invoke-Command -ComputerName PC01 -ScriptBlock { Get-Service Spooler }" -ForegroundColor Gray
    } catch {
        Write-Log -Message "Could not show WinRM info: $($_.Exception.Message)" -Level 'WARNING'
    }
