<#
.TITLE
    Find-LargeFiles - Find largest files (Top N + CSV)

.SYNOPSIS
    Find largest files (Top N + CSV)

.DESCRIPTION
    Scans a directory recursively for largest files (default Top 20, -TargetName as path), reports size, and exports CSV to Reports. Supports -WhatIf (no export). Skips access-denied folders.

.TAGS
    Utilities,Storage,Reporting

.PLATFORM
    Windows 10/11/Server 2019+

.PERMISSIONS
    Standard user (some folders need elevation)
...
.CHANGELOG
    2.1.0 (2026-08-31)
    - Initial release - Phase 2 expansion (tailored per-script output)
...
.EXAMPLE
    .\Find-LargeFiles.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Utilities,Storage,Reporting
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Find Large Files')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $scanPath = if ($Target -and (Test-Path $Target)) { $Target } else { $env:SystemDrive + "\" }
        $files = Get-ChildItem -Path $scanPath -Recurse -File -ErrorAction SilentlyContinue | Sort-Object Length -Descending | Select-Object -First 5
        if (-not $files) { throw "No files found under $scanPath" }
...
try {
    ...
    # Tailored display: largest files
    try {
        $scanPath = if ($TargetName -and (Test-Path $TargetName[0])) { $TargetName[0] } else { "$env:SystemDrive\" }
        $top = Get-ChildItem -Path $scanPath -Recurse -File -ErrorAction SilentlyContinue | Sort-Object Length -Descending | Select-Object FullName, @{N='SizeMB';E={[math]::Round($_.Length/1MB,1)}}, LastWriteTime -First 10
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Largest Files under $scanPath (Top 10) --" -ForegroundColor DarkGray
        if ($top) {
            $top | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
            try {
                ...
                $csv = Join-Path $reports "LargeFiles_$(Get-Date -Format yyyyMMdd_HHmmss).csv"
                $top | Export-Csv -Path $csv -NoTypeInformation -Encoding UTF8
                Write-Host "  CSV: $csv" -ForegroundColor Green
            } catch { Write-Log -Message "CSV export failed: $($_.Exception.Message)" -Level 'WARNING' }
        }
    } catch {
        Write-Log -Message "Could not find large files: $($_.Exception.Message)" -Level 'WARNING'
    }
# Export professional HTML report
    try {
        $lfScanPath = if ($TargetName -and (Test-Path -LiteralPath $TargetName[0])) { $TargetName[0] } else { "$env:SystemDrive\" }
        $largestFiles = Get-ChildItem -Path $lfScanPath -Recurse -File -ErrorAction SilentlyContinue | Sort-Object Length -Descending | Select-Object -First 25 FullName, @{N='SizeMB';E={[math]::Round($_.Length/1MB,2)}}, @{N='SizeGB';E={[math]::Round($_.Length/1GB,3)}}, Extension, LastWriteTime
        $lfCount = @($largestFiles).Count
        $lfTotalGB = if ($largestFiles) { [math]::Round(($largestFiles | Measure-Object Length -Sum).Sum / 1GB, 2) } else { 0 }
        $lfTopExt = if ($largestFiles) { ($largestFiles | Group-Object Extension | Sort-Object Count -Descending | Select-Object -First 1).Name } else { 'n/a' }
        $lfKpis = @(@{value="$lfTotalGB GB"; label='Top 25 Total'; color='#0f62fe'}, @{value=$lfCount; label='Files Listed'; color='#24a148'}, @{value=$lfTopExt; label='Top Extension'; color='#8a3ffc'}, @{value=$env:COMPUTERNAME; label='Device'; color='#f1c21b'})
        $lfRows = if($largestFiles){ $b=($largestFiles | ForEach-Object { $name=[System.Net.WebUtility]::HtmlEncode($_.FullName); $ext=if($_.Extension){$_.Extension}else{'(none)'}; $dt=$_.LastWriteTime.ToString('yyyy-MM-dd HH:mm'); "<tr><td><code>$name</code></td><td>$($_.SizeMB) MB</td><td>$($_.SizeGB) GB</td><td>$ext</td><td>$dt</td></tr>" }) -join "`n"; "<table><thead><tr><th>Path</th><th>Size (MB)</th><th>Size (GB)</th><th>Ext</th><th>Modified</th></tr></thead><tbody>$b</tbody></table>" } else { "<p style=`"color:#8d8d8d;padding:16px`">No large files found</p>" }
        $lfExtRows = if($largestFiles){ $extGroups=$largestFiles | Group-Object Extension | Sort-Object @{E={($_.Group | Measure-Object Length -Sum).Sum}; Desc=$true} | Select-Object -First 10; $b=($extGroups | ForEach-Object { $total=[math]::Round((($_.Group | Measure-Object Length -Sum).Sum)/1GB,3); "<tr><td>$($_.Name)</td><td>$($_.Count)</td><td>$total GB</td></tr>" }) -join "`n"; "<table><thead><tr><th>Extension</th><th>File Count</th><th>Combined Size (GB)</th></tr></thead><tbody>$b</tbody></table>" } else { '' }
        $lfDirRows = if($largestFiles){ $dirGroups=$largestFiles | ForEach-Object { Split-Path -Parent $_.FullName } | Group-Object | Sort-Object Count -Descending | Select-Object -First 10; $b=($dirGroups | ForEach-Object { "<tr><td><code>$([System.Net.WebUtility]::HtmlEncode($_.Name))</code></td><td>$($_.Count)</td></tr>" }) -join "`n"; "<table><thead><tr><th>Directory</th><th>Files Found</th></tr></thead><tbody>$b</tbody></table>" } else { '' }
        $lfBody = "<div class=`"section-title`">Large File Scan</div><div class=`"card`"><h2>Scan Path: <code>$([System.Net.WebUtility]::HtmlEncode($lfScanPath))</code></h2><p style=`"color:#c6c6c6`">Top 25 largest files found by recursive scan (sorted by size descending).</p>$lfRows</div><div class=`"grid-2`"><div class=`"card`"><h2>By Extension</h2>$lfExtRows</div><div class=`"card`"><h2>Top Directories</h2>$lfDirRows</div></div>"
        $lfBase = if ($PSScriptRoot) { $PSScriptRoot } elseif ($PSCommandPath) { Split-Path -Parent $PSCommandPath } else { (Get-Location).Path }
        $lfHtml = Join-Path $lfBase "Reports\LargeFiles_$(Get-Date -Format yyyyMMdd_HHmmss).html"
        Export-ProfessionalHtmlReport -OutputPath $lfHtml -Title 'Large Files' -Subtitle "Top 25 largest files under $lfScanPath" -Kpis $lfKpis -Body $lfBody -ReportName 'Large Files' -Version '2.1.0'
        Write-Log -Message "HTML report: $lfHtml" -Level 'SUCCESS'
