<#
.TITLE
    Get-ScheduledTaskReport - Audit scheduled tasks (suspicious)

.SYNOPSIS
    Audit scheduled tasks (suspicious)

.DESCRIPTION
    Enumerates scheduled tasks, flags non-Microsoft publishers and tasks with suspicious actions (powershell, rundll32, mshta). Supports -NonMicrosoftOnly.

.TAGS
    Utilities,ScheduledTask,Audit
...
.EXAMPLE
    .\Get-ScheduledTaskReport.ps1 -TargetName "Server01","Server02"
    Runs against the specified targets.

.NOTES
    Part of Windows-Scripts toolkit - Utilities,ScheduledTask,Audit
...
$SolutionName = 'Get-ScheduledTaskReport'
$ScriptMode   = 'run'
...
function Invoke-TargetAction {
    ...
    try {
        if (-not $PSCmdlet.ShouldProcess($Target, 'Audit Tasks')) {
            return [PSCustomObject]@{ Target = $Target; Success = $true; Skipped = $true }
        }
        $tasks = Get-ScheduledTask -ErrorAction SilentlyContinue
        if (-not $tasks) { throw "No scheduled tasks found" }
...
try {
    ...
    # Tailored display: scheduled tasks
    try {
        $tasks = Get-ScheduledTask -ErrorAction SilentlyContinue | Select-Object -First 20
        $suspicious = $tasks | Where-Object { $_.Actions.Execute -match "powershell|pwsh|rundll32|mshta" } | Select-Object TaskName, TaskPath -First 10
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Scheduled Tasks (sample 20) --" -ForegroundColor DarkGray
        $tasks | Select-Object TaskName, State -First 10 | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
        if ($suspicious) {
            Write-Host "  -- Suspicious --" -ForegroundColor Yellow
            $suspicious | Format-Table -AutoSize | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Yellow }
        } else { Write-Host "  No suspicious task actions found" -ForegroundColor Green }
    } catch {
        Write-Log -Message "Could not audit tasks: $($_.Exception.Message)" -Level 'WARNING'
    }
    # Export professional HTML report
    try {
        $schTasks = Get-ScheduledTask -ErrorAction SilentlyContinue | Where-Object { $_.TaskName -notlike "*Microsoft*" } | Select-Object TaskName, TaskPath, State -First 20
        $schCount = @($schTasks).Count; $schReady = @($schTasks | Where-Object State -eq 'Ready').Count
        $schKpis = @(@{value=$schCount; label='Tasks'; color='#0f62fe'}, @{value=$schReady; label='Ready'; color='#24a148'}, @{value=($schCount-$schReady); label='Other'; color='#f1c21b'}, @{value=$env:COMPUTERNAME; label='Device'; color='#8a3ffc'})
        $schRows = if($schTasks){ $h="<th>Task</th><th>Path</th><th>State</th>"; $b=($schTasks | ForEach-Object { $bg=if($_.State -eq 'Ready'){'success'}else{'neutral'}; "<tr><td>$([System.Net.WebUtility]::HtmlEncode($_.TaskName))</td><td>$([System.Net.WebUtility]::HtmlEncode($_.TaskPath))</td><td><span class=`"badge $bg`">$($_.State)</span></td></tr>" }) -join "`n"; "<table><thead><tr>$h</tr></thead><tbody>$b</tbody></table>" } else { "<p style=`"color:#8d8d8d;padding:16px`">No tasks found</p>" }
        $schBody = "<div class=`"section-title`">Scheduled Tasks</div><div class=`"card`"><h2>Task Report ($schCount)</h2>$schRows</div>"
        $schBase = if ($PSScriptRoot) { $PSScriptRoot } elseif ($PSCommandPath) { Split-Path -Parent $PSCommandPath } else { (Get-Location).Path }
        $schHtml = Join-Path $schBase "Reports\ScheduledTasks_$(Get-Date -Format yyyyMMdd_HHmmss).html"
        Export-ProfessionalHtmlReport -OutputPath $schHtml -Title 'Scheduled Tasks' -Subtitle "Scheduled task report" -Kpis $schKpis -Body $schBody -ReportName 'Scheduled Tasks' -Version '2.1.0'
        Write-Log -Message "HTML report: $schHtml" -Level 'SUCCESS'
