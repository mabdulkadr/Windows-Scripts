...
try {
    ...
    # Tailored display: elevation check
    try {
        $elev = Test-IsElevated
        $user = [Security.Principal.WindowsIdentity]::GetCurrent().Name
        Write-Host "" -ForegroundColor Gray
        Write-Host "  -- Elevation --" -ForegroundColor DarkGray
        [PSCustomObject]@{ User=$user; IsElevated=$elev; UAC=(Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System' -Name EnableLUA -ErrorAction SilentlyContinue).EnableLUA } | Format-List | Out-String | ForEach-Object { Write-Host $_ -ForegroundColor Cyan }
    } catch {
